...
class VerificarSesionVac {
    ...
    function esEnSupervisores() {
        ...
        if ($ibm == $ibmPerNom || $ibm == $ibmPerRoot || $ibm == $ibmPerRoot2 || $ibm == $ibmPerRoot3 || $ibm == $ibmPerRoot4 || $ibm == $ibmPerRoot5) {
            return;
        }
    ...
    function esEnSupIntendente() {
        ...
        if ($ibm == $ibmPerNom || $ibm == $ibmPerRoot || $ibm == $ibmPerRoot3) {
            return;
        }
    ...
    function contarCorrecciones() {
        ...
        $stmt = "SELECT COUNT(*) AS total 
                ...
                WHERE enc.Vc_revisado = 0
                AND enc.Vc_autorizado <> 2
                AND enc.Vc_autorizado = 1";
    ...
    function contarPorFirma(){
        ...
        $stmt = "SELECT COUNT(*) AS total
                    FROM TLX002MXDB.dbo.tblMXPRVacacionesEnc enc
                    WHERE enc.Vc_firmaRI IS NULL
                    AND enc.Vc_revisado = 1
                    AND enc.Vc_autorizado <> 2";
        $resStmt = sqlsrv_query($conn, $stmt);        
