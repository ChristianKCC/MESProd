...
class Vacaciones {
  async consulta() {
    ...
    const respuestaRaw = await fetch(
      `php/index.php?tblVacacionesEnc&fecha=${fechaFiltro}&depto=${departamento}&ibm=${ibmFiltro}`,
    );
    ...
    respuesta.forEach((folio) => {
      ...
      // Estado de prima dominical
      let estadoClassPD = "badge bg-secondary text-white";
      let estadoTextoPD = "Prima vacacional no pagada";

      if (folio.Vc_primaDomPagada == 1) {
        estadoClassPD = "badge bg-success";
        estadoTextoPD = "Prima vacacional pagada";
      }
      ...
      // Boton / badge de prima dominical
      let botonPrimaDom = "";
      if (folio.Vc_primaDomPagada == 1) {
        botonPrimaDom = `
                    <span class="badge bg-primary">
                         ${folio.Vc_primaDomSemana}
                    </span>`;
      } else if (folio.Vc_revisado == 1) {
        botonPrimaDom = `
                    <button class="btn btn-warning btn-sm"
                            onclick="abrirPrimaDominical(${folio.id}, '${folio.nombre}')">
                        <i class="fa-solid fa-dollar-sign"></i> Pagar P. Vac.
                    </button>`;
      } else {
        botonPrimaDom = `
                    <span class="badge bg-secondary text-white">
                        <i class="fa-solid fa-lock"></i> Valida primero
                    </span>`;
      }
      ...
      let row = `
                <tr>
                    ...
                    <td>
                        ...
                        <span class="${estadoClassPD}">${estadoTextoPD}</span>
                    </td>
                    <td>${accionesHtml}</td>
                </tr>
            `;
...
window.autorizarVac = async function (id, accion) {
  ...
  if (respuesta === "Listo") {
    Swal.fire(
      "Éxito",
      `La solicitud con folio ${id} fue procesada.`,
      "success",
...
window.comprobarDias = async function (folio, nombre, departamento, noemp) {
  try {
    const respDias = await fetch(
      `php/index.php?diasSolicitados&folio=${folio}`,
    );
    ...
    for (const d of dias) {
      ...
      const respHoras = await fetch(
        `php/index.php?datoshoraysalida&noemp=${noemp}&fechabien=${d.fecha}`,
      );
    ...
    Swal.fire(
      "Error",
      "No se pudo recuperar los días: " + err.message,
      "error",
    );
  }
};
...
window.eliminarRegistro = async function (id) {
  ...
  const confirm = await Swal.fire({
    ...
    text: `Se eliminará tu solicitud con el folio: ${id} y todos sus registros asociados.`,
    icon: "warning",
...
window.abrirPrimaDominical = function (id, nombre) {
  document.getElementById("primaDomFolio").value = id;
  document.getElementById("primaDomSemana").value = "";
  document.getElementById("primaDomInfo").innerHTML = `
        <p><strong>Folio:</strong> ${id}<br>
        <strong>Empleado:</strong> ${nombre}</p>
    `;
  const modal = new bootstrap.Modal(document.getElementById("modalPrimaDom"));
  modal.show();
};

window.guardarPrimaDominical = async function () {
  const id = document.getElementById("primaDomFolio").value;
  const semana = document.getElementById("primaDomSemana").value.trim();

  if (!semana) {
    Swal.fire("UPS!!!", "Debes ingresar el identificador o semana", "info");
    return;
  }

  try {
    const respRaw = await fetch(
      `php/index.php?pagarPrimaDominical&id=${id}&semana=${encodeURIComponent(semana)}`,
    );
    const resp = await respRaw.json();

    if (resp.success) {
      bootstrap.Modal.getInstance(
        document.getElementById("modalPrimaDom"),
      ).hide();
      Swal.fire(
        "Éxito",
        `Prima Vacacional registrada para el folio ${id}.`,
        "success",
      ).then(() => window.Vacaciones.consulta());
    } else {
      Swal.fire("Error", "No se pudo registrar la prima vacacional", "error");
    }
  } catch (err) {
    Swal.fire("Error", "Fallo en la petición: " + err.message, "error");
  }
