...
//         // Recuperar fecha desde data-fecha
//         if (valor) {
//             const fechaAttr = sel.getAttribute('data-fecha'); 
//             if (fechaAttr) {
//                 const fecha = new Date(fechaAttr); // formato Y-m-d
//                 // Última fecha (máxima)
//                 if (!ultimaFecha || fecha > ultimaFecha) {
//                     ultimaFecha = fecha;
//                 }
//                 // Primera fecha (mínima)
...
//     // Ajustar campo vacaciones_hasta (última fecha)
//     if (ultimaFecha) {
//         const inputHasta = document.getElementById("vacaciones_hasta");
//         if (inputHasta) {
//             const isoFecha = ultimaFecha.toISOString().split('T')[0]; 
//             inputHasta.value = isoFecha;
//             console.log("Última fecha seleccionada para vacaciones_hasta:", isoFecha);
//         }
//     }

//     // Ajustar campo dias_habiles_partir (primera fecha)
//     if (primeraFecha) {
//         const inputDesde = document.getElementById("periodo_de");
//         if (inputDesde) {
//             const isoFecha = primeraFecha.toISOString().split('T')[0]; 
//             inputDesde.value = isoFecha;
//             console.log("Primera fecha seleccionada para dias_habiles_partir:", isoFecha);
//         }
//     }
// }
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    function recalcularContadores() {
        let vacaciones = 0, reposicionFestivo = 0, descanso = 0;
        let ultimaFecha = null, primeraFecha = null;
        ...
        document.querySelectorAll('.calendario select').forEach(sel => {
            ...
            const td = sel.closest('td');
            if (td) td.style.backgroundColor = (valor && colores[valor]) ? colores[valor] : "";
        ...
        if (ultimaFecha) {
            const inputHasta = document.getElementById("vacaciones_hasta");
            if (inputHasta) inputHasta.value = ultimaFecha.toISOString().split('T')[0];
        }
        if (primeraFecha) {
            const inputDesde = document.getElementById("periodo_de");
            if (inputDesde) inputDesde.value = primeraFecha.toISOString().split('T')[0];
        }

        // Badge de control de días
        // if (badgeDiasV) {
        //     const limite = getLimiteAntiguedad();
        //     badgeDiasV.textContent = `Días V: ${vacaciones} / ${isFinite(limite) ? limite : '∞'}`;
        //     badgeDiasV.classList.remove('bg-secondary','bg-success','bg-danger','bg-warning','text-dark');
        //     if (vacaciones > limite)        badgeDiasV.classList.add('bg-danger');
        //     else if (vacaciones === limite) badgeDiasV.classList.add('bg-warning','text-dark');
        //     else                            badgeDiasV.classList.add('bg-success');
        // }
        // Badge de control de días
        // if (badgeDiasV) {
        //     const limite = getLimiteAntiguedad();
        //     const TipoSol = document.getElementById("tipo_solicitud").value;
        ...
        //         }
        //     }
        //     else if(TipoSol == "Adelanto") {
        //         badgeDiasV.textContent = `Tu tipo de solicitud (${TipoSol}) admite n cantidad de días`;
        //         badgeDiasV.classList.add('bg-warning');                            
        //     }
        // }
        if (badgeDiasV) {
            badgeDiasV.classList.remove('bg-secondary','bg-success','bg-danger','bg-warning','bg-info','text-dark');
        ...
        if (sel.value === 'V' && aplicaLimite()) {
            ...
            if (contarV() > limite) {
                ...
                Swal.fire({
                    ...
                    html: `No puedes seleccionar más días de <b>Vacaciones (V)</b> ` +
    ...
    function bindSelectsDia(scope) {
        (scope || document).querySelectorAll('.calendario select').forEach(sel => {
    ...
    if (contExtra) {
        contExtra.addEventListener('click', function (e) {
            ...
            const tieneSeleccion = [...mes.querySelectorAll('.calendario select')]
                .some(s => s.value !== '');
            ...
            if (tieneSeleccion) {
                Swal.fire({
                    ...
                    if (r.isConfirmed) { mes.remove(); recalcularContadores(); }
                });
            } else {
                mes.remove();
                recalcularContadores();
