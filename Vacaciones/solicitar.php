...
$IbmRecuperado = $_POST["ibm"] ?? ($_SESSION['solicitud']['ibm'] ?? $_SESSION['ibm']);
$NombreRecuperado = $_POST["nombre"] ?? ($_SESSION['solicitud']['nombre'] ?? $_SESSION['nombre']);
$DiasRecuperado = $_POST["dias"] ?? ($_SESSION['solicitud']['limite_dias'] ?? $_SESSION['dias']);
$EmpleadoRecuperado = $_POST["empleado"] ?? ($_SESSION['solicitud']['tipo_empleado'] ?? $_SESSION['tipo_empleado']);
$fecha_ingreso = $_POST["fingreso"] ?? ($_SESSION['solicitud']['fingreso'] ?? $_SESSION['fingreso']);
  ...
  <div style="float:left" class="row">
    <div class="px-4">
      <small class="alert alert-primary">
        ...
        Desde esta sección marca los días de vacaciones deseados haciendo clic en la fecha de inicio y arrastra con el
        <code>MOUSE</code> hasta el dia de finalización de tus vacaciones, cuando finalices da clic en 'Continuar con el
        proceso'.
      </small>
    </div>
  </div>
    ...
    <div class="row ">
      <div class="col-md-2">
        <div class="card">
          ...
          <div class="card-body">
            <?php if ($empleado): ?>
              <p><strong>Nombre:</strong> <code><?= $NombreRecuperado ?></code></p>
              <p><strong>IBM:</strong> <code><?= $IbmRecuperado ?></code></p>
              <p><strong>Días disponibles:</strong> <code><?= $DiasRecuperado ?> </code></p>
              <?php
              $tipo = $EmpleadoRecuperado;
              if ($tipo === "EMPL") {
          ...
          <div class="card-body">
            ...
            <div class="d-flex justify-content-between gap-3 mt-4">
              <form id="formSolicitud" action="finalizarSolicitud.php" method="POST">
                ...
                <input type="hidden" id="ibmSolicitud"
                  value="<?php echo htmlspecialchars($_POST['ibm'] ?? ($ibmSession ?? '')); ?>">
                ...
                <input type="hidden" name="limite_dias" id="limite-dias" value="<?= $DiasRecuperado ?>">
                <input type="hidden" name="tipo_empleado" id="tipo-empleado" value="<?= col($empleado, COL_TIPO) ?>">
                <input type="hidden" name="fecha_ingreso" id="fecha-ingreso"
                  value="<?= col($empleado, COL_FINGRESO) ?>">
                <input type="hidden" name="origen" id="origen" value="solicitar.php">
                ...
                <div class="d-flex gap-3">
                  ...
                  <button type="button" id="btn-unselect" class="btn btn-primary rounded-3">
                    <i class="fa-solid fa-arrow-rotate-right"></i> Quitar días
                  </button>
                </div>
              </form>
              ...
              <form action="Consulta.php" method="POST">
                <button type="submit" id="btn-regresar" class="btn btn-danger rounded-3">
                  <i class="fa-solid fa-arrow-rotate-left"></i> Regresar a la consulta
                </button>
              </form>
            </div>
          </div>
          ...
          <div class="card-body">
            <div class="d-flex flex-column gap-3">
              <div class="d-flex align-items-center">
                <div style="width:20px; height:20px; background-color:#fff3cd; border:1px solid #000000;" class="me-2">
                </div>
                <span>Dia actual</span>
              </div>
              <div class="d-flex align-items-center">
                <div style="width:20px; height:20px; background-color:#cdd9ff; border:1px solid #000000;" class="me-2">
                </div>
                <span>Pendiente</span>
              </div>
              <div class="d-flex align-items-center">
                <div style="width:20px; height:20px; background-color:#d4edda; border:1px solid #000000;" class="me-2">
                </div>
                <span>Aprobado</span>
              </div>
              <div class="d-flex align-items-center">
                <div style="width:20px; height:20px; background-color:#ff6666; border:1px solid #000000;" class="me-2">
                </div>
                <span>Dias Festivos</span>
              </div>
              <div class="d-flex align-items-center">
                <span>Los dias 'Pendiente' no podran volverse a seleccionar a menos que sean
                  rechazados.</span>
              </div>
            </div>
          </div>
