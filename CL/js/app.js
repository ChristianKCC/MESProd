...
(function () {
  ...
  function watchIframe(iframe, wrapper, barHeight, bgColor) {
    ...
    /* Reaplicar cuando el iframe recargue (cambio de archivo, etc.) */
    iframe.addEventListener('load', function () {
      applyBarCover(wrapper, barHeight, bgColor);
      setTimeout(function () { applyBarCover(wrapper, barHeight, bgColor); }, 800);
      setTimeout(function () { applyBarCover(wrapper, barHeight, bgColor); }, 2000);
    ...
    /* SharePoint re-renderiza al filtrar/ordenar SIN disparar 'load':
      reaplicamos la capa cada cierto tiempo para que la barra no reaparezca */
    setInterval(function () { applyBarCover(wrapper, barHeight, bgColor); }, 1500);
  }
  ...
  if (xlWrapper && xlIframe) {
    ...
    watchIframe(xlIframe, xlWrapper, 10, '#1e1e22');
  }
