class Proact extends Herramientas {
    inicio() {
        document.getElementById('observacion').addEventListener('change',()=>{
            ...
            observacion == '' ?  document.getElementById('observacionreal').innerHTML = '' :
            this.llnarslcruta("php/index.php?Observacionreal&id="+observacion, "observacionreal")
        });
        this.tblpro();
                ...
                if (result.isConfirmed) {
                    (async () => {
                        ...
                        if (respuesta == "Vacios") {
                            ...
                            Swal.fire('UPS!!!', 'Hay problemas al insertar a la base de datos', 'error');
                        }
                        else {
                            Swal.fire('LISTO!!!', 'Información guardada con exito', 'success');
                            form.reset();
                            proact.tblpro();
                        }
                    })();
    ...
    reporte() {
        ...
        document.getElementById("consulta").addEventListener("click", function (event) {
            ...
            data.append("data", form);
            (async () => {
                const [respuestaraw, respuestaraw2, respuestaraw3, respuestaraw4, respuestaraw5, datos] = await Promise.all([
                ...
                respuestadatos.forEach(elemento => {
                    body += `<tr><td>${elemento.id}</td><td>${elemento.Observador}</td><td>${elemento.Observacion}</td><td>${elemento.Observado}</td>
    ...
    reportexavance() {
        ...
        document.getElementById("consulta").addEventListener("click", function (event) {
            ...
            const datef = document.getElementById("fechai").value;
            if (datei == "" || datef == "") {
                document.getElementById("resultado").innerHTML = "<div class='alert alert-danger'>Debes seleccionar las fechas</div>";
                return false;
            }
            (async () => {
                const form = document.getElementById("reporteLSW");
