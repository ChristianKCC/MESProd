...
class Reporte60 {
    async consulta() {
        ...
        respuesta.forEach(row => {
            ...
            const horasRegl = horasReglamentarias || "No hay identificación de turno";
            // Horas por folio
            const horasExtras = row.totalHorasExtrasSolicitadas ?? 0;
            // Horas individuales
            const horasIndividuales = row.horasExtrasRegistro ?? 0;
            const horasTotales = parseFloat(horasReglamentarias) + parseFloat(horasIndividuales);

            let tipoRegistro = "";
            if (row.esDoblete == 1 && horasTotales >= 60.5) {
                tipoRegistro = "<span class='badge bg-primary text-white'>≥ 60.5 hrs y Doblete</span>";
            } else if (row.esDoblete == 1) {
                tipoRegistro = "<span class='badge bg-warning text-dark'>Doblete</span>";
            } else if (horasTotales >= 60.5) {
                tipoRegistro = "<span class='badge bg-success'>≥ 60.5 hrs</span>";
            } else {
                tipoRegistro = "<span class='badge bg-secondary text-dark'>Otro</span>";
            }
...
document.getElementById("consultar").addEventListener("click", (e) => {
    ...
    window.Reporte60.consulta().then(() => {
