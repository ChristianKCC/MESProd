...
class BitacoraElectronica
{
	...
	function getDataNow($maquina)
	{
		...
		RechazosCorrida,TiempoarribaTurno,TiempoabajoTurno,idBitacora,Enhebrando FROM tblBitacoraMaquinas WHERE Maquina=$maquina ORDER BY id DESC";
		$result = sqlsrv_query($conn, $query);
		$array = array();
		$tarriba = 0;
		$tabajo = 0;
		$tenhebrando = 0;
		while ($row = sqlsrv_fetch_array($result)) {
			if ($maquina >= 60 && $maquina <= 65) {
				$tarriba = $row['TiempoarribaTurno'] / 60;
				$tabajo = $row['TiempoabajoTurno'] / 60;
				$tenhebrando = $row['Enhebrando'] / 60;
			} else {
				$tarriba = $row['TiempoarribaTurno'];
				$tabajo = $row['TiempoabajoTurno'];
				$tenhebrando = $row['Enhebrando'];
			}
			array_push($array, [
				'turno' => $row['Turno'],
				...
				'TiempoarribaTurno' => $tarriba,
				'TiempoabajoTurno' => $tabajo,
				'Enhebrando' => $tenhebrando,
				'idBitacora' => $row['idBitacora']
			]);
		}
		echo json_encode($array);
	...
	function normalizarVelocidad($v)
	{
		$min = 100;
		$max = 1000;
		$escalaMin = 1;
		$escalaMax = 10;
		return (($v - $min) / ($max - $min)) * ($escalaMax - $escalaMin) + $escalaMin;
	}
	function GetDataMonitor($maquina)
	{
		$Conecta = new ClassConexion();
		$conn = $Conecta->conexion("TLX004MXDB");
		$hora = $Opera = $merma = $velocidad = array();
		...
		while ($row = sqlsrv_fetch_array($result)) {
			...
			$vel = $this->normalizarVelocidad($row["VelocidadActual"]);
			...
			array_push($velocidad, number_format($vel, 2));
		}
		$respuesta = ["hora" => $hora, "datos" => $Opera, "merma" => $merma, "velocidad" => $velocidad];
	...
	function abreturno()
	{
		...
		while ($row = sqlsrv_fetch_array($res)) {
			array_push($array, ['id' => $row['IdEncabezadoBItacora'], 'turno' => $row['Turno'], 'horasTrabajadas' => $row['HorasTrabajadas'], 'sesion' => $maquina, 'RechazosTurno' => $row['Rechazos']]);
			$cont++;
		}
		if ($cont > 0)
			echo json_encode($array);
	...
	// Funcion anterior de turno anterior sin validacion de usuario
	function turnoanterior()
	{
		...
		$array = array();
		while ($row = sqlsrv_fetch_array($res)) {
			array_push($array, ['id' => $row['IdEncabezadoBItacora'], 'turno' => $row['Turno'], 'Maquina' => $row['NoMaquina']]);
		}
		echo json_encode($array);
	}
	...
	// 	$sql = "SELECT tEB.IdEncabezadoBItacora, tEB.Fecha, tEB.Turno, tEB.HorasTrabajadas, tSM.NoSupervisor, TE.ContrasenaOpcional 
	...
	// 	$array = array();
	// 	while ($row = sqlsrv_fetch_array($res)) {
	// 		array_push($array, ['id' => $row['IdEncabezadoBItacora'], 'turno' => $row['Turno'], 'horasTrabajadas' => $row['HorasTrabajadas']]);
	// 	}

	// 	// Si no se encontraron resultados, también puede considerarse un error
	// 	if (empty($array)) {
	...
	function tblctrltiempos()
	{
		...
		$query = "SELECT *,
					...
					LEFT JOIN tblProduccionSecciones on tblProduccionSecciones.idSeccion= tblBitCtrltiempos.seccion
					LEFT JOIN tblProduccionModulos ON tblProduccionModulos.idModulos=tblBitCtrltiempos.modulo WHERE folio=" . $folio;
		$array = array();
		$result = sqlsrv_query($conn, $query);
		while ($row = sqlsrv_fetch_array($result)) {
			array_push($array, [
	...
	function tblInfoParos()
	{
		$Conecta = new ClassConexion();
		$conn = $Conecta->conexion("TLX004MXDB");
		$folio = $_GET["folio"];
		$query = " SELECT *
					FROM vwBitacoraParosMaquinas vwBPM
					WHERE IdEncabezadoBItacora = $folio
					ORDER BY id DESC
					";
		$array = array();
		$result = sqlsrv_query($conn, $query);
		while ($row = sqlsrv_fetch_array($result)) {
			array_push($array, [
				...
				"Hora" => $row["HoraParo"]->format("H:i:s"),
				"Seccion" => $row["Seccion"],
				"Modulo" => $row["Modulos"],
				"Cortes" => $row["Cortes"],
	...
	function saveSanitizacion()
	{
		...
		$query = "INSERT INTO tblBitSanitizacionEnc(idparo,motivo,tiempo) VALUES (?, ?, ?)";
		$result = sqlsrv_query($conn, $query, array($folio, $motivo, $tiempo));
		$result === false ? http_response_code(500) : http_response_code(200);
	}
	...
	function ObtenerdatosPlanProduccion()
	{
		...
		$sql = "WITH UltimoSTD AS (
					SELECT 
						Clave,
						NoMaquina,
						NombreMaquina,
						SUM([std]) AS std_total_acumulado,
						COUNT(*) AS cantidad_registros
					FROM dbo.vw_UltimoSTDPorTurno
					WHERE NoMaquina = $idMaquina
					GROUP BY Clave, NoMaquina, NombreMaquina
				),
				ProduccionReciente AS (
					SELECT *,
						ROW_NUMBER() OVER (PARTITION BY clave ORDER BY 
							CASE WHEN configuracion IS NOT NULL THEN 0 ELSE 1 END,
							id DESC
						) AS rn
					FROM dbo.tblProduccionPlan
					WHERE maquina = $idMaquina
					AND YEAR(fechasave) = YEAR(GETDATE())
					AND MONTH(fechasave) = MONTH(GETDATE())
				)
				SELECT 
					COALESCE(pr.id, 0) AS id,
					COALESCE(pr.clave, std.Clave) AS clave,
					pr.fecha,
					COALESCE(pr.maquina, std.NoMaquina) AS maquina,
					pr.STD,
					pr.configuracion,
					tblcla.Descripcion_Articulo AS descripcion,
					tblcla.Producto, 
					tblppr.Producto AS nombreProducto, 
					tblcla.Etapa, 
					tblpet.Etapa AS nombreEtapa,
					COALESCE(tblm.NombreMaquina, std.NombreMaquina) AS NombreMaquina,
					std.std_total_acumulado AS STDAcumulado
				FROM ProduccionReciente pr
				FULL OUTER JOIN UltimoSTD std
					ON std.Clave = pr.clave 
					AND std.NoMaquina = pr.maquina
				LEFT JOIN TLX009MXDB.dbo.tblMaquinas tblm 
					ON tblm.NoMaquina = COALESCE(pr.maquina, std.NoMaquina)
				LEFT JOIN TLX002MXDB.dbo.tblValeEClaves tblcla 
					ON tblcla.NoClave = COALESCE(pr.clave, std.Clave)
				LEFT JOIN TLX002MXDB.dbo.tblProduccionProductos tblppr 
					ON tblppr.idProducto = tblcla.Producto
				LEFT JOIN TLX002MXDB.dbo.tblProduccionEtapas tblpet 
					ON tblpet.idEtapa = tblcla.Etapa
				WHERE pr.rn = 1 OR pr.rn IS NULL
				ORDER BY nombreProducto ASC, Etapa ASC";
		$array = array();
	...
	function ObtenerdatosPlanProduccionFecha()
	{
		$idMaquina = $_SESSION['idmaquina'];
		...
		$sql = "WITH UltimoSTD AS (
					...
					FROM dbo.vw_UltimoSTDPorTurno
					WHERE NoMaquina = $idMaquina
					AND Fecha <= '$fechaCorte'  -- Filtra hasta la fecha de corte
					GROUP BY Clave, NoMaquina, NombreMaquina
				),
				ProduccionReciente AS (
					SELECT *,
	...
	function actualizarHoras()
	{
		...
		$query = "UPDATE tblEncabezadoBitacora SET HorasTrabajadas=? WHERE IdEncabezadoBItacora=?";
		$result = sqlsrv_query($conn, $query, array($horas, $folio));
		if ($result === false) {
			http_response_code(500);
			die(print_r(sqlsrv_errors(), true));
		} else {
	...
	function guardarRechazos()
	{
		$Conecta = new ClassConexion();
		$conn = $Conecta->conexion("TLX004MXDB");
		$folio = $_POST["folio"];
		$rechazos = $_POST["rechazos"];
		$query = "UPDATE tblEncabezadoBitacora SET Rechazos=? WHERE IdEncabezadoBItacora=?";
		$result = sqlsrv_query($conn, $query, array($rechazos, $folio));
		if ($result === false) {
			http_response_code(500);
			die(print_r(sqlsrv_errors(), true));
		} else {
	...
	function guardarRechazoskgs()
	{
		$Conecta = new ClassConexion();
		$conn = $Conecta->conexion("TLX004MXDB");
		$folio = $_POST["folio"];
		$rechazos = $_POST["rechazos"];
		$query = "UPDATE tblEncabezadoBitacora SET KGSRechazados=? WHERE IdEncabezadoBItacora=?";
		$result = sqlsrv_query($conn, $query, array($rechazos, $folio));
		if ($result === false) {
			http_response_code(500);
			die(print_r(sqlsrv_errors(), true));
...
} else if (isset($_GET["actualizarHoras"])) {
	$BitacoraElectronica = new BitacoraElectronica();
	$BitacoraElectronica->actualizarHoras();
} else if (isset($_GET["tblInfoParos"])) {
	$BitacoraElectronica = new BitacoraElectronica();
	$BitacoraElectronica->tblInfoParos();
} else if (isset($_GET["guardarRechazos"])) {
	$BitacoraElectronica = new BitacoraElectronica();
	$BitacoraElectronica->guardarRechazos();
