...
class BitacoraElectronica
{
	...
	function tblInfoParos(){
		...
		$conn = $Conecta->conexion("TLX004MXDB");
		$folio = $_GET["folio"];
		$query = " SELECT IdSubEncabezadoBitacora, Hora, NoSeccion, tblPS.Seccion, NoModulo, tblPM.Modulos, IdFalla, Cortes, Rechazos,
					TArriba, TAbajo, TParo, NoTecnologia, VelProm, VelStp, EstadoParo, Motivo, Correccion
					FROM tblSubEncabezadoBItacora tblSEB
					LEFT JOIN TLX002MXDB.dbo.tblProduccionSecciones tblPS ON tblPS.idSeccion = tblSEB.NoSeccion
  					LEFT JOIN TLX002MXDB.dbo.tblProduccionModulos tblPM On tblPM.idModulos = tblSEB.NoModulo
					WHERE IdEncabezadoBItacora = $folio
					ORDER BY idSubEncabezadoBitacora DESC
					";
		$array = array();
		$result = sqlsrv_query($conn, $query);
		while ($row = sqlsrv_fetch_array($result)) {
