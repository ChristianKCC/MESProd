...
// pedir al supervisor elegir la hora de salida del día siguiente.
// ─────────────────────────────────────────────────────────────────────────────
...
async function procesarPorLapso(noemp, date, folioRegistro, turnoAsignado, tipoEmpleado, datosRegistro, mostrarError) {
    ...
    if (checador.regActual.length === 1) {
        ...
        // limpieza
        window._salidaElegidaLapso = null;
    ...
    // Ajuste de horas aun sin excedente
...
async function procesarTurnoNocturno(noemp, date, folioRegistro, turnoAsignado, tipoEmpleado, datosRegistro, mostrarError) {
    ...
    if (excedente < 55) {
        ...
        await Swal.fire({
            title: `${turnoAsignado} — No apto (excedente insuficiente)`,
            html: `Excedente <b>${minutosAHoraConSegundos(excedente)}</b> no alcanza los 55 min mínimos.<br><br>
                   Entrada (${diaEntrada}): <b>${resolucion.entrada.hora_limpia}</b><br>
                   Salida (${diaSalida}): <b>${resolucion.salida.hora_limpia}</b><br>
                   Total trabajado: <b>${minutosAHoraConSegundos(minutosTrabajados)}</b><br>
                   Hoy: <b>${regHoyStr}</b> | Ayer: <b>${regAyerStr}</b> | Mañana: <b>${regMañanaStr}</b><br>
...
// ─────────────────────────────────────────────────────────────────────────────
// procesarReingreso — razon = "Reingreso"
// ─────────────────────────────────────────────────────────────────────────────
