...
class Tiempoextra {
    ...
    inicio() {
        ...
        // this.Tools.llnarslc('CatalogoPersonal',"GetSlcDepsByNoemp&claveDepto=" + document.getElementById("clvdepartamento").innerText,"departamentoenc",0);
        this.tblenc();
    }
    ...
    aplicarRestriccionDepartamento() {
        ...
        for (let i = 0; i < selectDeptos.options.length; i++) {
            ...
            if (opt.value !== claveDepto) {
                ...
                opt.title = "No disponible para tu departamento";
            } else {
                opt.disabled = false;
                opt.style.color = "";
    ...
    async getinfohoraentradaysalida(noemp, date) {
        ...
        var fechaAnterior = new Date(fechaActual); fechaAnterior.setDate(fechaAnterior.getDate() - 1);
        var fechaSiguiente = new Date(fechaActual); fechaSiguiente.setDate(fechaSiguiente.getDate() + 1);

        function formatearFecha(fecha) {
            return fecha.getFullYear() + "-" + (fecha.getMonth() + 1).toString().padStart(2, '0') + "-" + fecha.getDate().toString().padStart(2, '0');
        }

        const [respuestaAnterior, respuestaActual, respuestaSiguiente] = await Promise.all([
            fetch("../Components/CatalogoSeguridad.php?datoshoraysalida&noemp=" + noemp + "&fechabien=" + formatearFecha(fechaAnterior)).then(r => r.json()),
            fetch("../Components/CatalogoSeguridad.php?datoshoraysalida&noemp=" + noemp + "&fechabien=" + date).then(r => r.json()),
            fetch("../Components/CatalogoSeguridad.php?datoshoraysalida&noemp=" + noemp + "&fechabien=" + formatearFecha(fechaSiguiente)).then(r => r.json())
        ]);

        function agruparPorHora(registros) {
            var horasVistas = {}, resultado = [];
            registros.forEach(function (registro) {
                if (registro.fecha_h) {
                    var h = registro.fecha_h.substring(0, 8);
                    if (!horasVistas[h]) { horasVistas[h] = true; resultado.push({ fecha_h: registro.fecha_h, hora_limpia: h }); }
                }
            });
            return resultado;
        }
        ...
        function horaAMinutos(hora) { var p = hora.split(":"); return parseInt(p[0]) * 60 + parseInt(p[1]); }
        function minutosAHora(m) { var h = Math.floor(m / 60), mn = m % 60; return h.toString().padStart(2,'0') + ":" + mn.toString().padStart(2,'0'); }

        var turnos = {
            turno1:  { entrada: { min: 5*60, max: 11*60 }, salida: { min: 13*60, max: 20*60 } },
            turno2:  { entrada: { min: 13*60, max: 17*60 }, salida: { min: 20*60, max: 24*60 } },
            turno3:  { entrada: { min: 20*60, max: 24*60 }, salida: { min: 5*60, max: 11*60 } },
            mixto1:  { entrada: { min: 7*60+20, max: 9*60 }, salida: { min: 15*60+30, max: 19*60 } },
            mixto2:  { entrada: { min: 7*60+30, max: 10*60 }, salida: { min: 17*60, max: 21*60 } },
            mixto3:  { entrada: { min: 7*60, max: 9*60 }, salida: { min: 16*60, max: 17*60 } },
            mixto4:  { entrada: { min: 6*60+45, max: 7*60+15 }, salida: { min: 16*60+30, max: 17*60+30 } }
        };

        function estaEnRango(m, r) { return m >= r.min && m <= r.max; }
        ...
        if (rAct.length >= 2) {
            ...
            for (var t in turnos) {
                if (estaEnRango(ultimaMin, turnos[t].entrada) && (t === "turno3" || t === "mixto3")) { esTurnoNocturno = true; break; }
            }
            if (esTurnoNocturno) { entrada = rAct[rAct.length - 1]; salida = rSig.length > 0 ? rSig[0] : null; }
            else { entrada = rAct[0]; salida = rAct[rAct.length - 1]; }
            ...
            if (estaEnRango(unicaMin, turnos.turno3.entrada) || estaEnRango(unicaMin, turnos.mixto3.entrada)) {
                entrada = rAct[0]; salida = rSig.length > 0 ? rSig[0] : null;
            } else if (estaEnRango(unicaMin, turnos.turno3.salida)) {
                salida = rAct[0]; entrada = rAnt.length > 0 ? rAnt[rAnt.length - 1] : null;
            } else { entrada = rAct[0]; salida = null; }
        }

        if (!entrada || !salida) return;

        var horaentrada = entrada.hora_limpia, horasalida = salida.hora_limpia;
        ...
        if (horaExtrass >= horaReglamen) horaExtra2 = minutosAHora(horaExtrass - horaReglamen);

        validarTiempoExtraLocal(horaExtra2, resultado.totalHoras, resultado.horasReglamentarias);
        if (document.getElementById("horaEX")) document.getElementById("horaEX").value = horaExtra2;
        if (document.getElementById("hrsReg")) document.getElementById("hrsReg").value = resultado.horasReglamentarias;
    }
    ...
    async abrirtiempoextra() {
        ...
        if (fechaenc === "" || departamentoenc === "") { Swal.fire('UPS!!!', 'No puede haber campos vacíos', 'info'); return false; }
        if (folio != "") { Swal.fire('UPS!!!', 'Estas editando un folio, selecciona empezar de nuevo para crear un nuevo folio', 'info'); return false; }
        ...
        try {
            ...
            const respuesta   = await respuetaraw.json();
            if (respuesta.error) { Swal.fire({ icon: 'error', title: 'No se pudo registrar el folio', text: respuesta.error, confirmButtonText: 'Entendido' }); return; }
        ...
        } catch (err) { Swal.fire('ERROR!', 'La respuesta no es JSON válido: ' + err, 'error'); }
    }
    ...
    async guardartiempoextra() {
        ...
        if (semanaRegistro < (semanaFolio - 1)) { Swal.fire('UPS!!!', 'La fecha seleccionada pertenece a una semana demasiado antigua respecto al folio.', 'warning'); return; }
        if (semanaRegistro > (semanaFolio + 1)) { Swal.fire('UPS!!!', 'La fecha seleccionada pertenece a una semana muy adelantada respecto al folio.', 'warning'); return; }
        ...
        // ── Modo Reingreso ────────────────────────────────────────────────────
        if (activaReingreso) {
            turnosel = "turno1"; turnoSeleccionado = "turno1";
        ...
        if (motivoNum === 8) {
            ...
            horaf = Math.floor(finComidaMin / 60).toString().padStart(2, "0") + ":" + (finComidaMin % 60).toString().padStart(2, "0") + ":00";
            turnosel = "turno1";
        } else if (motivoNum === 10 || motivoNum === 12) {
            if (!horai || !horaf || turnosel === "") { Swal.fire("Error", "Debes seleccionar el turno antes de guardar.", "warning"); return; }
        } else if (!activaAnticipo && !activaReingreso) {
            if (activaTurno3_12hrs && turnoSeleccionado === "turno3") turnosel = "turno3_12hrs";
            else if (activaTurno3_12hrs && turnoSeleccionado === "turno2") turnosel = "turno2_13hrs";
        }

        // ── Validaciones de campos vacíos ─────────────────────────────────────
        const motivosSinDuracion = [9, 8, 10, 12];
        const necesitaDuracion   = !motivosSinDuracion.includes(motivoNum) && !activaReingreso;

        if (folio === "") { Swal.fire('Error!', 'Necesitas crear un folio primero', 'warning'); return; }
        if (noemp === "" || fechainput === "" || maquina === "" || motivos === "" || razon === "" || nombre === "") { Swal.fire('Error', 'No puede haber campos vacíos', 'info'); return; }
        if (necesitaDuracion && !document.getElementById("duracionTE").value) { Swal.fire('Error', 'Debes ingresar la cantidad de horas de tiempo extra.', 'info'); return; }
        if (![9, 8].includes(motivoNum) && !activaReingreso && (!turnosel || turnosel === "")) { Swal.fire('Error', 'Debes seleccionar un turno.', 'info'); return; }
        ...
        if (IBM_ESPECIALES.includes(noempInt)) {
            ...
            if (![10, 12].includes(motivoNum)) {
                Swal.fire('Restricción', `El empleado <b>${noemp}</b> solo puede solicitar tiempos extra por <b>Descanso Trabajado</b> o <b>Día Festivo</b>.`, 'warning');
                return;
            }
        ...
        if (!activaAnticipo && !activaReingreso && necesitaDuracion) {
            ...
            const esSabado = [6, 0].includes(new Date(fechainput + "T00:00:00").getDay());
        ...
        if (respuesta.warning) {
            const result = await Swal.fire({
                title: 'Advertencia', text: respuesta.message, icon: 'warning',
                showCancelButton: true, confirmButtonText: 'Sí, continuar', cancelButtonText: 'No, cancelar'
            });
        ...
        if (respuesta === "Listo") { Swal.fire('Listo!!!', 'Registro guardado y enviado con éxito', 'success'); cleanData(); }
        else if (respuesta === "Existe") { Swal.fire('UPS!!!', 'Ya tienes un tiempo extra existente', 'error'); }
        else if (respuesta === "LimiteSemana") { Swal.fire('UPS!!!', 'Se alcanzó el límite de 60.5 horas en esta semana', 'warning'); }
        else { Swal.fire('ERROR!!!', 'Error al guardar en la base de datos', 'error'); }
    ...
    async cambioTempTurno(noemp, estadoTurno, nombre, depto, id) {
        abrirModal();
        document.getElementById("nombre_receptor").value    = nombre + " - " + noemp;
    ...
    async tblenc() {
        ...
        respuesta.forEach(elemento => {
            ...
            if (elemento.terminado == null) body += `<button class="btn btn-sm btn-warning" onclick="editEnc(${elemento.id})"><i class="fa-solid fa-pen-to-square"></i> Crear/eliminar registros</button>`;
            else body += `<button class="btn btn-sm btn-danger" onclick="pdfFin(${elemento.id})"><i class="fa-solid fa-file-pdf"></i> Descargar resultado PDF</button>`;
            body += `</td></tr>`;
        });
    ...
    enviar(id) {
        Swal.fire({ title: '¿Estás seguro?', text: "¿Deseas enviar este tiempo extra a autorización del supervisor?", icon: 'warning', showCancelButton: true, confirmButtonText: 'Si, seguro!', cancelButtonText: 'No, cancela!', reverseButtons: true })
        .then((result) => {
            if (result.isConfirmed) {
                (async () => {
                    const respuestaraw = await fetch("./php/index.php?enviarfol&id=" + id);
                    const respuesta    = await respuestaraw.json();
                    respuesta === false ? Swal.fire('ERROR!', 'Hay un error con la base de datos', 'error') : Swal.fire('Terminado!', 'La solicitud fue enviada al área correspondiente', 'success');
...
function minutosAHoraConSegundos(minutos) { return minutosAHora(minutos) + ":00"; }

function getWeekNumber(input) {
    let date = input instanceof Date ? new Date(input.getTime()) : (() => { const [y,m,d] = input.split('-').map(Number); return new Date(y, m-1, d); })();
...
function validarTiempoExtraLocal(horasExtra, totalHoras, horasReglamentarias) { /* referencial */ }
...
function calcularTurnoYHoras(horaEntrada, horaSalida, esSabado = false) {
    ...
    for (var key in turnos) {
        var t = turnos[key]; var mE = (key === "mixto4") ? 15 : margen;
        if (Math.abs(entradaMin - t.inicio) <= mE && Math.abs(salidaMin - t.fin) <= margen * 4) { turnoDetectado = key; break; }
    }
    if (!turnoDetectado) {
        var prioridad = ['mixto4','mixto1','mixto2','turno1','turno2','mixto3','turno3'];
        for (var i = 0; i < prioridad.length; i++) {
            var k = prioridad[i], tt = turnos[k], mE2 = (k === "mixto4") ? 15 : margen;
            if (Math.abs(entradaMin - tt.inicio) <= mE2) { turnoDetectado = k; break; }
        }
    }
    var td = turnoDetectado ? turnos[turnoDetectado] : null;
    var horaInicioTurno, horaFinTurno, nombreTurno, horasReglamentarias;
    if (td) {
        horaInicioTurno = minutosAHora(td.inicio); horaFinTurno = minutosAHora(td.fin);
        nombreTurno = td.nombre; horasReglamentarias = esSabado ? "05:00:00" : td.duracion;
        if (esSabado) nombreTurno = "MIXTO GUARDIA (Sáb/Dom)";
    } else { horaInicioTurno = horaEntrada; horaFinTurno = "00:00:00"; nombreTurno = "No hay"; horasReglamentarias = "00:00:00"; }
    var minutosTrabajados = salidaMin - entradaMin;
    var finTurnoMin = td ? td.fin : salidaMin;
    var horasExtrasMin = salidaMin - finTurnoMin;
    return { turno: nombreTurno, horaInicioTurno, horaFinTurno, horasExtras: horasExtrasMin > 0 ? minutosAHora(horasExtrasMin) : "00:00:00", totalHoras: minutosAHora(minutosTrabajados), horasReglamentarias, salidaMin, entradaMin, finTurnoMin };
}
    ...
    for (let i = 0; i < selectMotivos.options.length; i++) {
        ...
        if (opt.value === "9") {
            if (tipo === "empleado") {
                ...
                if (selectMotivos.value === "9") selectMotivos.value = "";
            } else { opt.disabled = false; opt.style.color = ""; opt.style.backgroundColor = ""; opt.title = ""; }
            break;
        }
    }
}
...
function resetearCheckboxes() {
    ...
    const horai = document.getElementById("horai"), horaf = document.getElementById("horaf");
    const turnoSel = document.getElementById("turnosel"), duracionTE = document.getElementById("duracionTE");
    const razon = document.getElementById("razon");
    horai.readOnly = true; horaf.readOnly = true;
    turnoSel.hidden = false; duracionTE.hidden = false;
    razon.readOnly = false; razon.value = "";
...
function recalcularHorasAnticipo() {
    ...
    if (!turnoSel || !durStr) { horai.value = ""; horaf.value = ""; return; }
    ...
    document.getElementById("horaFinalSinMargenHidden").value = minutosAHoraConSegundos(def.inicio);
    document.getElementById("horaFinalConMargenHidden").value = minutosAHoraConSegundos((def.inicio + 15) % (24 * 60));
    document.getElementById("horariosDeTurno").style.display = "block";
    document.getElementById("valorTurnoHora").textContent = minutosAHora(def.inicio) + " - " + minutosAHora(def.fin);
}
...
function validarCondicionesCheckbox12hrs() {
    ...
    if (!horasStr || !turno) { cont12.style.display = "none"; chk12.checked = false; return; }
    ...
    if ((turno === "turno3" || turno === "turno2") && minutos >= 210 && minutos <= 270) {
        ...
        cont12.style.display = "none"; chk12.checked = false;
    }
}
...
document.getElementById("abrir").addEventListener("click", function (event) {
    ...
    Tiempoextra.abrirtiempoextra().then(() => { Tiempoextra.tblsubenc(); Tiempoextra.tblenc(); });
});
...
document.getElementById("guardar").addEventListener("click", function (event) {
    ...
    Tiempoextra.guardartiempoextra()
        ...
        .finally(() => { btn.disabled = false; btn.innerHTML = '<i class="fa-solid fa-floppy-disk"></i> Guardar y enviar'; });
});
...
document.getElementById("motivos").addEventListener("change", function () {
    ...
    if (motivoNum === 8) { elHorai.hidden = true; elHoraf.hidden = true; elTurnosel.hidden = true; elDuracionTE.hidden = true; elCambiohrario.hidden = false; }
    else if (motivoNum === 9) { elHorai.hidden = true; elHoraf.hidden = true; elTurnosel.hidden = true; elDuracionTE.hidden = true; elHoracomida.hidden = false; }
    else if (motivoNum === 10 || motivoNum === 12) { elDuracionTE.hidden = true; elDuracionTE.value = ""; elTurnosel.disabled = false; }
});
...
document.getElementById("turnosel").addEventListener("change", function () {
    ...
    if (!turnosDef[turnoSeleccionado]) { document.getElementById("horariosDeTurno").style.display = "none"; return; }
    const datosTurno = turnosDef[turnoSeleccionado];
    document.getElementById("horariosDeTurno").style.display = "block";
    document.getElementById("valorTurnoHora").textContent = datosTurno.horaIdealEntrada + ' - ' + datosTurno.horaIdealSalida;
    const finTurnoMin = horaAMinutos(datosTurno.horaIdealSalida);

    if (chkAnticipo.checked) { recalcularHorasAnticipo(); return; }
    ...
    document.getElementById("horaFinalConMargenHidden").value      = minutosAHoraConSegundos(finTurnoMin + duracionTiempoE + 15);
...
document.addEventListener("DOMContentLoaded", function () {
    ...
    const horai = document.getElementById("horai"), horaf = document.getElementById("horaf");
    const motivo = document.getElementById("motivos"), razon = document.getElementById("razon");
    ...
    duracionTE.addEventListener("input", () => {
        ...
        if (duracionTE.value.trim() === "") {
            turnoSel.disabled = true; turnoSel.value = ""; horai.value = ""; horaf.value = "";
        } else { turnoSel.disabled = false; }

        if (checkboxAnticipo.checked && turnoSel.value) { recalcularHorasAnticipo(); }
            ...
            if (parseInt(motivo.value) !== 10 && parseInt(motivo.value) !== 12) {
                const turnosDef2 = { turno1: "15:00:00", turno2: "22:30:00", turno3: "07:00:00", turno3_12hrs: "22:30:00", turno2_13hrs: "15:00:00", mixto1: "17:00:00", mixto2: "18:30:00", mixto3: "16:30:00", mixto4: "17:00:00" };
                if (turnosDef2[turnoSel.value] && duracionTE.value) {
                    ...
                    horaFinalConMargenHidden.value = minutosAHoraConSegundos(finTurnoMin + dur + 15);
                }
            }
        }
        validarCondicionesCheckbox12hrs();
    ...
    duracionTE.addEventListener("blur", function () {
        ...
        if (this.value && !regex.test(this.value)) {
            ...
            this.value = ""; turnoSel.value = ""; horai.value = ""; horaf.value = "";
        }
    });
    ...
    checkboxTurno3.addEventListener("change", function () {
        if (this.checked) {
            checkboxAnticipo.checked = false; checkboxApoyo.checked = false;
            estadoOriginal = { horai: horai.value, horaf: horaf.value, horaFinalSinMargen: horaFinalSinMargenHidden.value, horaFinalConMargen: horaFinalConMargenHidden.value };
            if (turnoSel.value === "turno3") {
                horai.value = "10:30:00"; horaf.value = "22:30:00"; turnoHidden.value = "turno3";
                horaFinalSinMargenHidden.value = "22:30:00"; horaFinalConMargenHidden.value = "22:45:00";
            } else if (turnoSel.value === "turno2") {
                horai.value = "10:30:00"; horaf.value = "15:00:00"; turnoHidden.value = "turno2";
                horaFinalSinMargenHidden.value = "15:00:00"; horaFinalConMargenHidden.value = "15:15:00";
            }
        } else {
    ...
    checkboxAnticipo.addEventListener("change", function () {
        if (this.checked) {
            checkboxTurno3.checked = false; checkboxApoyo.checked = false;
            turnoSel.disabled = false; duracionTE.hidden = false; turnoSel.hidden = false;
            horai.readOnly = true; horaf.readOnly = true;
            razon.readOnly = true; razon.value = "Anticipo";
            document.getElementById("contenedorCheckbox12hrs").style.display = "none";
            if (turnoSel.value && duracionTE.value) recalcularHorasAnticipo();
            else { horai.value = ""; horaf.value = ""; }
        } else {
            horai.readOnly = true; horaf.readOnly = true; razon.readOnly = false; razon.value = "";
            horai.value = ""; horaf.value = ""; turnoSel.value = ""; duracionTE.value = "";
            turnoSel.disabled = true; document.getElementById("horariosDeTurno").style.display = "none";
        }
    });
    ...
    checkboxApoyo.addEventListener("change", function () {
        if (this.checked) {
            checkboxTurno3.checked = false; checkboxAnticipo.checked = false;
            duracionTE.hidden = true; turnoSel.hidden = true;
            horai.hidden = false; horaf.hidden = true;
            horai.readOnly = false; razon.readOnly = true; razon.value = "Reingreso";
            duracionTE.value = "01:00"; turnoSel.value = "turno1";
            document.getElementById("turnoSeleccionadoHidden").value = "turno1";
            horai.value = ""; horaf.value = "00:00:00";
            document.getElementById("contenedorCheckbox12hrs").style.display = "none";
            document.getElementById("horariosDeTurno").style.display = "none";
        } else {
            duracionTE.hidden = false; turnoSel.hidden = false;
            horai.hidden = false; horaf.hidden = false;
            horai.readOnly = true; horaf.readOnly = true; razon.readOnly = false; razon.value = "";
            horai.value = ""; horaf.value = ""; duracionTE.value = ""; turnoSel.value = ""; turnoSel.disabled = true;
        }
    });
    ...
    const steps = [
        ...
        },
        {
            ...
            popover: {
                ...
                description: "Debes de seleccionar el caso que te corresponda segun tus necesidades, se contemplan 12 horas, Anticipos y Reingresos en turno seguido",
                side: "top"
            }
        },
        {
    ...
    function launchDriver(pasos) {
        ...
        driverObj = driver({ showProgress: true, allowClose: false, disableInteraction: true, progressText: "Paso {{current}} de {{total}}", doneBtnText: "Finalizar", nextBtnText: "Siguiente", prevBtnText: "Atrás", steps: pasos });
        driverObj.drive();
    }

    const tutorialKey = "tutorial_tiempoextra";
    if (!localStorage.getItem(tutorialKey)) { launchDriver(steps); localStorage.setItem(tutorialKey, "true"); }
    document.getElementById("btnAyuda")?.addEventListener("click", () => launchDriver(steps));

    const tutorialModalKey = "tutorial_modalCambioTurno";
    const modalOverlay = document.getElementById("modalOverlay");
    if (modalOverlay) {
        modalOverlay.addEventListener("shown.bs.modal", () => {
            if (!localStorage.getItem(tutorialModalKey)) { launchDriver(stepsModal); localStorage.setItem(tutorialModalKey, "true"); }
        });
    }
...
document.getElementById("btnGuardar").addEventListener("click", async function () {
    ...
    if (folio === "" || hora_presentacion === "" || horario_desde === "" || horario_hasta === "" || hasta_tripulacion === "") { Swal.fire('Error', 'Debes de ingresar todos los campos', 'error'); return; }
...
function abrirModal() {
    const hoy = new Date();
    document.getElementById('fecha_emision').value = hoy.getFullYear() + "-" + String(hoy.getMonth()+1).padStart(2,'0') + "-" + String(hoy.getDate()).padStart(2,'0');
