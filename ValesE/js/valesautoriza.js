...
modalenv.addEventListener("show.bs.modal", function (event) {
  ...
  Valesm.ValesConstxid(recipient).then((element) => {
    ...
    document.getElementById("folioencvista").innerHTML =
      "Folio: " + element[0].maquina + " - " + element[0].foliocons;
    element[0].estado != "Enviado"
      ? (document.getElementById("validaenvio").hidden = true)
      : (document.getElementById("validaenvio").hidden = false);
    element[0].estado == "Enviado" ? (btncancelar = 1) : (btncancelar = 0);
    element[0].estado == "Enviado"
      ? (document.getElementById("formaddmat").style.display = "block")
      : (document.getElementById("formaddmat").style.display = "none");
    llenarclase();
    Valesm.tblMaterialesAgregados(recipient, element[0].maquinaid).then((infotbl) => {
      let body = "";
...
document.getElementById("clase").addEventListener("change", function () {
  ...
  const clave4 = document.getElementById("clave4").value;
  const maquinaid = document.getElementById("maquinaid").value;
  Valesm.tblMateriales(clase, clave1, clave2, clave3, clave4, maquinaid).then(
    (element) => {
      let body = "";
...
window.addMaterial = function (param) {
  ...
  Valesm.ValidaMatRemplazados(folio).then((respuesta) => {
    if (respuesta[0].cont2 == respuesta[0].cont3) {
      swal.fire("Ups", "Ya no puedes cambiar mas materiales", "warning");
      return false;
    }
    Valesm.addMaterialadmin(param).then(() => {
      ...
      Valesm.tblMaterialesAgregados(folio, element[0].maquinaid).then((infotbl) => {
        let body = "";
        infotbl.forEach((listado) => {
          body += `
                    <tr ${listado.estadomat == 2 && 'class="text-danger"'}>
                        <td>${listado.folio}</td>
                        <td>${listado.NoMaterial}</td>
                        <td>${listado.NombreMaterial}</td>
                        <td>${listado.CentroCosto}</td>
                        <td>${listado.TiempoMaterial}</td>
                        <td>${listado.TipoMontacargas}</td>
                        <td>${listado.Cantidad}  ${listado.UM} </td>
                        <td>${btncancelar == 1 ? '<button class="btn btn-sm btn-danger" onclick="cancelaMaterial(this,' + listado.folio + ')"><i class="fas fa-random"></i></button>' : ""}</td></tr>`;
        });
        document.getElementById("tblmaterialesagregados").innerHTML = body;
      });
