...
document.getElementById("noemp").addEventListener("keyup", function () {
  ...
  noemp != "" &&
    Vales.datosemp(noemp).then((element) => {
      if (element != false) {
...
function llenarclase(c1 = "", c2 = "", c3 = "", c4 = "") {
  const clave1 = c1 || document.getElementById("clave1").value;
  const clave2 = c2 || document.getElementById("clave2").value;
  const clave3 = c3 || document.getElementById("clave3").value;
  const clave4 = c4 || document.getElementById("clave4").value;
  return Vales.SelectClasesM(clave1, clave2, clave3, clave4);
}
document.getElementById("clave1").addEventListener("change", function () {
  llenarclase(this.value);
});
...
//         ? swal.fire(
//             "Ups!",
//             "Los campos Noemp, turno y Clave 1 son obligatorios",
//             "warning",
//           )
//         : Vales.creaVale(noemp, turno, clave1, clave2, clave3, clave4).then(
//             (element) => {
...
//       swal.fire(
//         "Vale incompleto",
//         "Hay un vale existente, debes terminarlo o cancelarlo.",
//         "warning",
//       );
...
//         element[0].clave1 === 0 ? "" : element[0].clave1;
//       document.getElementById("clave2").value =
//         element[0].clave2 === 0 ? "" : element[0].clave2;
//       document.getElementById("clave3").value =
//         element[0].clave3 === 0 ? "" : element[0].clave3;
//       document.getElementById("clave4").value =
//         element[0].clave4 === 0 ? "" : element[0].clave4;
//       document.getElementById("foliovale").value = element[0].id;
...
// Función reutilizable para cargar un vale existente
async function cargarValeExistente(element) {
  document.getElementById("noemp").value = element[0].noemp;
  document.getElementById("turnoen").value = element[0].turno;
  document.getElementById("foliovale").value = element[0].id;
  document.getElementById("foliocons").value = element[0].foliocons;

  Vales.datosemp(element[0].noemp).then((emp) => {
    if (emp != false) {
      document.getElementById("nombre").value = emp[0].nombre;
      document.getElementById("puesto").value = emp[0].puesto;
    }
  });

  const c1 = element[0].clave1 === 0 ? "" : element[0].clave1;
  const c2 = element[0].clave2 === 0 ? "" : element[0].clave2;
  const c3 = element[0].clave3 === 0 ? "" : element[0].clave3;
  const c4 = element[0].clave4 === 0 ? "" : element[0].clave4;

  await llenarclase(c1, c2, c3, c4);

  // Pequeña pausa para asegurar que el DOM renderizó las opciones
  await new Promise(resolve => setTimeout(resolve, 100));

  document.getElementById("clave1").value = c1;
  document.getElementById("clave2").value = c2;
  document.getElementById("clave3").value = c3;
  document.getElementById("clave4").value = c4;

  // Bloquear campos para que no se puedan modificar
  ...
  document.getElementById("clave4").disabled = true;
  document.getElementById("noemp").disabled = true;
  document.getElementById("turnoen").disabled = true;
...
document.addEventListener("DOMContentLoaded", () => {
  Vales.validaUltimoVale().then((element) => {
    if (element.length > 0) {
      cargarValeExistente(element);
    }
  });
});

// Listener del botón ya simplificado
document.getElementById("crearVale").addEventListener("click", function (e) {
  e.preventDefault();
  Vales.validaUltimoVale().then((element) => {
    if (element.length === 0) {
      ...
      Swal.fire({
        ...
        if (result.isConfirmed) {
          Vales.creaVale(noemp, turno, clave1, clave2, clave3, clave4).then((element) => {
            ...
            document.getElementById("clave3").disabled = true;
            document.getElementById("clave4").disabled = true;
            Swal.fire("¡Listo!", "Vale creado exitosamente", "success");
          });
        }
      });
...
document.getElementById("cancelar").addEventListener("click", function (e) {
  ...
  if (folio === "") {
    swal.fire("Hay un problema", "No hay un folio seleccionado", "warning");
    return;
  }

  Swal.fire({
    title: "¿Está seguro?",
    text: "¿Desea cancelar el vale? Esta acción no se puede deshacer.",
    icon: "warning",
    showCancelButton: true,
    confirmButtonText: "Sí, cancelar vale",
    cancelButtonText: "No, regresar",
    confirmButtonColor: "#d33",
  }).then((result) => {
    if (result.isConfirmed) {
      Vales.cancelarVale(folio).then(() => Vales.limpiar());
    }
  });
});
document.getElementById("enviar").addEventListener("click", function (e) {
  ...
  const folio = document.getElementById("foliovale").value;
  folio != ""
    ? Vales.enviarVale(folio).then(() => Vales.limpiar())
    : swal.fire("Hay un problema", "No hay un folio seleccionado", "warning");
});
document
  .getElementById("limpiarinicio")
...
window.addmm2material = function (param) {
  Vales.tblMaterialesAgregados(param).then((infotbl) => {
    ...
    infotbl.forEach((listado) => {
      body += `<tr>
                        <td>${listado.folio}</td>
                        <td>${listado.NoMaterial}</td>
                        <td>${listado.NombreMaterial}</td>
                        <td>${listado.CentroCosto}</td>
                        <td>${listado.TiempoMaterial}</td><td>${listado.TipoMontacargas}</td><td>${listado.Cantidad}  ${listado.UM} </td>
...
document
  .getElementById("tblvalescambios")
  .addEventListener("input", function (event) {
    ...
    if (target.tagName === "INPUT" && target.name !== "") {
      ...
      var mm2 = event.target
        .closest("tr")
        .querySelector("input[name=mm2kg]").value;
      var envasesrec = event.target
        .closest("tr")
        .querySelector("input[name=envasesrec]").value;
