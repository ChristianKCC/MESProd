export class Maquinas {
  ...
  async consultarxid(clave, ruta) {
    const dataPromise = await fetch(
      "../Produccion/php/Maquinas.php?" + ruta + "&id=" + clave
    );
  ...
  async saveSeccion(idSeccion, nombreSeccion) {
    ...
    dataResp.status == 200 &&
      swal.fire("Listo", "Se guardo la sección correctamente", "success");
    dataResp.status == 500 &&
  ...
  async saveModulo(idModulo, nombreModulo) {
    ...
    dataResp.status == 200 &&
      swal.fire("Listo", "Se guardo el modulo correctamente", "success");
    dataResp.status == 500 &&
      swal.fire("Ups", "Hay un problema al guardar la clase", "error");
    dataResp.status == 201 &&
      swal.fire("Ups", "Todos los campos son obligatorios", "warning");
  ...
  async saveFalla(idFalla, nombreFalla) {
    ...
    dataResp.status == 200 &&
      swal.fire("Listo", "Se guardo la falla correctamente", "success");
    dataResp.status == 500 &&
      swal.fire("Ups", "Hay un problema al guardar la clase", "error");
    dataResp.status == 201 &&
      swal.fire("Ups", "Todos los campos son obligatorios", "warning");
