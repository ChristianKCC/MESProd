...
class Tiempoextra
{
    ...
    function abrirtiempoextra()
    {
        ...
        // Tiempos extra  →  antes de $query = "INSERT INTO TiempoextraEnc..."
    ...
    function buscarJefeInmediato(string $ibmSupervisor, string $nombreDepto = ''): array
    {
        ...
        while (($line = fgetcsv($handle, 0, CSV_SEPARATOR)) !== false) {
            ...
            if ($num === '' || $num !== trim($ibmSupervisor))
                continue; // no es este supervisor
    ...
    function guardartiempoextra()
    {
        ...
        if (!$esExcedente) {
            ...
            elseif ($turnosel === "turno1")
                $turnoNuevoFor = '1er Turno';
            elseif ($turnosel === "turno2")
                $turnoNuevoFor = '2do Turno';
            elseif ($turnosel === "turno3")
                $turnoNuevoFor = '3er Turno';
            elseif ($turnosel === "mixto1")
                $turnoNuevoFor = '1er Mixto';
            elseif ($turnosel === "mixto2")
                $turnoNuevoFor = '2do Mixto';
            elseif ($turnosel === "mixto3")
                $turnoNuevoFor = '3er Mixto';
            elseif ($turnosel === "mixto4")
    ...
    */
    ...
    function getSemana()
    {
        ...
        if (!$resCal)
            error_log("Error SQL calendario: " . print_r(sqlsrv_errors(), true));
        ...
        if (!$resExtras)
            error_log("Error SQL extras: " . print_r(sqlsrv_errors(), true));
    ...
    function getExtraAcumuladas()
    {
        ...
        if (!$res)
            error_log("[getExtraAcumuladas] Error SQL: " . print_r(sqlsrv_errors(), true));
