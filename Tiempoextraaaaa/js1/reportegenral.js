...
class Reportes {
    ...
    const dif = parseFloat(element.dif); // ej: 0.5, 2.6, 2.4
    let horasInt = Math.floor(dif);      
    let decimal  = parseFloat((dif - horasInt).toFixed(1)); // normalizar decimal
    let minsInt  = 0;

    // reglas de redondeo específicas
    if (decimal === 0.5 || decimal === 0.7) {
        minsInt = 30; // .5 o .6 → media hora
        // si no hay horas (ej. dif = 0.5), no sumes horas, deja 00:30
        if (horasInt === 0) {
            horasInt = 0;
        }
    } else if (decimal > 0.7) {
        horasInt += 1; // mayor a .6 → sube al entero
        minsInt = 0;
    } else {
        minsInt = 0;   // menor a .5 → baja al entero
    }

    const totalGeneralFormateado =
        String(horasInt).padStart(2, '0') + ':' +
        String(minsInt).padStart(2, '0');

    // 👀 Debug en consola
    console.log(`dif bruto: ${element.dif}, decimal ajustado: ${decimal}, convertido: ${totalGeneralFormateado}`);
