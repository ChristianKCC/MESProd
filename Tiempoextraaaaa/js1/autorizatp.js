...
function seleccionarEntradaDiurna(def, regActual, margenMin = 90) {
    ...
    const dist  = min => Math.min(
        Math.abs(min - ideal),
        Math.abs(min - ideal + 1440),
        Math.abs(min - ideal - 1440)
    );
    const enVentana = regActual.filter(r => dist(horaAMinutos(r.hora_limpia)) <= margenMin);
    if (enVentana.length) return enVentana[0]; // primero dentro de la ventana
    return regActual.reduce((mejor, r) =>
        dist(horaAMinutos(r.hora_limpia)) < dist(horaAMinutos(mejor.hora_limpia)) ? r : mejor);
}
...
function seleccionarSalidaDespuesDe(entradaMin, regActual) {
    const posteriores = regActual.filter(r => horaAMinutos(r.hora_limpia) > entradaMin);
    return posteriores.length ? posteriores[posteriores.length - 1]
...
async function procesarAnticipo(noemp, date, folioRegistro, turnoAsignado, tipoEmpleado, datosRegistro, mostrarError) {
    ...
    const candidatosRango = checador.regActual.filter(r => {
        ...
        return min >= (horaiSolicitadaMin - MARGEN_ANTICIPO) &&
    ...
    const regDentroRango = candidatosRango.length
        ? candidatosRango.reduce((mejor, r) =>
            ...
            Math.abs(horaAMinutos(mejor.hora_limpia) - horaiSolicitadaMin) ? r : mejor)
        : null;
