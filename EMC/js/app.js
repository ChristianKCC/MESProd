...
//   /**
//    * Crea o reposiciona la capa tapadora sobre un iframe
//    * @param {HTMLElement} wrapper - contenedor del iframe (.xl-iframe-wrap o .pptx-stage)
//    * @param {number} barHeight - altura en px de la barra a tapar
//    * @param {string} bgColor - color de fondo igual al del contenedor
//    */
//   function applyBarCover(wrapper, barHeight, bgColor) {
//     if (!wrapper) return;

//     /* Buscar si ya existe la capa, si no crearla */
...
//   /**
//    * Aplica la capa y la reaplica cuando el iframe termina de cargar
//    * (SharePoint a veces re-renderiza la barra después del load)
//    */
//   function watchIframe(iframe, wrapper, barHeight, bgColor) {
//     /* Aplicar inmediatamente */
//     applyBarCover(wrapper, barHeight, bgColor);

//     /* Reaplicar cuando el iframe cargue */
//     iframe.addEventListener('load', function () {
//       applyBarCover(wrapper, barHeight, bgColor);

//       /* SharePoint suele hacer renders adicionales ~1s después */
//       setTimeout(function () { applyBarCover(wrapper, barHeight, bgColor); }, 800);
//       setTimeout(function () { applyBarCover(wrapper, barHeight, bgColor); }, 2000);
//       setTimeout(function () { applyBarCover(wrapper, barHeight, bgColor); }, 4000);
//     });
//   }
...
//     /* Asegurarse que el wrapper tenga position:relative */
...
(function () {
  ...
  if (uploadForm) {
    ...
    uploadForm.addEventListener('submit', function (e) {
      ...
      if (!cut) {
        showAlert('El enlace no es válido. Revisa que contenga "sourcedoc=".', 'warning');
        if (linkError) linkError.style.display = 'flex';
        return;
      }
      ...
      xhr.addEventListener('load', function () {
        ...
        try { res = JSON.parse(xhr.responseText); }
          ...
          showAlert('Error inesperado del servidor al subir el archivo.', 'danger');
          resetBoton();
          return;
        }
      ...
      function guardarEnlace(nombre, enlace, resArchivo) {
        ...
        .then(function (data) {
          if (data.success) {
            ...
            showAlert(data.message || 'El archivo se subió pero el enlace no se pudo guardar.', 'danger');
            resetBoton();
          }
        })
        .catch(function () {
          showAlert('El archivo se subió pero hubo un error de red al guardar el enlace.', 'danger');
          resetBoton();
        });
      }
    });
  ...
  function refreshStatusPanel(data) {
    ...
    if (metaEl && data && data.updated)  metaEl.textContent = 'Actualizado: ' + data.updated;
    if (badgeEl) {
      badgeEl.textContent = 'Disponible';
      badgeEl.className = 'status-badge badge-ok';
    }
  ...
  function applyBarCover(wrapper, barHeight, bgColor) {
    ...
    if (!cover) {
      ...
      cover.style.cssText = [
        ...
        'z-index: 99','pointer-events: all','display: block'
      ].join(';');
      wrapper.appendChild(cover);
    }
    cover.style.height     = barHeight + 'px';
