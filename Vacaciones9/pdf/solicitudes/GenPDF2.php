...
// 1. Encabezado
$queryEnc = "SELECT Vc_id, Vc_ibm, Vc_autorizado, Vc_autoriza, Vc_terminado, Vc_tipo, Vc_revisado, Vc_firmaRI
             FROM tblMXPRVacacionesEnc
             WHERE Vc_id = ?";
...
if ($firmaPath) {
    $pdf->Image($firmaPath, $x+20, $y-12, 13);
}
...
if ($autorizo == 1 && !empty($autorizoemp) && file_exists($firmaPath)) {
    $pdf->Image($firmaPath, $fx2 + 10, $fy - 12, $size); 
}
...
if ($verifico == 1 && $autorizo== 1  && $firmoRI==1 && file_exists($firmaRIPath)) {
    $pdf->Image($firmaRIPath, $fx2 + 84, $fy - 17, 15); 
}
