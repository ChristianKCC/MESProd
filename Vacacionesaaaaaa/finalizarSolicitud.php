...
<div class="container p-4">
    ...
    <div style="float:left" class="row">
        <div class="col-20">
            <small class="alert alert-info">
                ...
                Verifica tu información, una vez los datos esten correctos da click en "GUARDAR Y GENERAR PDF" para su
                posterior envio y autorización.
            </small>
        </div>
    </div>
    <br />
    ...
    <form id="formVac" action="./pdf/generar_pdf.php" method="POST" target='_blank'>
        <div class="page">
            ...
            <input hidden type="date" name="fecha_solicitud" value="<?php echo date('Y-m-d'); ?>">
            ...
            <div class="campos">
                ...
                <div>
                    ...
                    <div class="campo-grupo">
                        ...
                        <input type="date" id="fecha_ingreso" name="fecha_ingreso" value="<?php
                    ...
                    <div class="campo-grupo">
                        ...
                        <input type="text" id="solicitud_por" name="solicitud_por" placeholder="MOTIVO / PERIODO"
                            value="<?= htmlspecialchars($dias) ?>" required>
                    </div>

                    <div class="campo-grupo fila-inline">
                        <!-- <label>DE: </label> -->
                        <input hidden type="date" id="vacaciones_de" name="vacaciones_de"
                            value="<?= htmlspecialchars($fechaDe) ?>" style="max-width:80px">
                        <!-- <label>DEL 20: </label> -->
                        <input hidden type="text" id="vacaciones_anio_de" name="vacaciones_anio_de" value="26"
                            style="max-width:50px" placeholder="AA">
                        <label>HASTA EL DIA: </label>
                        <input type="date" id="vacaciones_hasta" name="vacaciones_hasta"
                            value="<?= htmlspecialchars($fecha_a) ?>">
                    </div>

                    <div class="campo-grupo">
                        <label>DÍAS CORRESPONDIENTES POR ANTIGUEDAD:</label>
                        <input type="number" id="dias_antiguedad" name="dias_antiguedad"
                            value="<?= htmlspecialchars($limite_dias) ?>" min="0" required readonly>
                    </div>

                    <div class="campo-grupo">
                        <label>DÍAS DE VACACIONES SOLICITADOS:</label>
                        <input type="number" id="dias_solicitados" name="dias_solicitados" min="0"
                            value="<?= htmlspecialchars($dias) ?>" required>
                    </div>
                    ...
                    <div class="campo-grupo">
                        ...
                        <input type="number" id="dias_reposicion" name="dias_reposicion" min="0"
                            value="<?= $diasReposicionFestivo ?>" required>
                    </div>
                    ...
                    <div class="campo-grupo">
                        ...
                        <input type="number" id="total_dias" name="total_dias" min="0"
                            value="<?= htmlspecialchars($ResTot) ?>" required>
                    </div>
                </div>
                ...
                <div>
                    <div class="campo-grupo">
                        ...
                        <input type="text" id="tarjeta" name="tarjeta" placeholder="NO. DE TARJETA"
                            value="<?= htmlspecialchars($ibm) ?>" required>
                    </div>
                    ...
                    <div class="campo-grupo">
                        <label>ANTIGUEDAD DE:</label>
                        <input type="text" id="antiguedad_de" name="antiguedad_de" placeholder="ANTIGUEDAD DE:" value="<?php
                        $fechaNormalizada = normalizarFechaISO(col($empleado, COL_FINGRESO));
                    ...
                    <div class="campo-grupo">
                        ...
                        <input type="date" id="periodo_de" name="periodo_de" value="<?= htmlspecialchars($fechaDe) ?>">
                    </div>

                    <div class="campo-grupo fila-inline">
                        <!-- <label>DE: </label> -->
                        <input type="hidden" id="dias_habiles_partir" name="dias_habiles_partir"
                            placeholder="DÍAS HABILES" value="<?= htmlspecialchars($fechaDe) ?>" required>

                        <!-- <label>DEL 20: </label> -->
                        <input hidden type="text" id="periodo_anio_de" name="periodo_anio_de" style="max-width:35px"
                            value="26" placeholder="AA">
                        <label>PERIODO SOLICITADO:</label>
                        <input type="text" id="periodo_solicitado" name="periodo_solicitado"
                            placeholder="PERIODO SOLICITADO" required>
                    </div>

                    <div class="campo-grupo">
                        <label>TIPO DE SOLICITUD:</label>
                        <input type="text" id="tipo_solicitud" name="tipo_solicitud" placeholder="Normal / Adelanto"
                            value="<?= htmlspecialchars($tipo_Solicitud) ?>" required>
                    </div>
            ...
            <div class="header glosarioColores">
                <div class="titulo" style="padding:4px 8px; border:1px solid #000;"> SELECCIONA EN LOS SIGUIENTES
                    RECUADROS LO QUE CORRESPONDA SEGUN TU CASO: </div>
                <div class="planta">
                    <div class="d-flex flex-wrap justify-content-center gap-3 mt-3">
                        <span style="background-color: rgb(198,224,180); padding:4px 8px; border:1px solid #000;">V =
                            VACACIONES</span>
                        <span style="background-color: rgb(255,255,153); padding:4px 8px; border:1px solid #000;">D =
                            DESCANSO</span>
                        <span style="background-color: rgb(255,153,153); padding:4px 8px; border:1px solid #000;">F =
                            FESTIVO</span>
                        <span style="background-color: rgb(180,198,231); padding:4px 8px; border:1px solid #000;">R =
                            REPOSICION</span>
                    </div>
                </div>
            </div>
            ...
            function pintarFila($inicio, $fin, $anio, $mes, $diasSemana, $diasSeleccionados, $diasFestivos, $colores)
            {
                ...
                for ($i = $inicio; $i <= $fin; $i++) {
                    ...
                    echo "<td style='$style'>";
                    echo "<select name='dia_$fechaStr' data-fecha='$fechaStr'>";
            ...
            // }
            echo "<div id='contenedorCalendarios'>";
            foreach ($mesesInvolucrados as $m) {
                $anio = $m['anio'];
                $mes = $m['mes'];

                echo "<div class='mesCalendario' data-mes='$anio-$mes'>";
                echo "<h5>Calendario de {$nombreMeses[$mes]} $anio</h5>";
                pintarFila(1, 16, $anio, $mes, $diasSemana, $diasSeleccionados, $diasFestivos, $colores);
                pintarFila(17, 31, $anio, $mes, $diasSemana, $diasSeleccionados, $diasFestivos, $colores);
                echo "</div>";
            ...
            <!-- Meses agregados manualmente se inyectan aquí -->
            <div id="contenedorCalendariosExtra"></div>

            <div class="d-flex align-items-center flex-wrap gap-3 my-3" id="zonaAgregarMes">
                <button type="button" id="btnAgregarMes" class="btn btn-success">
                    <i class="fa-solid fa-calendar-plus"></i> Agregar mes siguiente
                </button>
                <span id="badgeDiasV" class="badge bg-secondary" style="font-size:0.9rem;">
                    Días V: 0 / 0
                </span>
            </div>
            ...
            <div class="obs-section observacionesSeccion">
                ...
                <input type="text" id="observaciones" maxlength="150" name="observaciones"
            ...
            <div class="obs-section fechasReposicionfestivo">
                ...
                <input type="text" id="fechas_reposicion" maxlength="150" name="fechas_reposicion"
            ...
            <div class="botones">
                <button type="button" class="btn btn-primary botonRegresar"
                    onclick="location.href='<?= htmlspecialchars($origen) ?>'">
