class Vacaciones {
  async consulta() {
    ...
    const respuestaRaw = await fetch(
      `php/index.php?tblVacacionesRIEnc&fecha=${fechaFiltro}&ibm=${ibmFiltro}`,
    );

    // const respuestaRaw = await fetch(
    //   `php/index.php?tblVacacionesRIEnc&fecha=${fechaFiltro}&estatus=${estatusFiltro}`,
    // );
    ...
    respuesta.forEach((folio) => {
      ...
      if (
        folio.autorizado == 1 &&
        folio.revisado == 1 &&
        (folio.firmaRI === null || folio.firmaRI == 0)
      ) {
        accionesHtml = `                    
...
window.firmarInfo = async function (id) {
  ...
  try {
    ...
    if (respuesta.success) {
      Swal.fire(
        "Éxito",
        `Solicitud firmada y aceptada correctamente.`,
        "success",
      ).then(() => window.Vacaciones.consulta());
    } else {
