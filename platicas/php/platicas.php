...
class PlaticasMaquina
{
	function tblPlaticas5min()
	{
		...
		$query = "SELECT TOP 200 
					...
					p.minutos,
					p.archivo
				FROM tblPlaticas5min AS p
				INNER JOIN TLX032MXDB.dbo.tblEmpleados AS e
					ON e.NoEmp = p.noemp
				INNER JOIN TLX009MXDB.dbo.tblDepartamentos AS d
					ON d.NoDepto = e.NombreDepartamento
				WHERE p.fecha >= DATEFROMPARTS(YEAR(GETDATE()), 1, 1)
				AND p.fecha <  DATEFROMPARTS(YEAR(GETDATE()) + 1, 1, 1)
				ORDER BY p.id DESC;
				";
	...
	// Funcion para actualizacion de archivo
	function updatePlaticaArchivo()
	{
		...
		$sql = "UPDATE tblPlaticas5min SET archivo = ? WHERE id = ?";
		$params = array($ruta, $folio);
...
} else if (isset($_GET["updatePlaticaArchivo"])) {
	$PlaticasMaquina = new PlaticasMaquina();
	$PlaticasMaquina->updatePlaticaArchivo();
}
