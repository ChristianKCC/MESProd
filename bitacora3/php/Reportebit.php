...
Class ReporteBit{
	function tblasistenciasbitrep(){
		...
		WHERE tblEncabezadoBitacora.IdEncabezadoBItacora = $folio";
		$array=array();
	...
	function tblpresentacionesbitrep(){
		...
		WHERE tblBitPresentacionGolpes.hora = tblBitPresentacionSub.hora AND  tblEncabezadoBitacora.IdEncabezadoBItacora = $folio";
		$array=array();
		$result=sqlsrv_query($conn,$query);
		while($row = sqlsrv_fetch_array($result)){
