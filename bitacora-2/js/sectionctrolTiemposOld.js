export class Tiempos {
  async tblctrltiempos() {
    ...
    const respuestaraw = await fetch(
      "./php/bitacora.php?tblctrltiempos&folio=" + folio
    );
    ...
    respuesta.forEach((element) => {
      body += `
            <tr>
                ...
                <td>
                <button class="btn btn-sm btn-warning" onclick="consultarCtrlTiempos(${element.id}); return false;"><i class="fa-solid fa-pen"></i></button>
                    <a class="btn btn-sm btn-info" data-bs-toggle="modal" data-bs-target="#modalsanitizacionOld" data-bs-whatever="${element.id}">Sanitización</a>
                </td>
            </tr>`;
    });
    document.getElementById("tblctrltiemposOld").innerHTML = body;
  }

  async tblInfoParos() {
    let folio = document.getElementById("folio").value;
    const dataPromise = await fetch(
      "./php/bitacora.php?tblInfoParos&folio=" + folio
    );
      ...
      body += `
        <tr>
            ...
            <td>${element.Rechazos ?? "Sin informacion"}</td>
             <td>${
               element.TParo == null || element.TParo === ""
                 ? "Sin informacion"
                 : isNaN(Number(element.TParo))
                 ? element.TParo
                 : Number(element.TParo).toFixed(2)
             }</td>
            <td>${
              element.Motivo == null || element.Motivo === ""
                ? "Sin informacion"
                : element.Motivo
            }</td>
            <td>${
              element.Correccion == null || element.Correccion === ""
                ? "Sin informacion"
                : element.Correccion
            }</td>
            <td>
  ...
  async saveSanitizacion(folio, motivo, tiempo, empleados) {
