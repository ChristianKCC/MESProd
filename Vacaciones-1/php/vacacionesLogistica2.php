...
function normalizarFechaISO(string $fechaStr): string {
    ...
    try {
        ...
        // Log objeto DateTime
        error_log("📌 normalizarFechaISO: interpretado = " . $fecha->format('Y-m-d'));

        return $fecha->format('Y-m-d'); // siempre ISO
    } catch (Exception $e) {
        error_log("❌ normalizarFechaISO: error = " . $e->getMessage());
        return '';
    }
}
...
function calcularAntiguedad(string $fechaISO): string {
    ...
    $resultado = ($anios === 0) ? "$meses mes(es)" :
                 (($meses === 0) ? "$anios año(s)" : "$anios año(s) y $meses mes(es)");

    error_log("📌 calcularAntiguedad: fechaISO = [$fechaISO], resultado = [$resultado]");
