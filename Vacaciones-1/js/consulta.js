    ...
    btnConsultarEmpleado.addEventListener("click", function() {
        ...
        fetch(`../Vacaciones/php/buscarEmpleado.php?ibm=${encodeURIComponent(ibm)}&nombre=${encodeURIComponent(nombre)}`)
            ...
            .then(data => {
                ...
                document.querySelector(".dato-valor.fingreso code").innerText = fechaISO;
                document.querySelector(".dato-valor.antiguedad code").innerText = calcularAntiguedad(fechaISO);
...
//   (respuesta.data ?? respuesta).forEach(folio => {
...
//     let accionesHtml = (folio.revisado == 1 || folio.autorizado == 2)
...
// }    
    ...
    async function cargarSolicitudesModal() {
        ...
        const totalRegistros = (respuesta.total ?? (respuesta.data ?? respuesta).length);
