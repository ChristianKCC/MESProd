...
function transformarDatos(array $datos, string $fecha, int $noDepto = 0): array
{
        ...
        if ($idCategoria !== null && isset($configuracionMaquinas[$idCategoria])) {
            ...
            foreach ($orden as $noMaq => $nombreMostrar) {
                ...
                if ($encontrada !== null) {
                    ...
                    // Marcar como "configurada" para que no se elimine más adelante
                    // aunque su producción de los últimos 7 días termine en 0
                    // (ej. PA03 con solo AcumuladoUSTD y sin USTD diario)
                    $maquinasOrdenadas[$nombreMostrar]['_forzada'] = true;
                } else {
                    // Sin datos — insertar vacía (marcada para no ser eliminada por el filtro)
    ...
    foreach ($agrupado as $cat => &$maquinas) {
        foreach ($maquinas as $maq => &$data) {
            ...
            // pero respetar las máquinas "forzadas" (definidas en configuracionMaquinas)
            if (empty($data['productos']) && empty($data['_forzada'])) {
                unset($maquinas[$maq]);
            }
        }
        unset($data);
