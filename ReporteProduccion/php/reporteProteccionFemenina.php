...
function pintarEncabezadoTurnos(FPDF $pdf, float $x, float $y): void
{
    ...
    $labels = ['MP01', 'MP03', 'MP08', 'MP09', 'MP10', 'MP11', 'MP12', 'MP13', 'MP14', 'MP15', 'MP16', 'Total Turno'];
    $offsets = [9, 19, 29, 39, 49, 59, 69, 79, 89, 99, 109, 119];
...
function pintarEncabezadoClaves(FPDF $pdf, float $x, float $y): void
{
    ...
    // Encabezados
    $pdf->SetXY($x, $y - 2);
    $pdf->Cell(8, 3.5, 'CLAVE', 1, 1, 'C', true);

    $labels = ['DESCRIPCION', 'MP01', 'MP03', 'MP08', 'MP09', 'MP10', 'MP11', 'MP12', 'MP13', 'MP14', 'MP15', 'MP16', 'Total Turno'];
    $offsets = [-15, 25, 38, 51, 64, 77, 90, 103, 116, 129, 142, 155, 168];
    $widths = [40, 13, 13, 13, 13, 13, 13, 13, 13, 13, 13, 13, 18];
...
{
    $colMachines = ['BCM4', 'BCM3', 'PE10', 'BCM1', 'MP22', 'MP25'];
...
function pintarCuerpoTurnos(
        ...
        foreach ($colMachines as $idx => $nombreMaquina) {
            ...
            if (isset($mapaNombres[$nombreMaquina])) {
                ...
                if (isset($dataDia[$noMaq][$i]) && isset($dataDia[$noMaq][$i][$campo]) && is_numeric($dataDia[$noMaq][$i][$campo])) {
                    ...
                    $valorStr = ($valorNum == 0.0) ? '' : number_format($valorNum, 2);
                }
            }
            ...
            $pdf->Cell($colW[$idx + 1], $rowHeight, $valorStr, 1, 0, 'R');
        }

        $pdf->SetXY($x + 119, $currY - 2);
        $pdf->Cell($colW[12], $rowHeight, number_format($totalTurno), 1, 0, 'R');
    ...
    $pdf->SetXY($x + 119, $totalY - 2);
    $pdf->Cell($colW[12], $rowHeight, number_format($totalGeneral), 1, 0, 'R', true);
    ...
    if ($agregarAcumulado && $campoAcumulado !== '' && !empty($dataPeriodo)) {
        ...
        $pdf->MultiCell($colW[0], $rowHeight, $labelAcumulado, 1, 'C', 'C', true);
        ...
        foreach ($colMachines as $idx => $nombreMaquina) {
            ...
            $pdf->Cell($colW[$idx + 1], $rowHeight, $valorDisplay, 1, 0, 'R', true);
        }

        $pdf->SetXY($x + 119, $acumY - 2);
        $pdf->Cell($colW[12], $rowHeight, number_format($acum['totalGeneral']), 1, 0, 'R', true);
    }
...
function pintarCuerpoReales(
    ...
    for ($i = 0; $i < 3; $i++) {
        ...
        $pdf->Cell($colW[12], $rowHeight, number_format($trunc2, 2, '.', ''), 1, 0, 'C');
    ...
    $pdf->Cell($colW[12], $rowHeight, number_format($valorTruncado, 2), 1, 0, 'C', true);
    ...
    if ($agregarAcumulado && $campoAcumulado !== '' && !empty($dataPeriodo)) {
        ...
        $pdf->Cell($colW[12], $rowHeight, number_format($acum['totalGeneral'], 2), 1, 0, 'C', true);
    }
...
function pintarCuerpoCortesRechazos(
    ...
    $pdf->Cell($colW[0], $rowHeight, 'TOTAL', 1, 0, 'C', true);
    ...
    if ($agregarAcumulado && $campoAcumulado !== '' && !empty($dataPeriodo)) {
        ...
        $pdf->SetFillColor(245, 245, 245);
        $pdf->SetXY($x - 3, $acumY);
        $pdf->MultiCell($colW[0], $rowHeight - 1.75, $labelAcumulado, 1, 'C', 'C', true);
...
function pintarCuerpoMermaMaquinas(
    ...
    for ($i = 0; $i < 3; $i++) {
        ...
        if ($modoPorcentaje) {
            ...
            $pdf->Cell($colW[12], $rowHeight, number_format($totalTurnoSimple), 1, 0, 'R');
        }
    }
    ...
    if ($modoPorcentaje) {
        ...
        $pdf->Cell($colW[12], $rowHeight, number_format($pctG, 2) . '%', 1, 0, 'C', true);
    } else {
        $pdf->Cell($colW[12], $rowHeight, number_format($totalGeneral), 1, 0, 'C', true);
    }
    ...
    if ($agregarAcumulado && $campoAcumulado !== '' && !empty($dataPeriodo)) {
        ...
        // MultiCell estándar de FPDF: (w, h, txt, border, align, fill)
        $pdf->SetFillColor(235, 242, 250);
        $pdf->MultiCell($colW[0], $rowHeight, 'ACUMULADO', 1, 'C', true);
        ...
        if ($modoPorcentaje) {
            ...
            $pdf->SetXY($x + 119, $acumY);
            $pdf->Cell($colW[12], $rowHeight, number_format((float) $acum['totalGeneral']), 1, 0, 'R', true);
        }
    }
...
function pintarMermaMaquinasTotal(
    ...
    if ($agregarAcumulado && $campoAcumulado !== '' && !empty($dataPeriodo)) {
        ...
        $pdf->Cell($colW[12], $rowHeight, $numGTexto, 1, 0, 'C', true);
    ...
    // ---------------- Fila YIELD ----------------
    if ($agregarAcumulado && $campoAcumulado !== '' && !empty($dataPeriodo)) {
        $acumY = $totalY + $rowHeight + 5;

        $pdf->SetFillColor(245, 245, 245);
        $pdf->SetXY($x - 3, $acumY - 1.5);
        $pdf->SetFillColor(235, 242, 250);
        ...
        if ($modoPorcentaje) {
            ...
            foreach ($colMachines as $idx => $nombreMaquina) {
                $pdf->SetXY($x + $colOffsets[$idx + 1], $acumY - 1.5);
            ...
            $pdf->SetXY($x + 119, $acumY);
            $pdf->Cell($colW[12], $rowHeight, $acum, 1, 0, 'R', true);
        }
    }
...
function pintarTiempoAbajoMaquinas(
    ...
    for ($i = 0; $i < 3; $i++) {
        ...
        $valortTotal = ($minutosTrabajadosTurno > 0)
            ...
            : '';
        $pdf->Cell($colW[12], $rowHeight, number_format($totalTurno, 0) . ' / ' . $valortTotal, 1, 0, 'C');
    ...
    if ($agregarAcumulado && $campoAcumulado !== '' && !empty($dataPeriodo)) {
        ...
        foreach ($colMachines as $idx => $nombreMaquina) {
            $pdf->SetXY($x + $colOffsets[$idx + 1], $acumY + 10.5);
    ...
    if ($agregarAcumulado && $campoAcumulado !== '' && !empty($dataPeriodo)) {
        ...
        foreach ($colMachines as $idx => $nombreMaquina) {
            $pdf->SetXY($x + $colOffsets[$idx + 1], $acumY + 14);
            $num = (float) ($acumNum['porMaquina'][$nombreMaquina] ?? '');
            $den = (float) ($acumDen['porMaquina'][$nombreMaquina] ?? '');
    ...
    if ($agregarAcumulado && $campoAcumulado !== '' && !empty($dataPeriodo)) {
        ...
        foreach ($colMachines as $idx => $nombreMaquina) {
            $pdf->SetXY($x + $colOffsets[$idx + 1], $acumY + 17.5);
            $num = (float) ($acumNum['porMaquina'][$nombreMaquina] ?? '');
            $den = (float) ($acumDen['porMaquina'][$nombreMaquina] ?? '');
    ...
    if ($agregarAcumulado && $campoAcumulado !== '' && !empty($dataPeriodo)) {
        ...
        foreach ($colMachines as $idx => $nombreMaquina) {
            $pdf->SetXY($x + $colOffsets[$idx + 1], $acumY + 15);
            $num = (float) ($acumNum['porMaquina'][$nombreMaquina] ?? '');
            $den = (float) ($acumDen['porMaquina'][$nombreMaquina] ?? '');
...
function pintarMermaEmpaque(
    ...
    for ($i = 0; $i < 3; $i++) {
        ...
        $pdf->SetXY($x + 119, $currY);
        $pdf->Cell($colW[12], $rowHeight, $valorSTRTotalTurno, 1, 0, 'C');
    ...
    $pdf->Cell($colW[12], $rowHeight, $texto, 1, 0, 'C', true);
...
function pintarTiemposMaquinas(
    ...
    for ($i = 0; $i < 3; $i++) {
        ...
        $pdf->SetXY($x + 119, $currY + 6);
        $pdf->Cell($colW[12], $rowHeight, $valorSTRTotal, 1, 0, 'C');
    ...
    // ---------------- Fila acumulado de TiempoArriba (en minutos) ----------------
    {
        ...
        foreach ($colMachines as $idx => $nombreMaquina) {
            ...
            $min = (float) ($hrs * 60);
            $texto = ($hrs != 0.0) ? number_format($min) . ' / ' . number_format($hrs, 2) : '';
            $pdf->Cell($colW[$idx + 1], $rowHeight, $texto, 1, 0, 'C', true);
    ...
    if ($agregarAcumulado && $campoAcumulado !== '' && !empty($dataPeriodo)) {
        ...
        $pdf->SetXY($x + 119, $acumY + $rowHeight);
        $numG = (float) ($acumNum['totalGeneral'] ?? '');
        $textoG = ($numG != 0.0) ? number_format(($numG), 2) : '';
        $pdf->Cell($colW[12], $rowHeight, $textoG, 1, 0, 'C', true);
...
function pintarOEEMaquinas(
    ...
    $pdf->Cell($colW[12], $rowHeight, number_format($oeeTD, 2) . '%', 1, 0, 'C', true);
    ...
    if ($agregarAcumulado && $campoAcumulado !== '' && !empty($dataPeriodo)) {
        ...
        $acumUSTD = sumarCampoCortesRechazos($dataCortesRechazos, $mapaNombres, $campoUSTD);     // USTD 
        $acumDen = sumarCampoCortesRechazos($dataPeriodo, $mapaNombres, $campoDenominador);     // Cortes del periodo
        $acumNum = sumarCampoCortesRechazos($dataPeriodo, $mapaNombres, $campoNumerador);     // TotalPiezas
        $acumHTrabajadas = sumarTiempoArribaDiaPorMaquina($dataPeriodo, $mapaNombres, $campoHorasTrabajadas); // Horas trabajadas 
        $acumTAbajo = sumarTiempoAbajoDiaPorMaquina($dataPeriodo, $mapaNombres, $campoTiempoAbajo);  // Tiempo abajo
...
$texto = mb_convert_encoding("REPORTE DE PRODUCCIÓN PROTECCIÓN FEMENINA", 'ISO-8859-1', 'UTF-8');
...
$pdf->SetXY($x - 5, $y - 2);
$pdf->Cell(20, 5, '_________________________________________________________________________________________________________________________________________________________________________________________________________________________________________________________________________________');
$endY = $pdf->GetY();
...
$pdf->SetFont('Arial', 'B', 8);
$pdf->SetXY($tabla1_x + 63, $tabla1_y - 10);
...
$pdf->SetTextColor(30, 144, 227);
$pdf->SetFont('Arial', 'B', 8);
$pdf->SetXY($tabla2_x + 58, $tabla2_y - 10);
...
//------------------------------------------ TABLA 3: CORTES ----------------------------------------------------
$tabla3_x = $x - 2;
$tabla3_y = $tabla1_y + 33;
$pdf->SetTextColor(30, 144, 227);
$pdf->SetFont('Arial', 'B', 8);
$pdf->SetXY($tabla3_x + 60, $tabla3_y - 12);
...
$tabla5_x = $x - 2;
$tabla5_y = $tabla1_y + 62;
$pdf->SetTextColor(30, 144, 227);
$pdf->SetFont('Arial', 'B', 8);
$pdf->SetXY($tabla5_x + 50, $tabla5_y - 12);
...
//------------------------------------------ TABLA 7: % TPT ----------------------------------------------------------
$tabla7_x = $x - 2;
$tabla7_y = $tabla1_y + 90;
$pdf->SetTextColor(30, 144, 227);
$pdf->SetFont('Arial', 'B', 8);
$pdf->SetXY($tabla7_x + 53, $tabla7_y - 12);
...
//------------------------------------------ TABLA 9: HORAS TRABAJADAS -----------------------------------------------
$tabla9_x = $x + -2;
$tabla9_y = $tabla1_y + 130;
$pdf->SetTextColor(30, 144, 227);
$pdf->SetFont('Arial', 'B', 8);
$pdf->SetXY($tabla9_x + 50, $tabla9_y - 12);
$pdf->Write(10, mb_convert_encoding("HORAS TRABAJADAS", 'ISO-8859-1', 'UTF-8'));
...
$pdf->SetXY($tabla11_x + 80, $tabla11_y - 14);
$pdf->Write(10, mb_convert_encoding("CLAVES DE PRODUCCIÓN", 'ISO-8859-1', 'UTF-8'));
pintarEncabezadoClaves($pdf, $tabla11_x, $tabla11_y - 4);
