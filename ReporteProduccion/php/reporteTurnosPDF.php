...
$turnosObj = new TurnosSinConexion();
$fecha = $_GET['fecha'];
$maquina = $_GET['maquina'];
$turno = $_GET['turno'];

$data = $turnosObj->getDataTurnosAnteriores($fecha, $maquina, $turno);
$data2 = $turnosObj->clavesMaquinasSinRed($fecha, $maquina, $turno);
$dataSecciones = $turnosObj->seccionesMaquinasSinConexion($fecha, $maquina, $turno);
...
if (!empty($data) && is_array($data)) {
    foreach ($data as $row) {
        if (!empty($row['Fecha']) && is_string($row['Fecha'])) {
            $fechaPDF = mb_convert_encoding(
                $row['Fecha'],
                'ISO-8859-1',
                'UTF-8'
            );
...
$pdf->MultiCell(40, 5, $fechaPDF, 1, 'C'); //fecha
$endY = $pdf->GetY();
...
$pdf->MultiCell(25, 5, $nombreMaquina, 1, 'C');
$endY = $pdf->GetY();
...
$pdf->SetX($x + 105);
// Buscar el primer Conductor de máquina válido en $data
$nombreConductor = 'N/A';
$targetTurno = (is_numeric($turnoValue) ? (int) $turnoValue : null);

if (!empty($data) && is_array($data)) {
    // Intentar obtener conductor del turno especificado
    if ($targetTurno !== null) {
        foreach ($data as $row) {
            if (
                isset($row['Turno']) && (int) $row['Turno'] === $targetTurno
                && isset($row['NombreEmpleado']) && $row['NombreEmpleado'] !== null
                && trim((string) $row['NombreEmpleado']) !== ''
            ) {
                $nombreConductor = mb_convert_encoding($row['NombreEmpleado'], 'ISO-8859-1', 'UTF-8');
                break;
            }
        }
    }
    // Si no se encontró conductor para ese turno, tomar el primer disponible
    if ($nombreConductor === 'N/A') {
        foreach ($data as $row) {
            if (isset($row['NombreEmpleado']) && $row['NombreEmpleado'] !== null && trim((string) $row['NombreEmpleado']) !== '') {
                $nombreConductor = mb_convert_encoding($row['NombreEmpleado'], 'ISO-8859-1', 'UTF-8');
                break;
            }
        }
...
$pdf->MultiCell(65, 5, $nombreConductor, 1, 'C');
$endY = $pdf->GetY();
$pdf->SetXY($x + 45, $endY - 5);
$pdf->SetTextColor(0, 0, 0);
// //-----------------------------------------------------------------------------------------------------------------
...
$pdf->MultiCell(18, 8, mb_convert_encoding('Tiempo abajo', 'ISO-8859-1', 'UTF-8'), 1, 'C');
//-----------------------------------------------------------------------------------------------------------------
...
if ($data[0]['Turno'] == 1) {
    ...
    //Cortes
    $pdf->SetXY($x + 26, $y + 60);
    $pdf->MultiCell(18, 7, $data[0]['Cortes'], 0, 'C');
    //Cajas reales
    $pdf->SetXY($x + 44, $y + 60);
    $pdf->MultiCell(20, 7, $data[0]['PañalEmpacado'], 0, 'C');
    //Rechazos
    $pdf->SetXY($x + 64, $y + 60);
    // Rechazos (maneja null)
    $rechazos = (isset($data[0]['Rechazos']) && $data[0]['Rechazos'] !== null) ? $data[0]['Rechazos'] : 0;
    $pdf->MultiCell(18, 7, $rechazos, 0, 'C');
    //Número de paros
    $pdf->SetXY($x + 82, $y + 60);
    $pdf->MultiCell(18, 7, $data[0]['ParosMaquina'], 0, 'C');
    ...
    $pdf->MultiCell(18, 7, $data[0]['TiempoPerdido'], 0, 'C');
    // // Horas trabajadas
    $pdf->SetXY($x + 118, $y + 60);
    $pdf->MultiCell(20, 7, $data[0]['HorasTrabajadas'], 0, 'C');
    // // Porcentaje de merma en máquina
    $pdf->SetXY($x + 138, $y + 60);
    $rechazosA = isset($data[0]['Rechazos']) && is_numeric($data[0]['Rechazos']) ? (float) $data[0]['Rechazos'] : 0;
    $cortesA = isset($data[0]['Cortes']) && is_numeric($data[0]['Cortes']) ? (float) $data[0]['Cortes'] : 0;
    $merma = $cortesA != 0 ? ($rechazosA / $cortesA) * 100 : 0;
    $pdf->MultiCell(18, 7, number_format($merma, 2) . '%', 0, 'C');
    // Porcentaje de merma total 
    $pdf->SetXY($x + 156, $y + 60);
    $pdf->MultiCell(18, 7, number_format($data[0]['PañalEmpacado'] != 0 ? (1 - ($data[0]['PañalEmpacado'] / $data[0]['Cortes'])) * 100 : 0, 2) . '%', 0, 'C');
    // // Porcentaje de tiempo perdido
    $pdf->SetXY($x + 174, $y + 60);
    $pdf->MultiCell(18, 7, number_format($data[0]['TiempoPerdido'] != 0 ? (($data[0]['TiempoPerdido'] / 480)) * 100 : 0, 2) . '%', 0, 'C');
    //------------------------------------------------------ Fin de turno 1 -----------------------------------------------------------
...
if (isset($data[1]) && (($data[1]['Turno'] ?? null) == 2)) {
    ...
    $p1 = $data[1]['ParosMaquina'] ?? 0;
    $t1 = $data[1]['TiempoPerdido'] ?? 0;
    ...
    // //Cajas reales
    $pdf->SetXY($x + 44, $y + 67);
    $pdf->MultiCell(20, 7, $data[1]['PañalEmpacado'], 0, 'C');
    // //Rechazos
    $rechazos2 = (isset($data[1]['Rechazos']) && $data[1]['Rechazos'] !== null) ? $data[1]['Rechazos'] : 0;
    $pdf->SetXY($x + 64, $y + 67);
    $pdf->MultiCell(18, 7, $rechazos2, 0, 'C');
    // //Número de paros
    $pdf->SetXY($x + 82, $y + 67);
    ...
    //Porcentaje de merma en máquina (manejar null/no numérico en $rechazos2)
    $rechazos2_num = (isset($rechazos2) && is_numeric($rechazos2)) ? (float) $rechazos2 : 0.0;
    $merma1 = ($c1 != 0) ? number_format(($rechazos2_num / (float) $c1) * 100, 2) . '%' : '0.00%';
...
if (isset($data[2]) && (($data[2]['Turno'] ?? null) == 3)) {
    ...
    $c2 = $data[2]['Cortes'] ?? 0;
    $r2 = $data[2]['Rechazos'] ?? 0;
    $p2 = $data[2]['ParosMaquina'] ?? 0;
    $ta2 = $data[2]['TiempoPerdido'] ?? 0;
...
//     }
    $h2 = $data[2]['HorasTrabajadas'] ?? 0;
    $m2 = ($data[2]['PañalEmpacado'] ?? 0);
    ...
    $rechazos2_num = (isset($r2) && is_numeric($r2)) ? (float) $r2 : 0.0;
    $merma2 = ($c2 != 0) ? number_format(($rechazos2_num / (float) $c2) * 100, 2) . '%' : '0.00%';
    $pdf->MultiCell(18, 7, $merma2, 0, 'C');
    // Porcentaje de merma total (fijo)
    $pdf->SetXY($x + 156, $y + 74);
    $porcMermaTotal = ($c2 != 0) ? number_format((1 - ($m2 / $c2)) * 100, 2) . '%' : '0.00%';
    $pdf->MultiCell(18, 7, $porcMermaTotal, 0, 'C');
    $pdf->MultiCell(18, 7, '', 0, 'C');
    // Porcentaje de tiempo perdido
...
foreach ($data as $registro) {
    if (isset($registro['Turno']) && $registro['Turno'] !== null) {
        $CortesTotal += (float) $registro['Cortes'];
        $RechazosTotal += (float) $registro['Rechazos'];
        $ParosTotal += (float) $registro['ParosMaquina'];
        $tiempoRaw = $registro['TiempoPerdido'] ?? 0;
        $totalPañales += $registro['PañalEmpacado'];
        $TiempoAbajoTotal += is_numeric($tiempoRaw) ? (float) $tiempoRaw : 0;
        $horasTrabajadasTotal += (float) $registro['HorasTrabajadas'];
        $mermaMaquinaTotal += (float) $registro['PañalEmpacado'];
        $turnosConsiderados++;
    }
}
...
foreach ($dataSecciones as $turno => $registros) {
    foreach ($registros as $r) {
        $seccion = $r['NombreSeccion'] ?? mb_convert_encoding('Sin registro', 'ISO-8859-1', 'UTF-8');
...
$pdf->SetXY(9, $endY + 20);
$pdf->Cell(20, 5, '__________________________________________________________________');

// Posición base para las filas de datos
$endY = $pdf->GetY();
$pdf->SetFont('Arial', '', 8);
$pdf->SetTextColor(0, 0, 0);
$pdf->SetXY(10, $endY + 5);

// Inicializa lineY justo debajo del encabezado (no en 0)
$lineY = $endY + 5;

// Bandera para saber si se imprimió al menos una fila
$hayFilas = false;
...
foreach ($seccionesAgrupadas as $seccion => $datos) {
    ...
    foreach ([1, 2, 3] as $t) {
        ...
        $pdf->SetFont('Arial', '', 7);
        // Si no hay paros en el turno, muestra celda vacía
        $pdf->Cell(8, 5, $paros > 0 ? $paros : '', 'B', 0, 'C');
        $pdf->Cell(10, 5, ($paros > 0 && $datos[$t]['sato'] > 0) ? $sato : '', 'B', 0, 'C');

        $totalParos += $paros;
    }

    // Sato total por sección basado en totalParos y tiempoArribaTotal (evitar div/0)
    $totalSato = ($totalParos * 1440) / max($tiempoArribaTotal, 1);

    $pdf->Cell(10, 5, $totalParos, 'B', 0, 'C');
    $pdf->Cell(10, 5, $totalParos > 0 ? number_format($totalSato, 2) : '', 'B', 1, 'C');

    $endY = $pdf->GetY();
    $pdf->SetXY(10, $endY);
    $lineY = $endY; // para posicionar la fila de totales al final de las filas
}

// ==== Totales de secciones ====
// Calcula totales por turno y globales
$totalParosSecciones = 0;
$totalSatoSecciones = 0;
$totalesPorTurno = [1 => ['paros' => 0, 'sato' => 0], 2 => ['paros' => 0, 'sato' => 0], 3 => ['paros' => 0, 'sato' => 0]];

foreach ([1, 2, 3] as $t) {
    foreach ($seccionesAgrupadas as $datos) {
        $totalesPorTurno[$t]['paros'] += $datos[$t]['count'];
        $totalesPorTurno[$t]['sato'] += $datos[$t]['sato'];
    }
    $totalParosSecciones += $totalesPorTurno[$t]['paros'];
    $totalSatoSecciones += $totalesPorTurno[$t]['sato'];
}

// Mostrar la fila "Total" solo si hubo filas y hay algún valor > 0
if ($hayFilas && ($totalParosSecciones > 0 || $totalSatoSecciones > 0)) {
    $pdf->SetXY(30, $lineY);
    $pdf->SetTextColor(30, 144, 227);
    $pdf->SetFont('Arial', 'B', 8);
    ...
    foreach ([1, 2, 3] as $t) {
        ...
        // Solo imprime números si hay datos en el turno; si no, deja vacío
        $p = $totalesPorTurno[$t]['paros'];
        $s = $totalesPorTurno[$t]['sato'];

        $pdf->Cell(8, 5, $p > 0 ? $p : '', 0, 0, 'C');
        $pdf->Cell(10, 5, $s > 0 ? number_format($s, 2) : '', 0, 0, 'C');
    }

    $pdf->Cell(10, 5, $totalParosSecciones > 0 ? $totalParosSecciones : '', 0, 0, 'C');
    $totalSatoDia = ($tiempoArribaTotal > 0) ? number_format((($totalParosSecciones * 1440) / $tiempoArribaTotal), 2) : '0.00';
    $pdf->Cell(10, 5, $totalParosSecciones > 0 ? $totalSatoDia : '', 0, 1, 'C');
}
...
$pdf->SetTextColor(30, 144, 227);
$pdf->SetXY(20, 20);
$pdf->Cell(190, 10, mb_convert_encoding('Detalle de paros por sección', 'ISO-8859-1', 'UTF-8'), 0, 1, 'C');
$endY = $pdf->GetY();
...
foreach ($dataSecciones as $turno => $eventos) {
    ...
    foreach ($eventos as $evento) {
        ...
        // Hora
        $pdf->SetXY($x, $y);
        $pdf->Cell($wHora, $rowHeight, $evento['HoraInicio'], 0, 0, 'C');
        $x += $wHora;

        // Sección
        $pdf->SetXY($x, $y);
        $pdf->Cell($wSeccion, $rowHeight, ucfirst(mb_strtolower(mb_convert_encoding($evento['NombreSeccion'], 'ISO-8859-1', 'UTF-8'))), 0, 0, 'J');
        $x += $wSeccion;

        // Módulo (usa SIEMPRE 43)
        $pdf->SetXY($x, $y);
        $pdf->Cell($wModulo, $rowHeight, ucfirst(mb_strtolower(mb_convert_encoding($evento['NombreModulo'], 'ISO-8859-1', 'UTF-8'))), 0, 0, 'J');
        $x += $wModulo;
        ...
        $pdf->SetXY($x, $y);
        $pdf->MultiCell($wMotivo, $lineHeight, mb_convert_encoding(ucfirst(mb_strtolower($motivo)), 'ISO-8859-1', 'UTF-8'), 0, 'J');
        $x += $wMotivo;

        // Corrección
        $pdf->SetXY($x, $y);
        $pdf->MultiCell($wCorreccion, $lineHeight, mb_convert_encoding(ucfirst(mb_strtolower($correccion)), 'ISO-8859-1', 'UTF-8'), 0, 'J');
