...
$dataPeriodo = $departamentosObj->getReporteTelasNoTejidas($fechai, $fechaf);
...
$dataCortesRechazos = $departamentosObj->getInfoReporteTurnosTNT($fechai, $fechaf);
...
// Para los tiempos
$dataTiemposDeptos = $departamentosObj->getTiemposTNT($fechai, $fechaf);
...
$dataClavesProduccion = $departamentosObj->getInfoClavesTNT($fechai, $fechaf);
...
function pintarEncabezadoTurnos(FPDF $pdf, float $x, float $y): void
{
    ...
    // Encabezados
    $pdf->SetXY($x - 3, $y - 2);
    $pdf->Cell(12, 3.5, 'TURNO', 1, 0, 'C', true);

    $labels = ['SPOOLER 1', 'SPOOLER 3', 'TABBI'];
    $offsets = [9, 21, 33];
    $widths = [12, 12, 12];
...
function pintarEncabezadoClaves(FPDF $pdf, float $x, float $y): void
{
    ...
    $labels = ['DESCRIPCION', 'SPOOLER 1', 'SPOOLER 3', 'TABBI'];
    $offsets = [-8, 36, 51, 66];
    $widths = [44, 15, 15, 15];
...
{
    $colMachines = ['SPOOLER 1', 'SPOOLER 3', 'TABBI'];
...
function truncarDecimales($numero, $precision = 2)
{
    ...
    // Épsilon minúsculo para blindar contra errores de representación
    // binaria de punto flotante (ej. 54.756 guardado como 54.755999999...
    // tras sumas en SQL/PHP), que de otro modo el floor() truncaría mal.
    $epsilon = 1e-9;
    if ($numero >= 0) {
        return floor($numero * $factor + $epsilon) / $factor;
    } else {
        return ceil($numero * $factor - $epsilon) / $factor; // hacia 0 para negativos
    }
}
...
function pintarCuerpoTurnos(
    ...
    array $mapaNombres,
    string $campoMMC,                  // lo que muestra por turno: 'USTD' o 'Reales'
    bool $agregarAcumulado = false,
    array $dataPeriodo = [],        // todo el rango: $fechai..$fechaf
    string $campoAcumuladoMMC = '',    // 'TotalUSTD' o 'TotalReal'
    ...
    for ($i = 0; $i < 3; $i++) { 
        ...
        foreach ($colMachines as $idx => $nombreMaquina) {
            ...
            if (isset($mapaNombres[$nombreMaquina])) {
                ...
                if (isset($dataDia[$noMaq][$i]) && isset($dataDia[$noMaq][$i][$campoMMC]) && is_numeric($dataDia[$noMaq][$i][$campoMMC])) {
                    $valorNum = (float) $dataDia[$noMaq][$i][$campoMMC];

                    if ($valorNum == 0.0) {
                        $valorStr = '';
                    } else {
                        // Truncado a 3 decimales hacia cero (usa el helper truncarDecimales)
                        $valorNum = truncarDecimales($valorNum, 3);

                        // Formatear con 3 decimales fijos, punto como separador
                        $valorStr = number_format($valorNum, 3, '.', '');
                    }
                }

            }

            // Se acumula el valor YA truncado para que TOTAL DIA/ACUMULADO
            // coincidan exactamente con la suma de lo que se ve por turno.
            $totalesPorMaquina[$idx] += $valorNum;
            $totalTurno += $valorNum;

            $pdf->SetXY($cellX, $currY - 2);
            $pdf->Cell($colW[3], $rowHeight, $valorStr, 1, 0, 'C');
        }
    ...
    foreach ($colMachines as $idx => $nombreMaquina) {
        ...
        $v = truncarDecimales($totalesPorMaquina[$idx] ?? 0.0, 3);
    ...
    if ($agregarAcumulado && $campoAcumuladoMMC !== '' && !empty($dataPeriodo)) {
        $acum = sumarCampoPorMaquina($dataPeriodo, $mapaNombres, $campoAcumuladoMMC);
        ...
        foreach ($colMachines as $idx => $nombreMaquina) {
            ...
            $v = truncarDecimales((float) ($acum['porMaquina'][$nombreMaquina] ?? 0.0), 3);
            $valorDisplay = ((float) $v === 0.0) ? '' : number_format((float) $v);
            $pdf->Cell($colW[3], $rowHeight, $valorDisplay, 1, 0, 'C', true);
        }
    }
    ...
    if ($agregarAcumulado && $campoAcumuladoMMC !== '' && !empty($dataPeriodo)) {
        ...
        $pdf->MultiCell($colW[0], $rowHeight - 1.75, 'PROMEDIO MML DIA', 1, 'C', true);
        $acum = sumarCampoPorMaquina($dataPeriodo, $mapaNombres, $campoAcumuladoMMC);             // USTD Acumuladas
        ...
        foreach ($colMachines as $idx => $nombreMaquina) {
            ...
            $pdf->Cell($colW[3], $rowHeight, $texto, 1, 0, 'C', true);
        }
...
function pintarCuerpoMetrosCuadrados(
    FPDF $pdf,
    float $x,
    float $y,
    array $dataDia,                 // solo Fecha == $fechaf
    ...
    for ($i = 0; $i < 3; $i++) {
        ...
        foreach ($colMachines as $idx => $nombreMaquina) {
            ...
            // Se acumula el valor YA truncado para que TOTAL DIA coincida
            // exactamente con la suma de lo que se ve por turno (evita el
            // desfase visto entre esta tabla y CLAVES DE PRODUCCIÓN).
            $totalesPorMaquina[$idx] += $valorNum;
            $totalTurno += $valorNum;
    ...
    foreach ($colMachines as $idx => $nombreMaquina) {
        $pdf->SetXY($x + $colOffsets[$idx + 1], $totalY - 2);
        $v = truncarDecimales($totalesPorMaquina[$idx] ?? 0.0, 3);
        $valorDisplay = ((float) $v === 0.0) ? '' : number_format($v, 3);
        $pdf->Cell(
            $colW[$idx + 1],
            $rowHeight,
            $valorDisplay,
    ...
    if ($agregarAcumulado && $campoAcumulado !== '' && !empty($dataPeriodo)) {
        ...
        foreach ($colMachines as $idx => $nombreMaquina) {
            $pdf->SetXY($x + $colOffsets[$idx + 1], $acumY - 2);
            $v = truncarDecimales((float) ($acum['porMaquina'][$nombreMaquina] ?? 0.0), 3);
            $valorDisplay = ((float) $v === 0.0) ? '' : number_format((float) $v, 3, '.', '');
            $pdf->Cell($colW[$idx + 1], $rowHeight, $valorDisplay, 1, 0, 'C', true);
        }

        $pdf->SetXY($x + 24, $acumY - 2);
    }
    ...
    if ($agregarAcumulado && $campoAcumulado !== '' && !empty($dataPeriodo)) {
        ...
        $pdf->SetXY($x - 3, $acumY + 10);
        // MultiCell estándar de FPDF: (w, h, txt, border, align, fill)
        $pdf->MultiCell($colW[0], $rowHeight - 1.75, 'PROMEDIO MMC DIA', 1, 'C', true);
        ...
        foreach ($colMachines as $idx => $nombreMaquina) {
            ...
            $ustdProm = ($acum['porMaquina'][$nombreMaquina]) ?? 0.0;
            $num = (float) ($acumNum['porMaquina'][$nombreMaquina] ?? '');
            $dias = (float) number_format(($num / 24), 3); // 1440 minutos en un día

            $texto = ($dias != 0.0) ? number_format(($ustdProm) / $dias, 3) : '';
...
function pintarCuerpoKilogramos(
    FPDF $pdf,
    float $x,
    float $y,
    array $dataDia,                 // solo Fecha == $fechaf
    ...
    for ($i = 0; $i < 3; $i++) {
        ...
        foreach ($colMachines as $idx => $nombreMaquina) {
            ...
            if (isset($mapaNombres[$nombreMaquina])) {
                ...
                if (isset($dataDia[$noMaq][$i]) && isset($dataDia[$noMaq][$i][$campo]) && is_numeric($dataDia[$noMaq][$i][$campo])) {
                    ...
                    if ($valorNum == 0.0) {
                        ...
                        // Redondeo normal a 0 decimales (si el 3er decimal es >=5, sube)
                        $valorNum = round($valorNum, 0);

                        // Formatear sin decimales, coma como separador de miles
                        $valorStr = number_format($valorNum, 0, '.', ',');
                    }
                }
            ...
            // Se acumula el valor YA truncado para que TOTAL DIA coincida
            // exactamente con la suma de lo que se ve por turno.
            $totalesPorMaquina[$idx] += $valorNum;
            $totalTurno += $valorNum;
    ...
    foreach ($colMachines as $idx => $nombreMaquina) {
        $pdf->SetXY($x + $colOffsets[$idx + 1], $totalY - 2);
        $v = round($totalesPorMaquina[$idx] ?? 0.0, 0);
        $valorDisplay = ((float) $v === 0.0) ? '' : number_format($v, 0, '.', ',');
        $pdf->Cell(
            $colW[$idx + 1],
            $rowHeight,
            $valorDisplay,
    ...
    if ($agregarAcumulado && $campoAcumulado !== '' && !empty($dataPeriodo)) {
        ...
        foreach ($colMachines as $idx => $nombreMaquina) {
            $pdf->SetXY($x + $colOffsets[$idx + 1], $acumY - 2);
            $v = round((float) ($acum['porMaquina'][$nombreMaquina] ?? 0.0), 0);
            $valorDisplay = ((float) $v === 0.0) ? '' : number_format((float) $v, 0, '.', ',');
            $pdf->Cell($colW[$idx + 1], $rowHeight, $valorDisplay, 1, 0, 'C', true);
        }
    }
    ...
    if ($agregarAcumulado && $campoAcumulado !== '' && !empty($dataPeriodo)) {
        ...
        $pdf->SetXY($x - 3, $acumY + 10);
        // MultiCell estándar de FPDF: (w, h, txt, border, align, fill)
        $pdf->MultiCell($colW[0], $rowHeight - 1.75, 'PROMEDIO KGS DIA', 1, 'C', true);
        $acum = sumarCampoPorMaquina($dataPeriodo, $mapaNombres, $campoAcumulado);             // USTD Acumuladas
        $acumNum = sumarCampoCortesRechazos($dataTiempos, $mapaNombres, $HorasTrabajadas);     // Dias Acumulados por máquina en el periodo
...
function pintarCuerpoCortesRechazos(
    ...
    // ---------------- Turnos 1..3 (del día final seleccionado) ----------------
    for ($i = 0; $i < 3; $i++) {
        $currY = $bodyStartY + ($i * $rowHeight)+2;
        ...
        foreach ($colMachines as $idx => $nombreMaquina) {
            ...
            if (isset($mapaNombres[$nombreMaquina])) {
                ...
                if (isset($dataCRD[$noMaq][$i]) && isset($dataCRD[$noMaq][$i][$campo]) && is_numeric($dataCRD[$noMaq][$i][$campo])) {
                    ...
                    $valorStr = ($valorNum == 0.0) ? '' : number_format($valorNum, 3,'.','');
    ...
    // ---------------- Fila TOTAL (del día, del campo $campo) ----------------
    $totalY = $bodyStartY + (3 * $rowHeight)+2;
    $pdf->SetFillColor(235, 242, 250);
 ...
 function pintarMermaMaquinasTotal(
    ...
    for ($i = 0; $i < 3; $i++) {
        ...
        foreach ($colMachines as $idx => $nombreMaquina) {
            ...
            if (isset($mapaNombres[$nombreMaquina])) {
                ...
                // Denominador
                if (isset($dataCRD[$noMaq][$i][$campoKilos]) && is_numeric($dataCRD[$noMaq][$i][$campoKilos])) {
                    $den = (float) $dataCRD[$noMaq][$i][$campoKilos];
                }

                // Numerador (si modo porcentaje)
                if ($modoPorcentaje && isset($dataCRD[$noMaq][$i][$campoKilosRechazados]) && is_numeric($dataCRD[$noMaq][$i][$campoKilosRechazados])) {
                    $num = (float) $dataCRD[$noMaq][$i][$campoKilosRechazados];
                }
            }
    ...
    if ($agregarAcumulado && $campoAcumulado !== '' && !empty($dataPeriodo)) {
        ...
        $acumDen = sumarCampoCortesRechazos($dataPeriodo, $mapaNombres, $campoKilos);     // Cortes del periodo
        $acumNum = sumarCampoCortesRechazos($dataPeriodo, $mapaNombres, $campoKilosRechazados);     // TotalPiezas
        ...
        foreach ($colMachines as $idx => $nombreMaquina) {
            ...
            if ($modoPorcentaje) {
                ...
                if ($den > 0) {
                    ...
                    // (1 - (Pañales / Cortes) ) *100
                    $pct = ($num / $den) * 100; // CALCULO A REVISAR  
                    $texto = number_format($pct, 2, '.', '') . '%';
                } else {
                    $texto = '';
                }
    ...
    if ($agregarAcumulado && $campoAcumulado !== '' && !empty($dataPeriodo)) {
        ...
        if ($modoPorcentaje) {
            $acumDen = sumarCampoCortesRechazos($dataPeriodo, $mapaNombres, $campoKilos);     // Cortes del periodo
            $acumNum = sumarCampoCortesRechazos($dataPeriodo, $mapaNombres, $campoKilosRechazados);     // Rechazos del periodo 
...
function pintarTiempoAbajoMaquinas(
    ...
    if ($agregarAcumulado && $campoAcumulado !== '' && !empty($dataPeriodo)) {
        ...
        foreach ($colMachines as $idx => $nombreMaquina) {
            $pdf->SetXY($x + $colOffsets[$idx + 1], $totalY + 5.5);
    ...
    if ($agregarAcumulado && $campoAcumulado !== '' && !empty($dataPeriodo)) {
        ...
        foreach ($colMachines as $idx => $nombreMaquina) {
            $pdf->SetXY($x + $colOffsets[$idx + 1], $acumY + 16);
            $num = (float) ($acumNum['porMaquina'][$nombreMaquina] ?? '');
            $den = (float) ($acumDen['porMaquina'][$nombreMaquina] ?? '');
    ...
    if ($agregarAcumulado && $campoAcumulado !== '' && !empty($dataPeriodo)) {
        ...
        foreach ($colMachines as $idx => $nombreMaquina) {
            $pdf->SetXY($x + $colOffsets[$idx + 1], $acumY + 19.5);
            $num = (float) ($acumNum['porMaquina'][$nombreMaquina] ?? '');
            $den = (float) ($acumDen['porMaquina'][$nombreMaquina] ?? '');
    ...
    if ($agregarAcumulado && $campoAcumulado !== '' && !empty($dataPeriodo)) {
        ...
        foreach ($colMachines as $idx => $nombreMaquina) {
            $pdf->SetXY($x + $colOffsets[$idx + 1], $acumY + 17);
            $num = (float) ($acumNum['porMaquina'][$nombreMaquina] ?? '');
            $den = (float) ($acumDen['porMaquina'][$nombreMaquina] ?? '');
...
function pintarCuerpoClavesMaquinas(
    ...
    $bodyY = $y + 7.5; // debajo del encabezado
    ...
    foreach ($dataCRD as $noMaq => $registros) {
        foreach ($registros as $r) {
            ...
            $rows[$clave]['porMaquina'][$nombreMaquina]['num'] += truncarDecimales($num, 3);
    ...
    foreach ($rows as $clave => $info) {
        ...
        foreach ($colMachines as $m) {
            ...
            // Truncado a 3 decimales, igual que el resto de las tablas,
            // para que este TOTAL coincida con TOTAL DIA de MM CUADRADOS/KILOGRAMOS.
            $numTrunc = truncarDecimales($num, 3);

            // Acumular totales (con el valor YA truncado, para que la suma
            // coincida exactamente con lo que se ve fila por fila)
            $totMachine[$m]['num'] += $numTrunc;
            ...
            $txt = '';
            if ($num != 0.0 || $den != 0.0) {
                $txt = number_format($numTrunc, 3) . ' / ' . number_format($den);
            }
    ...
    foreach ($colMachines as $m) {
        // Truncado a 3 decimales (no redondeo) para que coincida con TOTAL DIA
        $num = truncarDecimales((float) $totMachine[$m]['num'], 3);
        $den = (float) $totMachine[$m]['den'];
        $txt = ($num != 0.0 || $den != 0.0)
            ? number_format($num, 3) . ' / ' . number_format($den, 0)
            : '';
...
function pintarOEEMaquinas(
    ...
    if ($agregarAcumulado && $campoAcumulado !== '' && !empty($dataPeriodo)) {
        ...
        $acumHTrabajadas = sumarTiempoArribaDiaPorMaquina($dataCRD); // Horas trabajadas 
        $acumTAbajo = sumarTiempoAbajoDiaPorMaquina($dataCRD);  // Tiempo abajo
...
$texto = mb_convert_encoding("REPORTE DE PRODUCCIÓN TELAS NO TEJIDAS", 'ISO-8859-1', 'UTF-8');
...
//------------------------------------------ TABLA 1: METROS LINEALES ------------------------------------------------
$tabla1_x = $x + 3;
$tabla1_y = $endY + 13;
$pdf->SetTextColor(30, 144, 227);
$pdf->SetFont('Arial', 'B', 6);
$pdf->SetXY($tabla1_x + 10, $tabla1_y - 10);
$pdf->Write(10, mb_convert_encoding("METROS LINEALES", 'ISO-8859-1', 'UTF-8'));
...
$pdf->SetTextColor(30, 144, 227);
$pdf->SetFont('Arial', 'B', 6);
$pdf->SetXY($tabla2_x + 11, $tabla2_y - 10);
$pdf->Write(10, mb_convert_encoding("MM CUADRADOS", 'ISO-8859-1', 'UTF-8'));
pintarEncabezadoTurnos($pdf, $tabla2_x, $tabla2_y);
pintarCuerpoMetrosCuadrados(
    $pdf,
    $tabla2_x,
    $tabla2_y,
    $dataDia,
...
$pdf->SetTextColor(30, 144, 227);
$pdf->SetFont('Arial', 'B', 6);
$pdf->SetXY($tabla3_x+13, $tabla3_y - 10);
$pdf->Write(10, mb_convert_encoding("KILOGRAMOS", 'ISO-8859-1', 'UTF-8'));
pintarEncabezadoTurnos($pdf, $tabla3_x, $tabla3_y);
pintarCuerpoKilogramos(
    $pdf,
    $tabla3_x,
    $tabla3_y,
    $dataDia,
...
//------------------------------------------ TABLA 4: KILOGRAMOS RECHAZADOS ----------------------------------------------------
$tabla4_x = $x + 153;
$tabla4_y = $endY + 13;
$pdf->SetTextColor(30, 144, 227);
$pdf->SetFont('Arial', 'B', 6);
$pdf->SetXY($tabla4_x + 10, $tabla4_y - 10);
$pdf->Write(10, mb_convert_encoding("KGS RECHAZADOS", 'ISO-8859-1', 'UTF-8'));
pintarEncabezadoTurnos($pdf, $tabla4_x, $tabla4_y);
...
$pdf->SetTextColor(30, 144, 227);
$pdf->SetFont('Arial', 'B', 6);
$pdf->SetXY($tabla5_x+11, $tabla5_y - 10);
$pdf->Write(10, mb_convert_encoding("% MERMA TOTAL", 'ISO-8859-1', 'UTF-8'));
pintarEncabezadoTurnos($pdf, $tabla5_x, $tabla5_y);
pintarMermaMaquinasTotal(
    ...
    $dataCRD,
    $mapaNombres,
    'AccMLineales',             // denominador
    true,
    $dataPeriodoCR,
    'AccMLineales',            // acumulado denominador
    'Cortes Acumulados',
    0,
    'AccKG',           // numerador 
...
$pdf->SetTextColor(30, 144, 227);
$pdf->SetFont('Arial', 'B', 6);
$pdf->SetXY($tabla6_x+14, $tabla6_y - 10);
$pdf->Write(10, mb_convert_encoding("TPT (min / %)", 'ISO-8859-1', 'UTF-8'));
pintarEncabezadoTurnos($pdf, $tabla6_x, $tabla6_y);
...
//------------------------------------------ TABLA 7: HORAS TRABAJADAS ----------------------------------------------------
$tabla7_x = $x + 103;
$tabla7_y = $endY + 45;
$pdf->SetTextColor(30, 144, 227);
$pdf->SetFont('Arial', 'B', 5);
$pdf->SetXY($tabla7_x + 5, $tabla7_y - 10);
$pdf->Write(10, mb_convert_encoding("HORAS TRABAJADAS (MIN / HORAS)", 'ISO-8859-1', 'UTF-8'));
pintarEncabezadoTurnos($pdf, $tabla7_x, $tabla7_y);
...
$pdf->SetTextColor(30, 144, 227);
$pdf->SetFont('Arial', 'B', 6);
$pdf->SetXY($tabla8_x+18, $tabla8_y - 10);
$pdf->Write(10, mb_convert_encoding("OEE", 'ISO-8859-1', 'UTF-8'));
pintarEncabezadoTurnos($pdf, $tabla8_x, $tabla8_y );
pintarOEEMaquinas(
    $pdf,
    $tabla8_x,
    $tabla8_y - 4,
    ...
    true,
    $dataPeriodoCR,
    'AccKG',            // acumulado denominador
    'AccMLineales Acumulados',
    0,
    'Rechazos',           // numerador 
    1,                  // decimales del porcentaje
    $dataPeriodo,
...
$pdf->SetTextColor(30, 144, 227);
$pdf->SetFont('Arial', 'B', 6);
$pdf->SetXY($tabla9_x + 35, $tabla9_y - 10);
$pdf->Write(10, mb_convert_encoding("CLAVES DE PRODUCCIÓN (MM2 / KGS)", 'ISO-8859-1', 'UTF-8'));
pintarEncabezadoClaves($pdf, $tabla9_x, $tabla9_y);
...
$dataPlanProduccion = $departamentosObj->getInfoPlanProduccionTNT($fechai, $fechaf);
function planStyles(): array
{
    return [
        ...
        'labels' => [
            ...
            'colHeaders' => ['CLAVE', 'DESCRIPCIÓN', 'MMC', 'KGS', 'PROGRAMA', 'DIF'],
        ]
    ];
}
...
function agregarFilasPorClave(array $list): array
{
    ...
    foreach ($list as $r) {
        ...
        // Sumar métricas
        $byClave[$clave]['TotalMMC'] += (float) ($r['TotalMMC'] ?? 0);
        $byClave[$clave]['TotalKilos'] += (float) ($r['TotalKilos'] ?? 0);
        $byClave[$clave]['PlanProduccion'] += (float) ($r['PlanProduccion'] ?? 0);
        $byClave[$clave]['DiferenciaMMC'] += (float) ($r['DiferenciaMMC'] ?? 0);
    }
    ...
    foreach ($dataPlanProduccion as $idProducto => $rows) {
        ...
        foreach ($rows as $r) {
            ...
            $porEtapa[$etapa][] = [
                'Clave' => (string) ($r['Clave'] ?? ''),
                'Descripcion' => (string) ($r['Descripcion'] ?? ''),
                'TotalMMC' => (float) ($r['TotalMMC'] ?? 0),
                'TotalKilos' => (float) ($r['TotalKilos'] ?? 0),
                'PlanProduccion' => (float) ($r['PlanProduccion'] ?? 0),
                'DiferenciaMMC' => (float) ($r['DiferenciaMMC'] ?? 0),
            ];
        }

        // 2) DENTRO DE CADA ETAPA, TOTALIZAMOS POR CLAVE
        $totalesProducto = ['TotalMMC' => 0, 'TotalKilos' => 0, 'PlanProduccion' => 0, 'DiferenciaMMC' => 0];
        $etapasList = [];
        foreach ($porEtapa as $etapa => $listRaw) {
            ...
            $t = ['TotalMMC' => 0, 'TotalKilos' => 0, 'PlanProduccion' => 0, 'DiferenciaMMC' => 0];
            foreach ($rowsAgg as $r) {
                $t['TotalMMC'] += $r['TotalMMC'];
                $t['TotalKilos'] += $r['TotalKilos'];
                $t['PlanProduccion'] += $r['PlanProduccion'];
                $t['DiferenciaMMC'] += $r['DiferenciaMMC'];
            }

            // Acumular al total de producto
            $totalesProducto['TotalMMC'] += $t['TotalMMC'];
            $totalesProducto['TotalKilos'] += $t['TotalKilos'];
            $totalesProducto['PlanProduccion'] += $t['PlanProduccion'];
            $totalesProducto['DiferenciaMMC'] += $t['DiferenciaMMC'];
...
function calcularTotalCategoria(array $bloques): array
{
    ...
    foreach ($bloques as $bloque) {
        $total['TotalMMC'] += $bloque['totales']['TotalMMC'];
        $total['TotalKilos'] += $bloque['totales']['TotalKilos'];
        $total['PlanProduccion'] += $bloque['totales']['PlanProduccion'];
        $total['DiferenciaMMC'] += $bloque['totales']['DiferenciaMMC'];
    }
    return $total;
}
...
function dibujarBloquePlan(FPDF $pdf, float $x, float $y, float $w, array $bloque, array $st): float
{
    ...
    foreach ($bloque['etapas'] as $etapa) {
        ...
        foreach ($etapa['rows'] as $r) {
            ...
            $pdf->Cell($colsW[2], $rowH, number_format((float) $r['TotalMMC'], 3), 1, 0, 'C');
            $pdf->Cell($colsW[3], $rowH, number_format(round((float) $r['TotalKilos']), 0), 1, 0, 'C');
            $pdf->Cell($colsW[4], $rowH, number_format((float) $r['PlanProduccion'], 3), 1, 0, 'C');
            $pdf->Cell($colsW[5], $rowH, number_format((float) $r['DiferenciaMMC'], 3), 1, 0, 'C');
            $pdf->Ln();
            $yActual += $rowH;
        }
        ...
        $pdf->Cell($colsW[2], $rowH, number_format($t['TotalMMC'], 3), 1, 0, 'C', true);
        $pdf->Cell($colsW[3], $rowH, number_format(round($t['TotalKilos']), 0), 1, 0, 'C', true);
        $pdf->Cell($colsW[4], $rowH, number_format($t['PlanProduccion'], 3), 1, 0, 'C', true);
        $pdf->Cell($colsW[5], $rowH, number_format($t['DiferenciaMMC'], 3), 1, 0, 'C', true);
        $pdf->Ln();
        $yActual += $rowH;
...
function dibujarTotalCategoria(FPDF $pdf, float $x, float $y, float $w, array $total, array $st, string $categoria): float
{
    ...
    $pdf->SetFillColor(255, 255, 255);
    $pdf->Cell($colsW[2], $rowH - 2, number_format($total['TotalMMC'], 3), 1, 0, 'C', true);
    $pdf->Cell($colsW[3], $rowH - 2, number_format(round($total['TotalKilos']), 0), 1, 0, 'C', true);
    $pdf->Cell($colsW[4], $rowH - 2, number_format($total['PlanProduccion'], 3), 1, 0, 'C', true);
    $pdf->Cell($colsW[5], $rowH - 2, number_format($total['DiferenciaMMC'], 3), 1, 0, 'C', true);
    $pdf->Ln();
    return $rowH + 2;
}
...
function dibujarTotalGeneral(FPDF $pdf, float $x, float $y, float $w, array $total, array $st): float
{
    ...
    $pdf->Cell($colsW[2], $rowH - 2, 'MMC', 1, 0, 'C', true);
    $pdf->Cell($colsW[3], $rowH - 2, 'KGS', 1, 0, 'C', true);
