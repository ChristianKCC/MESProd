...
{
    // Marco de la tabla (opcional, ajusta alto si cambias filas)
    // $pdf->SetDrawColor(30, 144, 227);
    // $pdf->Rect($x - 3, $y - 2, 102, 0); // (x, y, width, height)
    ...
    // Encabezados
    $pdf->SetXY($x - 3, $y - 2);
    $pdf->Cell(15, 3.5, 'TURNO', 1, 0, 'C', true);

    $labels = ['PA01', 'PA02', 'PA03', 'PA04', 'PA05', 'Total Turno'];
    $offsets = [12, 24, 36, 48, 60, 72];
    $widths = [12, 12, 12, 12, 12, 15];
...
function pintarEncabezadoClaves(FPDF $pdf, float $x, float $y): void
{
    ...
    $labels = ['DESCRIPCION', 'PA01', 'PA02', 'PA03', 'PA04', 'PA05', 'Total Turno'];
    $offsets = [-8, 42, 57, 72, 87, 102, 117];
...
{
    $colMachines = ['PA01', 'PA02', 'PA03', 'PA04', 'PA05'];
...
function pintarCuerpoTurnos(
    ...
    $colOffsets = [-3, 12, 24, 36, 48, 60, 72];
    $colW = [15, 12, 12, 12, 12, 12, 15];
    $colMachines = ['PA01', 'PA02', 'PA03', 'PA04', 'PA05'];
    ...
    for ($i = 0; $i < 3; $i++) {
        ...
        $pdf->SetXY($x + 72, $currY - 2);
        $pdf->Cell($colW[6], $rowHeight, number_format($totalTurno, 2), 1, 0, 'C');
    ...
    $pdf->SetXY($x + 72, $totalY - 2);
    $pdf->Cell($colW[6], $rowHeight, number_format($totalGeneral), 1, 0, 'C', true);
    ...
    if ($agregarAcumulado && $campoAcumulado !== '' && !empty($dataPeriodo)) {
        ...
        $pdf->SetXY($x + 72, $acumY - 2);
        $pdf->Cell($colW[6], $rowHeight, number_format($acum['totalGeneral']), 1, 0, 'C', true);
    }
...
function pintarCuerpoReales(
    ...
    for ($i = 0; $i < 3; $i++) {
        ...
        $pdf->Cell($colW[6], $rowHeight, number_format($trunc2, 2, '.', ''), 1, 0, 'C');
    ...
    $pdf->Cell($colW[6], $rowHeight, number_format($valorTruncado, 2), 1, 0, 'C', true);
    ...
    if ($agregarAcumulado && $campoAcumulado !== '' && !empty($dataPeriodo)) {
        ...
        $pdf->Cell($colW[6], $rowHeight, number_format($acum['totalGeneral'], 2), 1, 0, 'C', true);
    }
...
function pintarCuerpoCortesRechazos(
    ...
    for ($i = 0; $i < 3; $i++) {
        ...
        $pdf->SetXY($x + 72, $currY);
        $pdf->Cell($colW[6], $rowHeight, number_format($totalTurno), 1, 0, 'C');
    ...
    if ($agregarAcumulado && $campoAcumulado !== '' && !empty($dataPeriodo)) {
        ...
        $pdf->SetXY($x + 72, $acumY);
        $pdf->Cell($colW[6], $rowHeight, number_format($acum['totalGeneral']), 1, 0, 'R', true);
    }
...
function pintarCuerpoMermaMaquinas(
    ...
    for ($i = 0; $i < 3; $i++) {
        ...
        if ($modoPorcentaje) {
            ...
            $pdf->Cell($colW[6], $rowHeight, $textoTotal, 1, 0, 'C');
        } else {
            $pdf->Cell($colW[6], $rowHeight, number_format($totalTurnoSimple), 1, 0, 'R');
        }
    }
    ...
    if ($modoPorcentaje) {
        ...
        $pdf->Cell($colW[6], $rowHeight, number_format($pctG, 2) . '%', 1, 0, 'C', true);
    } else {
        $pdf->Cell($colW[6], $rowHeight, number_format($totalGeneral), 1, 0, 'C', true);
    }
    ...
    if ($agregarAcumulado && $campoAcumulado !== '' && !empty($dataPeriodo)) {
        ...
        if ($modoPorcentaje) {
            ...
            $pdf->SetXY($x + 72, $acumY);
            $pdf->Cell($colW[6], $rowHeight, number_format((float) $acum['totalGeneral']), 1, 0, 'R', true);
        }
    }
...
function pintarMermaMaquinasTotal(
    ...
    if ($agregarAcumulado && $campoAcumulado !== '' && !empty($dataPeriodo)) {
        ...
        $pdf->Cell($colW[6], $rowHeight, $numGTexto, 1, 0, 'C', true);
    ...
    if ($agregarAcumulado && $campoAcumulado !== '' && !empty($dataPeriodo)) {
        ...
        if ($modoPorcentaje) {
            ...
            $pdf->SetXY($x + 72, $acumY);
            $pdf->Cell($colW[6], $rowHeight, $acum, 1, 0, 'R', true);
        }
    }
...
function pintarTiemposMaquinas(
    ...
    for ($i = 0; $i < 3; $i++) {
        ...
        $pdf->SetXY($x + 72, $currY + 6);
        $pdf->Cell($colW[6], $rowHeight, $valorSTRTotal, 1, 0, 'C');
    ...
    // ---------------- Fila acumulado de TiempoArriba (en minutos) ----------------
    {
        ...
        // Etiqueta de la fila
        $pdf->SetXY($x - 3, $acumY);
        $pdf->MultiCell($colW[0], $rowHeight, 'T. ARRIBA DIA', 1, 'C', true);
    ...
    if ($agregarAcumulado && $campoAcumulado !== '' && !empty($dataPeriodo)) {
        ...
        $pdf->SetXY($x + 72, $acumY + $rowHeight);
        $numG = (float) ($acumNum['totalGeneral'] ?? '');
...
function pintarTiempoAbajoMaquinas(
    ...
    for ($i = 0; $i < 3; $i++) {
        ...
        foreach ($colMachines as $idxHM => $nombreMaquinaHM) {
            if (isset($mapaNombres[$nombreMaquinaHM])) {
                ...
                // Asegúrate de que $dataCRD[$noMaqHM][$i] exista y tenga HorasTrabajadas
        ...
        $valortTotal = ($minutosTrabajadosTurno > 0)
            ...
            : '';
        $pdf->Cell($colW[6], $rowHeight, number_format($totalTurno, 0) . ' / ' . $valortTotal, 1, 0, 'C');
    ...
    $pdf->SetXY($x + 72, $totalY + 6);
    $minG = (float) (number_format($acumArriba['totalGeneral'], 2) ?? 0);
    $tppGeneral = ($minG > 0) ? number_format(($totalGeneral / ($minG * 60)) * 100, 2) : 0;
    $pdf->Cell($colW[6], $rowHeight, number_format($minG, 2) . ' / ' . $tppGeneral . '%', 1, 0, 'C', true);
    ...
    if ($agregarAcumulado && $campoAcumulado !== '' && !empty($dataPeriodo)) {
        ...
        // MultiCell estándar de FPDF: (w, h, txt, border, align, fill)
        $pdf->MultiCell($colW[0], $rowHeight, 'TP ACUMULADO', 1, 'C', true);
...
function pintarCuerpoClavesMaquinas(
    ...
    // labels = ['DESCRIPCION','PA01','PA02','PA03','PA04','PA05'];
...
function pintarMermaEmpaque(
    ...
    for ($i = 0; $i < 3; $i++) {
        ...
        $pdf->SetXY($x + 72, $currY);
        $pdf->Cell($colW[6], $rowHeight, $valorSTRTotalTurno, 1, 0, 'C');
    ...
    $pdf->SetXY($x - 3, $acumY);
    $pdf->Cell($colW[0], $rowHeight, 'TOTAL PIEZAS DIA', 1, 0, 'C', true);
    ...
    $pdf->Cell($colW[6], $rowHeight, $texto, 1, 0, 'C', true);
...
function pintarOEEMaquinas(
    ...
    $pdf->Cell($colW[6], $rowHeight, number_format($oeeTD, 2) . '%', 1, 0, 'C', true);
...
$texto = mb_convert_encoding("REPORTE DE PRODUCCIÓN INCONTINENCIA", 'ISO-8859-1', 'UTF-8');
...
//------------------------------------------ TABLA 2: REALES ----------------------------------------------------
// Tu segunda tabla la dibujabas desplazada a la derecha +103 en X
$tabla2_x = $x + 106;
$tabla2_y = $endY + 13;
$pdf->SetTextColor(30, 144, 227);
$pdf->SetFont('Arial', 'B', 8);
$pdf->SetXY($tabla2_x + 35, $tabla2_y - 10);
...
$pdf->SetTextColor(30, 144, 227);
$pdf->SetFont('Arial', 'B', 8);
$pdf->SetXY($tabla3_x + 35, $tabla3_y - 12);
...
$pdf->SetTextColor(30, 144, 227);
$pdf->SetFont('Arial', 'B', 8);
$pdf->SetXY($tabla4_x + 33, $tabla4_y - 12);
...
$pdf->SetTextColor(30, 144, 227);
$pdf->SetFont('Arial', 'B', 8);
$pdf->SetXY($tabla5_x + 25, $tabla5_y - 12);
...
$pdf->SetTextColor(30, 144, 227);
$pdf->SetFont('Arial', 'B', 8);
$pdf->SetXY($tabla6_x + 28, $tabla6_y - 12);
...
$pdf->SetTextColor(30, 144, 227);
$pdf->SetFont('Arial', 'B', 8);
$pdf->SetXY($tabla8_x + 30, $tabla8_y - 12);
...
$pdf->SetTextColor(30, 144, 227);
$pdf->SetFont('Arial', 'B', 8);
$pdf->SetXY($tabla7_x + 10, $tabla7_y - 12);
...
$pdf->SetTextColor(30, 144, 227);
$pdf->SetFont('Arial', 'B', 8);
$pdf->SetXY($tabla9_x + 10, $tabla9_y - 12);
...
$pdf->SetTextColor(30, 144, 227);
$pdf->SetFont('Arial', 'B', 8);
$pdf->SetXY($tabla9_x + 42, $tabla9_y - 14);
