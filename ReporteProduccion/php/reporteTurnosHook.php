...
$turnosObj = new TurnosHook();
$data = $turnosObj->generarDatosAnterioresHook($id);
...
$pdf->MultiCell(40, 5, $fecha, 1, 'C'); //fecha
$endY = $pdf->GetY();
...
if (!empty($data) && is_array($data)) {
    ...
    if ($targetTurno !== null) {
        foreach ($data as $row) {
            if (
                ...
                && isset($row['EmpleadoNombreAsistencia']) && $row['EmpleadoNombreAsistencia'] !== null
                && trim((string) $row['EmpleadoNombreAsistencia']) !== ''
            ) {
                $nombreConductor = mb_convert_encoding($row['EmpleadoNombreAsistencia'], 'ISO-8859-1', 'UTF-8');
                break;
            }
        }
    }
    ...
    if ($nombreConductor === 'N/A') {
        foreach ($data as $row) {
            if (isset($row['EmpleadoNombreAsistencia']) && $row['EmpleadoNombreAsistencia'] !== null && trim((string) $row['EmpleadoNombreAsistencia']) !== '') {
                $nombreConductor = mb_convert_encoding($row['EmpleadoNombreAsistencia'], 'ISO-8859-1', 'UTF-8');
                break;
            }
        }
...
//-----------------------------------------------------------------------------------------------------------------
$pdf->SetXY($x + 8, $y + 52);
$pdf->Cell(12, 5, 'Turno', 1, 1, 'C');
//-----------------------------------------------------------------------------------------------------------------
$pdf->SetXY($x + 20, $y + 52);
$pdf->Cell(16, 5, 'MM Lineales', 1, 1, 'C');
//-----------------------------------------------------------------------------------------------------------------
$pdf->SetXY($x + 36, $y + 52);
$pdf->MultiCell(16, 2.5, mb_convert_encoding('MML Entregados', 'ISO-8859-1', 'UTF-8'), 1, 'C');
//-----------------------------------------------------------------------------------------------------------------
$pdf->SetXY($x + 52, $y + 52);
$pdf->MultiCell(17, 2.5, mb_convert_encoding('MML Rechazados', 'ISO-8859-1', 'UTF-8'), 1, 'C');
// //-----------------------------------------------------------------------------------------------------------------
$pdf->SetXY($x + 69, $y + 52);
$pdf->MultiCell(11, 5, mb_convert_encoding('# Paros', 'ISO-8859-1', 'UTF-8'), 1, 'C');
// //-----------------------------------------------------------------------------------------------------------------
$pdf->SetXY($x + 80, $y + 52);
$pdf->MultiCell(18, 5, mb_convert_encoding('Tiempo abajo', 'ISO-8859-1', 'UTF-8'), 1, 'C');
// //-----------------------------------------------------------------------------------------------------------------
...
if ($data[0]['Turno'] == 1) {
    ...
    //Cortes
    $pdf->SetXY($x + 26, $y + 60);
    $pdf->MultiCell(18, 7, number_format($data[0]['MetrosLineales'] / 1000, 3), 0, 'C');
    //Cajas reales
    $pdf->SetXY($x + 44, $y + 60);
    $pdf->MultiCell(20, 7, round($data[0]['MetrosEntregados'] / 1000, 3), 0, 'C');
    //Rechazos
    $pdf->SetXY($x + 64, $y + 60);
    $pdf->MultiCell(18, 7, number_format($data[0]['MetrosRechazados'] / 1000, 3), 0, 'C');
    //Número de paros
    $pdf->SetXY($x + 82, $y + 60);
    ...
    $tiempoAbajo = $data[0]['TiempoAbajo'] ?? 0;
    ...
    $mermaTotal = 0;
    $cortesA = isset($data[0]['CortesA']) ? (float) $data[0]['CortesA'] : 0;
    // $totalPanalA = isset($data[0]['TotalGeneralPañales']) ? (float) $data[0]['TotalGeneralPañales'] : 0;
    if ($cortesA != 0) {
        $mermaTotal = (1 - ($totalPanalA / $cortesA)) * 100;
    }
    // $pdf->MultiCell(18, 7, number_format($mermaTotal, 2) . '%', 0, 'C');
...
if (isset($data[1]) && (($data[1]['Turno'] ?? null) == 2)) {
    ...
    $c1 = $data[1]['MetrosLineales'] ?? 0;
    $r1 = $data[1]['MetrosRechazados'] ?? 0;
    $p1 = $data[1]['NoParosMaquinaA'] ?? 0;
    $t1_raw = $data[1]['TiempoAbajo'] ?? 0;
    $noMaquina1 = $data[1]['NoMaquina'] ?? null;
    // if (is_numeric($t1_raw) && in_array((int) $noMaquina1, [60, 61, 62, 63], true)) {
    ...
    //Cortes
    $pdf->SetXY($x + 26, $y + 67);
    $pdf->MultiCell(18, 7, number_format($c1 / 1000, 3), 0, 'C');
    //Cajas reales
    $pdf->SetXY($x + 44, $y + 67);
    $pdf->MultiCell(20, 7, number_format($data[1]['MetrosEntregados'] / 1000, 3), 0, 'C');
    //Rechazos
    $pdf->SetXY($x + 64, $y + 67);
    $pdf->MultiCell(18, 7, number_format($r1 / 1000, 3), 0, 'C');
    //Número de paros
    $pdf->SetXY($x + 82, $y + 67);
    ...
    $mermaTotal1 = ($c1 != 0) ? number_format((1 - ($m1 / $c1)) * 100, 2) . '%' : '0.00%';
    $pdf->MultiCell(18, 7, $mermaTotal1, 0, 'C');
...
//     //Cajas reales
//     $pdf->SetXY($x + 44, $y + 74);
//     $pdf->MultiCell(20, 7, round($m2), 0, 'C');
//     // Rechazos
//     $pdf->SetXY($x + 64, $y + 74);
...
//     $pdf->MultiCell(18, 7, number_format($c2 != 0 ? (1 - $m2 / $c2) * 100 : 0, 2) . '%', 0, 'C');
//     // Porcentaje de tiempo perdido
...
// $pdf->setXY($x + 44, $y + 81);
// $pdf->Cell(20, 7, round($totalPañales), 0, 0, 'C');
// $pdf->setXY($x + 64, $y + 81);
...
//         $tiempoArribaRaw = $r['HorasTrabajadas'] ?? 0;
//         $noMaquina = $r['NoMaquina'] ?? null;
//         // if (is_numeric($tiempoArribaRaw) && in_array((int) $noMaquina, [60, 61, 62, 63, 64], true)) {
//         $tiempoArriba = (float) $tiempoArribaRaw;
//         // } else {
//         //     $tiempoArriba = is_numeric($tiempoArribaRaw) ? (float) $tiempoArribaRaw : 0;
//         // }
//         //    #paros*tiempo operado/(tiempo operado-tiempo perdido)
//         if ($tiempoArriba != 0) {
//             $denominador = ($r['HorasTrabajadas'] * 60) - $r['TiempoAbajo'];
//             if ($denominador != 0) {
//                 $seccionesAgrupadas[$seccion][$turno]['sato'] = ($r['TotalPorSeccion'] * ($r['HorasTrabajadas'] * 60)) / $denominador;
//             }
//         }
//     }
//     $tiempoArribaTotal += $tiempoArriba ?? 0;
//     $tiempoAbajoRaw = $r['TiempoAbajo'] ?? 0;
//     $noMaquina = $r['NoMaquina'] ?? null;
//     // if (is_numeric($tiempoAbajoRaw) && in_array((int) $noMaquina, [60, 61, 62, 63, 64], true)) {
//     //     $tiempoAbajoTotal += (float) $tiempoAbajoRaw / 60;
//     // } else {
//     $tiempoAbajoTotal += is_numeric($tiempoAbajoRaw) ? (float) $tiempoAbajoRaw : 0;
//     // }
// }
...
//         $totalParos += $paros;
//     }
//     // #paros*(HorasTrabajadas(minutos)/(HorasTrabajadas(minutos)-tiempo perdido)
//     // Sato total por sección basado en totalParos y tiempoArribaTotal (evitar div/0)
//     $denominador = ($tiempoArribaTotal * 60) - $tiempoAbajoTotal;
//     $totalSato = $denominador > 0 ? ($totalParos * ($tiempoArribaTotal * 60)) / $denominador : 0;
...
//     // Sato = TotalSeccionnes *  HorasTrabajadas(minutos) / (HorasTrabajadas(minutos) - TiempoAbajoTotal)
