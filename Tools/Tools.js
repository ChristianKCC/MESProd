export class Toolsjs {
  ...
  exportartablaexcelVacaciones(tableID) {
    var uri = "data:application/vnd.ms-excel;base64,",
      template =
    ...
    // Clonar la tabla para manipularla sin afectar la vista
    var clone = table.cloneNode(true);

    // Reemplazar los números por el tipo de día
    clone.querySelectorAll("span").forEach(span => {
        // El tipo esta en un atributo y en el color, aquí se guarda en data-tipo
        const tipo = span.getAttribute("data-tipo");
        if (tipo) {
            span.textContent = tipo; // mostrar V, D, F, R en Excel
        }
    });

    var ctx = { worksheet: "Vacaciones", table: clone.innerHTML };
    window.location.href = uri + base64(format(template, ctx));
}
