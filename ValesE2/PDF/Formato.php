...
function getdatavale($folio)
{
    ...
    while ($row = sqlsrv_fetch_array($result)) {
        array_push($array, [
            'id' => $row['id'], 'maquina' => $row['maquina'], 'noemp' => $row['noemp'], 'nombre' => $row['Nombre'], 'turno' => $row['turno'], 'clave1' => $row['clave1'],
            'clave2' => $row['clave2'], 'clave3' => $row['clave3'], 'clave4' => $row['clave4'], 'estado' => $row['estado'],
            'fechac' => $row['fecha']->format('Y-m-d H:i:s'), 'fechae' => $row['fechaenviado']->format('Y-m-d H:i:s'), 'Puesto' => $row['puesto'], 'foliocons' => $row['foliocons'],
            'noempsup' => $row['noempsup'], 'nombresup' => $row['nombresup'], 'puestosup' => $row['puestosup']
        ]);
    }
    return $array;
    sqlsrv_close($conn);
...
function getmateriales($folio)
{
    ...
    $query = "SELECT  tblValeEMateriales.*,tblValeEMaterialesAdd.id as folio,tblValeEMaterialesAdd.cantidad,tblValeEMaterialesAdd.estado as estadomat FROM tblValeEMaterialesAdd 
            INNER JOIN tblValeEMateriales ON tblValeEMateriales.NoMaterial = tblValeEMaterialesAdd.idMaterial WHERE tblValeEMaterialesAdd.idValeEnc=$folio ORDER BY tblValeEMateriales.TipoMontacargas";
    $result = sqlsrv_query($conn, $query);
    $array = array();
    while ($row = sqlsrv_fetch_array($result)) {
        array_push($array, [
            'folio' => $row['folio'], 'NoMaterial' => $row['NoMaterial'], 'NombreMaterial' => $row['NombreMaterial'], 'CentroCosto' => $row['CentroCosto'],
            'TiempoMaterial' => $row['TiempoMaterial'], 'TipoMontacargas' => $row['TipoMontacargas'], 'Cantidad' => $row['cantidad'], 'UM' => $row['UM'],
...
foreach ($arraymat as $element) {
    ...
    $pdf->SetX(190);
    $elemento = '';
    if ($element['estadomat'] == 1) $elemento = 'Solicitado';
    else if ($element['estadomat'] == 2) $elemento = 'No disponible';
    else if ($element['estadomat'] == 3) $elemento = 'Envio MP';
