...
confClave.tblmateriales('').then(element => document.getElementById('tblmateriales').innerHTML = element);
document.getElementById('buscarmaterial').addEventListener('click', (e) => {
    ...
    confClave.tblmateriales(busqueda).then(element => document.getElementById('tblmateriales').innerHTML = element);
})
document.getElementById('savechgmateriales').addEventListener('click', function (e) {
    ...
    confClave.saveMaterial(idclase, nomaterial, nombrematerial, ummaterial, ummat, montacargas, '1', tiempo).then(() => {
        confClave.tblmateriales(busqueda).then(element => document.getElementById('tblmateriales').innerHTML = element);
        modal.hide();
    });
})
...
window.editarmaterialbtn = (param) => {
    confClave.editarMaterial(param, 'idmaterial', 'nomaterial', 'nombrematerial', 'ummaterial'
