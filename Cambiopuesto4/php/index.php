...
class Cambiopuesto
{
    ...
    function abrircambiopuesto()
    {
        ...
        $query = "INSERT INTO CambiopuestoEnc(
              ...
              VALUES (?, ?, GETDATE(), ?, ?, ?);
              SELECT SCOPE_IDENTITY() AS id;";
        $params = [$supervisor, $fechainicio, $GerNum, $nosemana, $Superint];
    ...
    */
    ...
    function tblenc()
    {
        ...
        $query = "SELECT CambiopuestoEnc.*,
                    ...
                    WHERE CambiopuestoEnc.supervisor='" . $_SESSION['ibm'] . 
    ...
    //             INNER JOIN TLX003MXDB.dbo.CambiopuestoSubEnc CamPSub ON CamPSub.folio = CamPE.id
    //             INNER JOIN TLX032MXDB.dbo.tblEmpleados eSup ON eSup.NoEmp = CamPE.supervisor
    //             WHERE CamPE.supervisor='" . $_SESSION['ibm'] . "' 
    //             GROUP BY CamPE.id, CamPE.supervisor, eSup.Nombre, CamPE.estadoTer , CamPE.fecha, CamPE.terminado
...
class Reportes
{
    ...
    function reportegenral()
    {
        ...
        $query = "SELECT 
                ...
                FROM CambiopuestoSubEnc INNER JOIN CambiopuestoEnc ON CambiopuestoEnc.id= CambiopuestoSubEnc.folio
                INNER JOIN TLX032MXDB.dbo.tblEmpleados ON tblEmpleados.NoEmp = CambiopuestoSubEnc.noemp
                INNER JOIN TLX009MXDB.dbo.tblMaquinas ON tblMaquinas.NoMaquina= CambiopuestoSubEnc.maquina
