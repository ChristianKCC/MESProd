...
const CATALOGO_TURNOS = {
    ...
    mixto1: { nombre: "Mixto 1 (07:30 - 17:00)", entrada: { ideal: 7*60+30, tolerancia: 35 }, salida: { ideal: 17*60, tolerancia: 60 }, cruzaMedia: false, horasReglamentarias: "10:00:00", sabado: { nombre: "Mixto 1 Sábado", salida: { ideal: 12*60+30, tolerancia: 60 }, horasReglamentarias: "05:00:00" } },
    mixto2: { nombre: "Mixto 2 (08:30 - 18:30)", entrada: { ideal: 8*60+30, tolerancia: 35 }, salida: { ideal: 18*60+30, tolerancia: 60 }, cruzaMedia: false, horasReglamentarias: "10:00:00", sabado: { nombre: "Mixto 2 Sábado", salida: { ideal: 13*60+30, tolerancia: 60 }, horasReglamentarias: "05:00:00" } },
    mixto3: { nombre: "Mixto 3 (07:00 - 16:30)", entrada: { ideal: 7*60, tolerancia: 35 }, salida: { ideal: 16*60+30, tolerancia: 60 }, cruzaMedia: false, horasReglamentarias: "09:30:00", sabado: { nombre: "Mixto 3 Sábado", salida: { ideal: 12*60, tolerancia: 60 }, horasReglamentarias: "05:00:00" } },
    mixto4: { nombre: "Mixto 4 (07:00 - 17:00)", entrada: { ideal: 7*60, tolerancia: 15 }, salida: { ideal: 17*60, tolerancia: 60 }, cruzaMedia: false, horasReglamentarias: "10:00:00", sabado: { nombre: "Mixto 4 Sábado", salida: { ideal: 12*60, tolerancia: 60 }, horasReglamentarias: "05:00:00" } }
};
...
// Toma primer y último registro del día directamente (lógica de lapso).
// ─────────────────────────────────────────────────────────────────────────────
function resolverLapsoDiurno(regActual) {
    if (regActual.length < 2) return null;
...
// Calcula entrada/salida del checador, resta horas reglamentarias,
// el excedente (≥ 55 min) es el tiempo extra.
// HoraFin   = última hora real | HoraInicio = HoraFin - excedente
// ─────────────────────────────────────────────────────────────────────────────
async function procesarPorLapso(noemp, date, folioRegistro, turnoAsignado, tipoEmpleado, datosRegistro, mostrarError) {
    ...
    if (checador.regActual.length < 2) {
        mostrarError(
            `Se necesitan al menos 2 registros en el checador del día ${date} para validar el turno <b>${turnoAsignado}</b>.`,
    ...
    console.log(`Entrada: ${primerReg.hora_limpia} | Salida: ${ultimoReg.hora_limpia} | Trabajado: ${minutosTrabajados}min | Excedente: ${excedente}min`);
    ...
    if (minutosTrabajados < (hrsReglMin - 5)) {
        ...
        await Swal.fire({
            ...
            html:  `El empleado trabajó <b>${minutosAHoraConSegundos(minutosTrabajados)}</b> pero el turno requiere <b>${hrsReglDef}</b>.<br><br>
                    ...
                    Primer registro (${checador.fechaActual}): <b>${primerReg.hora_limpia}</b><br>
                    Último registro: <b>${ultimoReg.hora_limpia}</b><br>
...
async function procesarDescansoOFestivo(noemp, date, folioRegistro, turnoAsignado, motivo, tipoEmpleado, datosRegistro, mostrarError) {
    ...
    if (esMixtoFestivo) {
        ...
        if (checador.regActual.length < 2) { mostrarError(`Se necesitan al menos 2 registros en el checador del día ${date}.`, { registrosHoy: regHoyStr, registrosAyer: regAyerStr }); return; }
        resolucion = { entrada: checador.regActual[0], salida: checador.regActual[checador.regActual.length - 1], escenario: "lapso_directo" };
    }
...
async function procesarAnticipo(noemp, date, folioRegistro, turnoAsignado, tipoEmpleado, datosRegistro, mostrarError) {
    ...
    if (hrsExtraRealesMin < 50) {
        window.Reportes.marcarRegistroComoNoApto(folioRegistro);
        await Swal.fire({ title: "Anticipo — No apto (excedente insuficiente)", html: `Excedente real (<b>${minutosAHoraConSegundos(hrsExtraRealesMin)}</b>) no alcanza 50 min mínimos.<br><br>Turno: <b>${turnoAsignado}</b> (regl.: <b>${def.horasReglamentarias}</b>)<br>Primer registro (${checador.fechaActual}): <b>${primerReg.hora_limpia}</b><br>Último: <b>${ultimoReg.hora_limpia}</b><br>Total: <b>${minutosAHoraConSegundos(totalTrabajadoMin)}</b><br>Tipo: <b>${tipoEmpleado}</b>`, icon: "warning" });
        const fid = window.Reportes.data.find(e => e.folioRegistro == folioRegistro)?.id; if (fid) window.Reportes.evaluarFolio(fid); return;
    }
