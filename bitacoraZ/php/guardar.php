...
function out($a)
{
    ...
    if (is_string($stray) && trim($stray) !== '')
        logwr('Salida inesperada antes del JSON: ' . $stray);
    if (!headers_sent())
...
function upsertTabla($conn, $tabla, $folio, array $cols, array $keyCols, array $rows)
{
    ...
    if ($sel === false)
        throw new Exception("SELECT $tabla: " . errs());
    ...
    foreach ($rows as $rrow) {
        ...
        if (array_key_exists($kstr, $existentes)) {
            ...
            if ($st === false)
                throw new Exception("UPDATE $tabla: " . errs());
            // Liberar stmt para siguuiente transaccion
            sqlsrv_free_stmt($st);
        }
        ...
        // INSERT en caso de ser un folio nuevo
            ...
            if ($st === false)
                throw new Exception("INSERT $tabla: " . errs());
            // Liberar stmt para siguuiente transaccion
            sqlsrv_free_stmt($st);
        }
        $n++;
    ...
    foreach ($existentes as $kstr => $id) {
        if (!isset($usados[$kstr])) {
            ...
            if ($st === false)
                throw new Exception("DELETE $tabla: " . errs());
            // Liberar stmt para siguuiente transaccion
            sqlsrv_free_stmt($st);
        }
    }
...
if ($chk === false) {
    logwr('Error verificando folio: ' . errs());
    out(['ok' => false, 'error' => 'No se pudo verificar el folio: ' . errs()]);
}
...
/* ---- Merma máquinas (kilos), se alimenta del Peso de Bolsas ---- */
$r = [];
...
/* ---- Pacas merma (ahora en pestaña 1). Incluye 100% Toalla como concepto ---- */
$r = [];
foreach (['BASURA', 'RECORTE', 'TOALLA'] as $cpt) {
    $kilos = numv($P['wr_pacasmerma'][$cpt][$tShort]['kilos'] ?? null);
    $pacas = numv($P['wr_pacasmerma'][$cpt][$tShort]['pacas'] ?? null);
    if ($kilos !== null || $pacas !== null)
        $r[] = ['Concepto' => $cpt, 'Kilos' => $kilos, 'Pacas' => $pacas];
}
...
foreach (array_keys($PESO_TIPOS) as $tipo) {
    ...
    foreach ($arr as $idx => $celda) {
        $val = cellNum($celda);
        if ($val !== null)
            $r[] = ['Tipo' => $tipo, 'NoFila' => $idx + 1, 'Peso' => $val];
    }
}
