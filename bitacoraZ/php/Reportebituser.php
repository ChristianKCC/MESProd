	...
	function tblctrltiemposbitrep()
	{
		...
		while ($row = sqlsrv_fetch_array($result)) {
			array_push($array, [
				...
				"correccion" => $row[15],
				"turno" => $row["Turno"],
				"fechah" => $row["fechah"]->format('Y-m-d')
			]);
		}
		echo json_encode($array);
	}

	function tblParosAutomaticos()
	{
		$conecta = new ClassConexion();
		...
		$query = "SELECT * FROM vwBitacoraParosMaquinas
				  WHERE Fecha >= '$fechai' AND Fecha <= '$fechaf' AND NoMaquina=" . $maquina . " $turno ORDER BY HoraParo DESC";
		$array = array();
		$result = sqlsrv_query($conn, $query);
		while ($row = sqlsrv_fetch_array($result)) {
			array_push($array, [
				...
				"horaInicio" => $row['HoraParo']->format('H:i:s'),
...
if (isset($_GET["tblasistenciasbitrep"])) {
	...
	$ReporteBit->tblParosAutomaticos();
}
?>