class Proact extends Herramientas {
  // ---------------------------------------------------------------------------------------------------------------------------------
  // SECCION DE REPORTES PHP -> proact.php
  // ---------------------------------------------------------------------------------------------------------------------------------
  inicio() {
    // Carga de las observaciones en tabla (Busqueda entre el tipo de la observacion)
    // Segun Observaciones o reglas criticas carga lo nuevo de cada uno
    // Php -> Proact
    document.getElementById("observacion").addEventListener("change", () => {
      // Busqueda del tipo de observacion
    ...
    // Llamada a tblpro para actualizar registros Php -> Proact
    this.tblpro();

    // Funcion para actualizar la hora cada segundo Php -> Proact
    setInterval(this.mostrarHora, 1000);

    // Select de usuarios (noemp & nombre) para select Php -> Proact
    this.llnarslcruta("php/index.php?Usuarios", "nombres");

    // Select de area para select Php -> Proact / Reporte
    this.llnarslcruta("php/index.php?Areas", "areas");

    // Observaciones y Reglas Criticas -> Actualiza a observacion Php -> Proact
    // Llenado de select en Observacion
    this.llnarslcruta("php/index.php?Observacion", "observacion");

    // Comporamiento de proactivo (Fatalidad, Lesion N/A) Php -> Proact
    this.llnarslcruta("php/index.php?proactComportamientos", "comportamiento");

    // Funcion para guardar el registro de proactivo Php -> Proact
    function guardar() {
      Swal.fire({
        title: "¿Estas seguro?",
        ...
        if (result.isConfirmed) {
          (async () => {
            ...
            // Validacion de campos vacios despues de mandar la solicitud no antes
    ...
    document.getElementById("areas").addEventListener("change", function () {
      ...
      // Construccion de query segun el departamento
    ...
    // Llamado a funcion con boton de guardar
    // Php -> Proact
    ...
    // Php -> Proact
    // Carga de informacion al quitar el focus del input
  ...
  // Php ->
  // Llenado de observaciones de ambos tipos en dos select
  ...
  // Funcion de carga de datos para Php -> proact
  async tblpro() {
    // Busqueda de datos
    ...
    resultado.forEach((elemento) => {
      body += `<tr>
                <td>${elemento.id}</td>
                <td>${elemento.Observador}</td>
                <td>${elemento.Observado}</td>
                <td>${elemento.Area}</td>
  ...
  // ---------------------------------------------------------------------------------------------------------------------------------
  // SECCION DE REPORTES PHP -> reporte.php
  // ---------------------------------------------------------------------------------------------------------------------------------
  reporte() {
    this.llnarslcruta("php/index.php?Areas", "areas");
    document.getElementById("areas").addEventListener("change", function () {
      ...
      // Llenado de maquina segun el area
      (async function () {
        const maquinas = await fetch("php/index.php?Maquinas&areas=" + areas);
        const respuestamaquinas = await maquinas.json();

        // Instancia de clase para llnarslcruta
        const herramientas = new Herramientas();
        herramientas.llnarslcruta("php/index.php?Maquinas&areas=" + areas,"maquinas");
      })();
    });

    // Accion para el boton de consultar info
    document.getElementById("consulta").addEventListener("click", function (event) {
        ...
        (async () => {
          const [
            ...
            // Busqueda de fecha y maquina
            fetch("php/index.php?consultarxfechamaq", {
              method: "POST",
              body: data,
            }),
            // Busqueda para observaciones con las que el trabajador no cumple
            fetch("php/index.php?consultarxfechaobs", {
              method: "POST",
              body: data,
            }),
            // Top observaciones por maquina (Grafica de barras)
            ...
            // Busqueda para total de personas observadas por maquina (grafica de pastel)
          ...
          // Validacion de datos en caso de faltar las fechas y recuperacion de datos
          const respuesta = await respuestaraw.json();
          ...
          // Creacion de objeto para graficas
          ...
          // Respuesta global de info
          const respuestadatos = await datos.json();

          //Insercion de informacion en la tabla de reporte general
          let body = "";
          respuestadatos.forEach((elemento) => {
            ...
            <td>${elemento.Observado}</td>
            <td>${elemento.Obsnombre}</td>
            <td>${elemento.opcion == 1 ? "SI" : "NO"}</td>
            <td>${elemento.Area}</td>
            <td>${elemento.Maquina}</td>
            ...
            <td>${elemento.Otra}</td>
            <td>${elemento.Deacuerdo == 1 ? "SI" : "NO"}</td>
            </tr>`;
          });
  ...
  reportexavance() {
    ...
    // Acciones del boton de coonsultar info
    document.getElementById("consulta").addEventListener("click", function (event) {
        ...
        (async () => {
          // Formulario
          const form = document.getElementById("reporteLSW");
          // Integracion de datos del form en data
          const data = new FormData(form);
          data.append("data", form);
    ...
    options: {
      onClick: async (evt, elements) => {
        if (elements.length > 0) {
          ...
          const personaId = respuesta.ids[index];
          const personaNombre = respuesta.etiquetas[index];
          const datei = document.getElementById("fechai").value;
          const datef = document.getElementById("fechaf").value;
          const maquina = document.getElementById("maquinas").value;

          const respuestaraw = await fetch(
            "php/index.php?detallepersona&persona=" + personaId +
            "&fechai=" + datei +
            "&fechaf=" + datef +
            "&maquina=" + maquina
          );
          const detalle = await respuestaraw.json();


        if (!Array.isArray(detalle) || detalle.length === 0) {
          document.getElementById("detallePersonaBody").innerHTML = "<tr><td colspan='9' class='text-center text-dark'>No se encontró información</td></tr>";
          document.getElementById("detalleTitulo").innerText =
            `${personaNombre} (${datei} a ${datef})`;
        } else {
          document.getElementById("detalleTitulo").innerText =
            `${personaNombre} (${datei} a ${datef})`;
          ...
          let body = "";
          detalle.forEach(d => {
            const fecha = d.fechainput && d.fechainput.date
              ? d.fechainput.date.split(" ")[0]
              : d.fechainput;
            const hora = d.hora && d.hora.date
              ? d.hora.date.split(".")[0]
              : d.hora;
            ...
            body += `<tr>
              ...
              <td>${fecha}</td>
              <td>${hora}</td>
              <td>${d.observacionreal}</td>
              <td>${d.otrainteraccion}</td>
              <td>${d.comentarios}</td>
            </tr>`;
          });
          document.getElementById("detallePersonaBody").innerHTML = body;
        }
