...
if (isset($_GET["getobservacionest1"])) {
    ...
    $sql = "SELECT TOP 10 tblProactEnc.id,tblProactEnc.observador, tblEmpleados.Nombre, tblDepartamentos.NombreDepto,tblProactAreas.NombreArea,tblProactEnc.fechainput
    FROM tblProactEnc LEFT JOIN TLX032MXDB.dbo.tblEmpleados ON tblEmpleados.NoEmp= tblProactEnc.observado
    LEFT JOIN TLX009MXDB.dbo.tblDepartamentos ON tblDepartamentos.NoDepto= tblProactEnc.departamento 
    INNER JOIN TLX003MXDB.dbo.tblProactAreas ON tblProactAreas.Id = tblProactEnc.maquina WHERE tblProactEnc.observador=" . $_SESSION['ibm'] . "  ORDER BY tblProactEnc.id DESC";
    $stpm = sqlsrv_query($conn, $sql);
    $datos = [];
    $i = 0;
...
if (isset($_GET["Observacionreal"])) {
    $id = $_GET["id"];
    Proact::llenarslc("SELECT id,nombre FROM tblObservaciones WHERE tipo=$id Order by nombre", $conn);
}
