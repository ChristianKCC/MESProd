...
const CATALOGO_TURNOS = {
    ...
    mixto1: { nombre: "Mixto 1 (07:30 - 17:00)", entrada: { ideal: 7*60+30, tolerancia: 35 }, salida: { ideal: 17*60, tolerancia: 60 }, cruzaMedia: false, horasReglamentarias: "09:30:00", sabado: { nombre: "Mixto 1 Guardia (Sáb/Dom)", salida: { ideal: 12*60+30, tolerancia: 60 }, horasReglamentarias: "05:00:00" } },
...
async function crearRegistroExcedente(datosRegistro, excedenteMin, motivo, horaInicioExcedente = null, horaFinExcedente = null) {
    ...
    if (!horaInicioExcedente || !horaFinExcedente) {
        ...
        const esSab    = [6, 0].includes(new Date(datosRegistro.fechaSol + "T00:00:00").getDay());
        ...
        horaFinExcedente    = minutosAHoraConSegundos((finMin + excedenteMin) % (24 * 60));
    }
        ...
        if (resp === "Listo") {
            await Swal.fire("Excedente registrado",
                `Se creó un registro adicional de <b>${minutosAHoraConSegundos(excedenteMin)}</b> hrs.<br>
                 Inicio: <b>${horaInicioExcedente}</b> | Fin: <b>${horaFinExcedente}</b>`, "success");
        } else {
...
// ─────────────────────────────────────────────────────────────────────────────
// procesarDoblete  ← REEMPLAZA tu versión activa
//
// Lógica:
//   · Entrada = primer registro del día (o entrada nocturna en turno3).
...
async function procesarDoblete(noemp, date, folioRegistro, turnoAsignado, tipoEmpleado, datosRegistro, mostrarError) {
    ...
    if (esTurnoNocturno) {
        const resol = resolverTurnoNocturno(checador.regAnterior, checador.regActual, checador.regSiguiente);
        ...
        diaSalida  = (resol.escenario.includes("mañana") || resol.escenario.includes("siguiente"))
            ? checador.fechaSiguiente : checador.fechaActual;
    } else {
        // Diurnos / mixtos: entrada = primer registro de hoy
        entradaReg = checador.regActual[0];
        const entradaMin = horaAMinutos(entradaReg.hora_limpia);
        const salidaInfo = resolverSalidaConLapso(entradaMin, checador.regActual, checador.regSiguiente, hrsReglMin, false);
        ...
        diaSalida = salidaInfo.dia === "siguiente" ? checador.fechaSiguiente : checador.fechaActual;
    }
    ...
    const horaInicioExtra = minutosAHoraConSegundos(def.salida.ideal);
    const horaFinExtra    = minutosAHoraConSegundos((def.salida.ideal + excedente) % 1440);

    console.log(`Entrada (${esTurnoNocturno ? "nocturno" : checador.fechaActual}): ${entradaReg.hora_limpia} | Salida (${diaSalida}): ${salidaReg.hora_limpia} | Trabajado: ${minutosTrabajados}min | Excedente: ${excedente}min`);
...
async function procesarDescansoOFestivo(noemp, date, folioRegistro, turnoAsignado, motivo, tipoEmpleado, datosRegistro, mostrarError) {
    ...
    if (!checador.rawActual.length) {
        mostrarError(`No hay registros en el checador para el día ${date}.`, { registrosHoy: "ninguno", registrosAyer: regAyerStr });
        return;
    }
    ...
    console.log(`═══ procesarDescansoOFestivo: folio=${folioRegistro} turno=${turnoAsignado} motivo=${motivo}`);
    console.log(`Entrada: ${entradaReal} | Salida: ${salidaReal} | Trabajado: ${minutosTrabajados}min`);
    ...
    if (minutosTrabajados <= LIMITE_HRS_MIN) {
        ...
        const ok = await ajustarHorasRegistro(folioRegistro, entradaReal, salidaReal);
        if (ok) await Swal.fire("Actualización realizada",
            `Registro validado.<br>Horas ajustadas: <b>${entradaReal}</b> → <b>${salidaReal}</b>`, "success");
    ...
    const salidaOcho      = minutosAHoraConSegundos((entradaMin + LIMITE_HRS_MIN) % (24 * 60));
    ...
    if (ok) {
        await Swal.fire("Registro principal ajustado",
            `Horas ajustadas a 8 hrs: <b>${entradaReal}</b> → <b>${salidaOcho}</b>`, "success");
    }

    // Crear registro excedente con el mismo motivo y razón
    await crearRegistroExcedente(datosRegistro, excedente, motivo, inicioExcedente, finExcedente);
...
async function procesarTurnoNocturno(noemp, date, folioRegistro, turnoAsignado, tipoEmpleado, datosRegistro, mostrarError) {
    ...
    // ─────────────────────────────────────────────────────────────────────────
    const esTurnoExtendido = ["turno3_12hrs", "turno2_13hrs"].includes(turnoAsignado);
    ...
    await Swal.fire({
        ...
        html: `Reglamentarias: <b>${def.horasReglamentarias}</b><br>
               ...
               ${esTurnoExtendido
                   ...
                   : `Excedente al <b>final</b> del turno:`}<br>
               Inicio T. extra: <b>${horaInicioExtra}</b> | Fin T. extra: <b>${horaFinExtra}</b>`,
        icon: "success"
    });
    const ok = await ajustarHorasRegistro(folioRegistro, horaInicioExtra, horaFinExtra);
    if (ok) await Swal.fire("Actualización realizada", `Horas ajustadas: <b>${horaInicioExtra}</b> → <b>${horaFinExtra}</b>`, "success");
    const fid = window.Reportes.data.find(e => e.folioRegistro == folioRegistro)?.id; if (fid) window.Reportes.evaluarFolio(fid);
}

// ─────────────────────────────────────────────────────────────────────────────
// resolverSalidaConLapso  ← NUEVA (pégala junto a resolverTurnoNocturno)
//
...
//   · dia : "hoy" | "siguiente"
// ─────────────────────────────────────────────────────────────────────────────
function resolverSalidaConLapso(entradaMin, regActual, regSiguiente, hrsReglMin, forzarDiaSiguiente = false) {
    // Salida candidata HOY: último registro posterior a la entrada
    const candidatosHoy = regActual.filter(r => horaAMinutos(r.hora_limpia) > entradaMin);
    const salidaHoy = candidatosHoy.length ? candidatosHoy[candidatosHoy.length - 1] : null;

    // Salida candidata MAÑANA: registro matutino (< 15:00) o el primero disponible
    const salidaMañana = regSiguiente.length
        ? (regSiguiente.find(r => horaAMinutos(r.hora_limpia) < 15 * 60) || regSiguiente[0])
        : null;
...
// ─────────────────────────────────────────────────────────────────────────────
// procesarAnticipo  ← REEMPLAZA tu versión activa
//
// Lógica:
//   1. Entrada = registro dentro del rango solicitado (esto ya funcionaba).
...
async function procesarAnticipo(noemp, date, folioRegistro, turnoAsignado, tipoEmpleado, datosRegistro, mostrarError) {
    ...
    if (checador.regActual.length < 1) {
        mostrarError(`No hay registros en el checador para el día ${date}.`, { registrosHoy: "ninguno", registrosMañana: regMañanaStr });
        return;
    }

    // ── Entrada: primer registro dentro del rango solicitado ──────────────────
    const regDentroRango = checador.regActual.find(r => {
        if (!r.hora_limpia) return false;
        const min = horaAMinutos(r.hora_limpia);
        return min >= horaiSolicitadaMin && min <= horafSolicitadaMin;
    });
    ...
    if (!regDentroRango) {
        ...
        await Swal.fire({
            ...
            html: `Solicitó <b>${minutosAHoraConSegundos(hrsSolicitadasMin)}</b> pero no se encontró ningún registro en el rango
                   <b>${datosRegistro.HoraInicio} → ${datosRegistro.HoraFin}</b>.<br><br>
    ...
    const salidaInfo = resolverSalidaConLapso(
        entradaMin, checador.regActual, checador.regSiguiente, hrsReglMin, esTurnoNocturno
    );
    ...
    const diaSalida = salidaInfo.dia === "siguiente" ? checador.fechaSiguiente : checador.fechaActual;
    const excedenteMin = Math.max(0, totalTrabajadoMin - hrsReglMin);

    console.log(`Anticipo: entrada=${regDentroRango.hora_limpia}(${checador.fechaActual}) salida=${salidaInfo.salida.hora_limpia}(${diaSalida}) total=${totalTrabajadoMin}min excedente=${excedenteMin}min solicitado=${hrsSolicitadasMin}min`);
    ...
    const inicioAnticipoMin = ((finAnticipoMin - excedenteMin) % 1440 + 1440) % 1440;
...
//     console.log("Horas solicitadas:", datosRegistro.HoraInicio, "→", datosRegistro.HoraFin, "=", hrsSolicitadasMin, "min");
...
//     // // Buscar registro dentro del rango solicitado
//     // const regDentroRango = checador.rawActual.find(r => {
//     //     const min = horaAMinutos(r.fecha_h.split(" ")[1] || r.hora_limpia);
//     //     return min >= horaiSolicitadaMin && min <= horafSolicitadaMin;
//     // });
//     // Buscar registro dentro del rango solicitado usando hora_limpia
//     const regDentroRango = checador.regActual.find(r => {
//         if (!r.hora_limpia) return false;
//         const min = horaAMinutos(r.hora_limpia);
//         return min >= horaiSolicitadaMin && min <= horafSolicitadaMin;
...
//         await Swal.fire({
//             title: "Anticipo — No apto",
//             html: `Solicitó <b>${minutosAHoraConSegundos(hrsSolicitadasMin)}</b> pero no se encontraron registros en el rango <b>${datosRegistro.HoraInicio} → ${datosRegistro.HoraFin}</b>.`,
//             icon: "error"
//         });
//         return;
//     }

//     const inicioAnticipo = horaAMinutos(regDentroRango.hora_limpia);
//     const finAnticipo    = def.entrada.ideal;

//     // Calcular excedente real contra salida
//     const ultimoReg = checador.regActual[checador.regActual.length - 1];
//     let salidaReal  = horaAMinutos(ultimoReg.hora_limpia);
//     if (salidaReal < inicioAnticipo) salidaReal += 1440;

//     const totalTrabajadoMin = salidaReal - inicioAnticipo;
//     const excedenteMin = totalTrabajadoMin - hrsReglMin;

//     console.log(`Anticipo: inicio=${regDentroRango.hora_limpia} salida=${ultimoReg.hora_limpia} total=${totalTrabajadoMin}min excedente=${excedenteMin}min solicitado=${hrsSolicitadasMin}min`);

//     if (totalTrabajadoMin < 50) {
//         window.Reportes.marcarRegistroComoNoApto(folioRegistro);
//         await Swal.fire({
//             title: "Anticipo — No apto (excedente insuficiente)",
//             html: `Excedente real <b>${minutosAHoraConSegundos(totalTrabajadoMin)}</b> no alcanza 50 min mínimos.<br>Solicitó: <b>${minutosAHoraConSegundos(hrsSolicitadasMin)}</b><br>Registro encontrado: <b>${regDentroRango.hora_limpia}</b>`,
//             icon: "warning"
//         });
//         return;
//     }

//     if (totalTrabajadoMin < hrsSolicitadasMin) {
//         window.Reportes.marcarRegistroComoNoApto(folioRegistro);
//         await Swal.fire({
//             title: "Anticipo — No apto (no cumple solicitado)",
//             html: `Solicitó <b>${minutosAHoraConSegundos(hrsSolicitadasMin)}</b> pero se encontraron <b>${minutosAHoraConSegundos(totalTrabajadoMin)}</b>.`,
//             icon: "error"
//         });
//         return;
//     }
...
//     await Swal.fire({
//         title: "Anticipo — Apto",
//         html: `Solicitó: <b>${minutosAHoraConSegundos(hrsSolicitadasMin)}</b><br>
//                Registro encontrado: <b>${regDentroRango.hora_limpia}</b><br>
//                Excedente real: <b>${minutosAHoraConSegundos(totalTrabajadoMin)}</b><br><br>
...
class Reportes {
    ...
    async getinfohoraentradaysalida(noemp, date, folioRegistro, HoraInicio, turnoAsignado, datosRegistro = null) {
        ...
        console.log(`═══ INICIO validación: folio=${folioRegistro} emp=${noemp} fecha=${date} turno=${turnoAsignado} motivo=${motivo} razon="${razonRegistro}" tipoEmpleado="${tipoEmpleado}"`);
    ...
    renderRow(elemento) {
        ...
        if (elemento.terminado === null || elemento.terminado === "") {
            if (elemento.validado == null || elemento.validado == 0) {
                ...
                accionHtml = 
                    `<span class="badge bg-success">Solicitud validada</span>`; 
            }
            accionTerminadoHtml = 
                    `<span class="badge bg-warning text-dark">En espera de gerente</span>`;
        } else if (elemento.terminado == 1) { 
                accionHtml = 
                    `<span class="badge bg-success">Aprobado</span>`; 
                accionTerminadoHtml = 
                    `<span class="badge bg-success">Aprobado por gerente</span>`; 
                }
