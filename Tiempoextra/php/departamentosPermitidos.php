...
// AJUSTA este include al archivo donde vive ClassConexion (el mismo que usa php/index.php)
require_once "../../conexion.php";
require_once(__DIR__ . "/deptosSupervisor.php");

$ibmSesion = trim((string)($_SESSION['ibm'] ?? ($_SESSION['IBM'] ?? '')));
if ($ibmSesion === '') { echo json_encode(['ok'=>false,'error'=>'Sin sesión IBM','ids'=>[]]); exit; }

// ── CONFIG: ruta real del CSV (ajústala a tu estructura) ──
...
function soloIbm($v) { return preg_replace('/[^\d].*$/', '', trim((string)$v)); }

$ibmBuscado = soloIbm($ibmSesion);

// ── 1) CSV -> nombres de depto (normalizados) del IBM en sesión ──
if (!is_file($rutaCsv)) { echo json_encode(['ok'=>false,'error'=>'No se encontró el CSV','ruta'=>$rutaCsv,'ids'=>[]]); exit; }

$fh = fopen($rutaCsv, 'r');
if ($fh === false) { echo json_encode(['ok'=>false,'error'=>'No se pudo abrir el CSV','ids'=>[]]); exit; }
...
while (($fila = fgetcsv($fh, 0, $delim)) !== false) {
    if ($esEncabezado) { $esEncabezado = false; continue; } // saltar fila 1
    if (!isset($fila[COL_IBM])) continue;
    if (soloIbm($fila[COL_IBM]) !== $ibmBuscado) continue;
