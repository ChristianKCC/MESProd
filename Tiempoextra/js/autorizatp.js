...
// Para el resto: tomar primer y último registro del día.
// ─────────────────────────────────────────────────────────────────────────────
function resolverTurnoNocturno(
  regAnterior,
  regActual,
  regSiguiente,
  salidaIdealMin = null,
  ...
  if (nocturnos.length > 0) {
    const entrada = nocturnos[nocturnos.length - 1];
    if (regSiguiente.length > 0) {
      // Candidatos de salida: registros matutinos del día siguiente (< 15:00)
      const matutinos = regSiguiente.filter(
        (r) => horaAMinutos(r.hora_limpia) < 15 * 60,
      );
      let salidaMañana;
      if (salidaIdealMin !== null && matutinos.length) {
        // Anclar a la salida ideal del turno para descartar checadas espurias
        // a mitad de jornada (p.ej. 03:34) y quedarse con la salida real (07:00).
        salidaMañana =
          seleccionarSalidaCercana(matutinos, salidaIdealMin) ||
          matutinos[matutinos.length - 1];
      } else {
        // Comportamiento original (sin ancla)
        salidaMañana =
          regSiguiente.find((r) => horaAMinutos(r.hora_limpia) < 15 * 60) ||
          regSiguiente[0];
      }
  ...
  // CASO B: solo diurnos hoy → son la salida, entrada en el día anterior
  if (diurnos.length > 0) {
    // Salida: si se proporcionó la ideal, anclar (descarta espurias);
    // si no, el último diurno del día (comportamiento original).
    const salida =
      salidaIdealMin !== null
        ? seleccionarSalidaCercana(diurnos, salidaIdealMin) ||
          diurnos[diurnos.length - 1]
        : diurnos[diurnos.length - 1];
    const nocturnosAyer = regAnterior.filter(
      (r) => horaAMinutos(r.hora_limpia) >= 18 * 60,
    );
...
// ─────────────────────────────────────────────────────────────────────────────
// seleccionarSalidaDespuesDe (auxiliar)
//
// Dado el minuto de la entrada ya elegida, devuelve el ÚLTIMO registro del día
// posterior a esa entrada (la salida real). Si no hay ninguno posterior,
// devuelve el último registro disponible como respaldo.
...
// ─────────────────────────────────────────────────────────────────────────────
// seleccionarSalidaCercana ← NUEVA
//
// Elige, de una lista de registros candidatos, el que esté MÁS CERCA de la
// salida IDEAL del turno (contemplando cruce de medianoche). Sirve para
// descartar checadas espurias a mitad de jornada — p.ej. una persona que checa
// por error a las 03:34 en un turno3_12hrs (19:00→07:00) — y quedarse con la
// salida real (07:00), SIN saltar a registros de otra jornada porque solo se le
// pasan candidatos de la misma ventana (matutinos < 15:00 o diurnos del día).
//
//   candidatos     : registros candidatos (limpios)
//   salidaIdealMin : minuto de la salida ideal del turno (def.salida.ideal)
//   margenMin      : ventana máxima permitida alrededor de la ideal.
//                    null = sin límite (siempre devuelve el más cercano).
//                    Si se define y NINGÚN candidato cae dentro, devuelve null
//                    (mejor reportar "sin salida" que tomar una hora errónea).
// ─────────────────────────────────────────────────────────────────────────────
function seleccionarSalidaCercana(
  candidatos,
  salidaIdealMin,
  margenMin = null,
) {
  ...
  const dist = (min) =>
    Math.min(
      Math.abs(min - salidaIdealMin),
      Math.abs(min - salidaIdealMin + 1440),
      Math.abs(min - salidaIdealMin - 1440),
    );
  let mejor = null,
    mejorDist = Infinity;
  for (const r of candidatos) {
    const d = dist(horaAMinutos(r.hora_limpia));
    if (d < mejorDist) {
      mejorDist = d;
      mejor = r;
    }
  }
  if (margenMin !== null && mejorDist > margenMin) return null;
  return mejor;
}
...
async function procesarTurnoNocturno(
  ...
  // Se pasa def.salida.ideal para que resolverTurnoNocturno ancle la salida a la
  // hora ideal del turno y descarte checadas espurias a mitad de jornada
  // (p.ej. un 03:34 colado en un turno3_12hrs 19:00→07:00).
  const resolucion = resolverTurnoNocturno(
    checador.regAnterior,
    checador.regActual,
    checador.regSiguiente,
