...
function obtenerMotivosDelSelect() {
    ...
    for (let i = 0; i < select.options.length; i++) {
        ...
        if (opt.value) opciones.push({ value: opt.value, text: opt.text, disabled: opt.disabled });
    }
    return opciones;
}
...
const TURNOS_MULTIDIA = {
    turno1: { label: "Turno 1 (07:00-15:00)", inicioSalida: "15:00:00", inicioTurno: "07:00:00" },
    turno2: { label: "Turno 2 (15:00-22:30)", inicioSalida: "22:30:00", inicioTurno: "15:00:00" },
    turno3: { label: "Turno 3 (22:30-07:00)", inicioSalida: "07:00:00", inicioTurno: "22:30:00" },
    mixto1: { label: "Mixto 1 (07:30-17:00)", inicioSalida: "17:00:00", inicioTurno: "07:30:00" },
    mixto2: { label: "Mixto 2 (08:30-18:30)", inicioSalida: "18:30:00", inicioTurno: "08:30:00" },
    mixto3: { label: "Mixto 3 (07:00-16:30)", inicioSalida: "16:30:00", inicioTurno: "07:00:00" },
    mixto4: { label: "Mixto 4 (07:00-17:00)", inicioSalida: "17:00:00", inicioTurno: "07:00:00" },
};
...
// ─────────────────────────────────────────────────────────────────────────────
function makeBid(fecha, idx) { return fecha + "__" + idx; }
function bidToFecha(bid)     { return String(bid).split("__")[0]; }
...
function getTiempoextra() {
    if (typeof window !== "undefined" && window.Tiempoextra) return window.Tiempoextra;
    try { if (typeof Tiempoextra !== "undefined" && Tiempoextra) return Tiempoextra; } catch (e) {}
    return null;
}
...
function toggleModoMultiple(checkbox) {
    ...
    if (checkbox.checked) {
        if (!folio) {
            Swal.fire("Sin folio", "Primero debes crear o seleccionar un folio para usar la captura múltiple.", "warning");
...
function renderizarPanel() {
    ...
    const motivosHtml = motivos
        .map(m => `<option value="${m.value}" ${m.disabled ? "disabled" : ""}>${m.text}</option>`)
        .join("");
    ...
    // Borrar tarjeta de la vista despues de Nuevo
    // container.innerHTML = `
    //     <div class="card mt-3">
    ...
    //                     <i class="fa-solid fa-circle-question"></i> ¿Quieres ver un tutorial de la captura multiple?
    //                 </button>
    //                 <button class="btn btn-sm btn-outline-secondary" onclick="seleccionarTodosDias(true)">Activar todos</button>
    //                 <button class="btn btn-sm btn-outline-secondary" onclick="seleccionarTodosDias(false)">Limpiar todos</button>
    //                 <button class="btn btn-success btn-sm" id="btnGuardarMultiple" onclick="guardarTodosDias()">
    ...
    for (let i = 0; i < 7; i++) {
        const f = new Date(_semanaBase); f.setDate(_semanaBase.getDate() + i);
    ...
    if (!localStorage.getItem(tutMultiKey)) {
        setTimeout(() => { lanzarTutorialMulti(); localStorage.setItem(tutMultiKey, "true"); }, 300);
    }
}
...
function agregarSubBloque(fecha) {
    ...
    if (bidsDeFecha(fecha).length >= MAX_REGISTROS_DIA) {
        Swal.fire("Límite alcanzado", `Solo se permiten ${MAX_REGISTROS_DIA} registros por día.`, "info");
        return;
    }
    ...
    aplicarRestriccionIbmEspecial(document.getElementById("multiNoemp")?.value || "");
...
function bidsDeFecha(fecha) {
    const cont = document.getElementById(`subs-${fecha}`);
    if (!cont) return [];
    return Array.from(cont.querySelectorAll(".sub-bloque")).map(b => b.getAttribute("data-bid"));
}
...
function mostrarSubBloque(fecha, idx) {
    ...
    if (paginador) {
        ...
        if (del) del.title = bids.length <= 1 ? "Limpiar este registro" : "Quitar el registro actual";
        // Ocultar "Agregar registro" al llegar al máximo
        if (add) add.style.display = bids.length >= MAX_REGISTROS_DIA ? "none" : "inline-block";
    }
}
...
async function onCambioNoempMulti() {
    ...
    if (!noemp) {
        ...
        for (let i = 0; i < 7; i++) {
            ...
            limpiarCardDia(fs);
            if (chk) { chk.checked = false; toggleCardDia(fs); }
        }
    ...
    if (!TE || typeof TE.getinfoemp !== "function") { avisarSinTiempoextra(); return; }

    document.getElementById("noemp").value = noemp;
    await TE.getinfoemp(noemp);

    const nombreInd = document.getElementById("nombre").value;
    if (!nombreInd) { limpiar(); return; }

    document.getElementById("multiNombre").value       = nombreInd;
    document.getElementById("multiDepartamento").value = document.getElementById("departamento").value;
    document.getElementById("multiPuesto").value       = document.getElementById("puesto").value;
...
function limpiarCardsPrecargadas() {
    for (let i = 0; i < 7; i++) {
        ...
        const algunoPrecargado = bidsDeFecha(fs).some(bid => _diasConfigurados[bid] && _diasConfigurados[bid]._idExistente);
        if (algunoPrecargado) {
            const chk = document.getElementById(`chk-dia-${fs}`);
            limpiarCardDia(fs);
            if (chk) { chk.checked = false; toggleCardDia(fs); }
...
async function copiarMaquinasDesdeIndividual() {
    ...
    for (let intento = 0; intento < 20; intento++) {
        if (origen.options.length > 0) { destino.innerHTML = origen.innerHTML; return; }
    ...
    document.querySelectorAll(".campo-motivo").forEach(select => {
        ...
        for (let i = 0; i < select.options.length; i++) {
            ...
            if (opt.value === "9" || opt.value === "8") {
                if (esEmpleadoNormal) {
                    opt.disabled = true; opt.style.color = "#aaa"; opt.style.backgroundColor = "#f0f0f0";
                    opt.title = "Solo disponible para sindicalizados";
                    if (select.value === "9" || select.value === "8") { select.value = ""; if (bid) onCambioMotivoDia(bid); }
                } else {
                    opt.disabled = false; opt.style.color = ""; opt.style.backgroundColor = ""; opt.title = "";
                }
            }
        }
    });
...
function aplicarRestriccionPrimerRegistro(fecha) {
    ...
    const select = document.querySelector(`.campo-motivo[data-bid="${primerBid}"]`);
    if (!select) return;

    const motivoSel = select.value;

    // Buscar el botón de agregar dentro del paginador de este día
    const btnAdd = document.querySelector(`#paginador-${fecha} .sub-paginador__add`);
    if (!btnAdd) return;
        ...
        for (let i = 0; i < select.options.length; i++) {
            ...
            if (esSegundo && esSindicalizado) {
                ...
                if (MOTIVOS_2DO_SINDICALIZADO.includes(opt.value)) {
                    ...
                    opt.disabled = true; opt.style.color = "#aaa"; opt.style.backgroundColor = "#f0f0f0";
                    opt.title = "En el segundo registro solo se permite Hora de comida o Reducción de horario";
                }
            } else {
        ...
        for (let i = 0; i < select.options.length; i++) {
            ...
            if (esEspecial) {
                if (MOTIVOS_IBM_ESPECIAL.includes(opt.value)) {
                    ...
                    opt.disabled = true; opt.style.color = "#aaa"; opt.style.backgroundColor = "#f0f0f0";
                    opt.title = "Este empleado solo puede solicitar Descanso Trabajado o Día Festivo";
                }
            }
...
async function marcarDiasExistentes(noemp, resincronizar = false) {
    ...
    for (let i = 0; i < 7; i++) {
        ...
        if (resincronizar && chk && chk.checked) {
            const bidsConId = bidsDeFecha(fs).filter(bid => _diasConfigurados[bid] && _diasConfigurados[bid]._idExistente);
        ...
        if (fechasOcupadas[fs]) {
            const detalles = fechasOcupadas[fs].map(rg => `${rg.motivo || ""} ${rg.horai || ""}-${rg.horaf || ""}`.trim()).join(" | ");
            badge.className = "badge bg-warning text-dark ms-auto";
            badge.textContent = fechasOcupadas[fs].length > 1 ? `Ya existe (${fechasOcupadas[fs].length})` : "Ya existe";
            ...
            if (String(badge.textContent).startsWith("Ya existe")) {
                ...
                badge.className = activo ? "badge bg-success ms-auto" : "badge bg-secondary ms-auto";
    ...
    bids.forEach(bid => {
        ...
        if (cfg && cfg._idExistente) {
            ...
            if (!idsVivos.includes(String(cfg._idExistente))) {
                const cont = document.getElementById(`subs-${fecha}`);
                const bloquesActuales = cont ? cont.querySelectorAll(".sub-bloque").length : 0;
...
function cargarMaquinasMulti(idDepto) {
    ...
    if (TE && TE.Tools && typeof TE.Tools.llnarslc === "function") {
        TE.Tools.llnarslc('CatalogoPersonal', 'GetSlcMaquinasxdep&departamento=' + idDepto, 'multiMaquinas', 0);
        return;
    }
...
function actualizarResumenDia(fecha) {
    ...
    bids.forEach(bid => {
        ...
        const motTxt = motSel && motSel.value ? motSel.options[motSel.selectedIndex].text : "";
        const horaI  = document.querySelector(`.campo-horai[data-bid="${bid}"]`)?.value || "";
        const horaF  = document.querySelector(`.campo-horaf[data-bid="${bid}"]`)?.value || "";
        if (motTxt) {
            const h = (horaI && horaF) ? ` ${horaI.substring(0,5)}-${horaF.substring(0,5)}` : "";
    ...
    if (partes.length === 0) {
        ...
        span.textContent = " · " + (partes.length > 1 ? `${partes.length} registros: ` : "") + resumen;
    }
}
...
async function toggleCardDia(fecha) {
    ...
    if (activo) {
        ...
        const yaPrecargado = bidsDeFecha(fecha).some(bid => _diasConfigurados[bid] && _diasConfigurados[bid]._precargado);
        if (tieneExistente && !yaPrecargado) {
            const lista = existentes.map((r, i) => `<li>#${i+1} — <b>${r.motivo||"-"}</b> ${r.horai||"-"} a ${r.horaf||"-"} (${r.razon||"-"})</li>`).join("");
        ...
        if (badge) {
            if (tieneExistente) {
                badge.className = "badge bg-warning text-dark ms-auto";
                badge.textContent = existentes.length > 1 ? `Ya existe (${existentes.length})` : "Ya existe";
            } else {
...
function inferirTurnoDesdeHoras(horai, horaf) {
    ...
    if (hi === "19:00" && hf === "07:00") return { turnoBase: "turno3", especial: "12hrs" };
    if (hi === "10:30" && hf === "22:30") return { turnoBase: "turno2", especial: "12hrs" };
    const finTurnoATurno = { "15:00":"turno1","22:30":"turno2","07:00":"turno3","17:00":"mixto1","18:30":"mixto2","16:30":"mixto3" };
    if (finTurnoATurno[hi]) return { turnoBase: finTurnoATurno[hi], especial: "normal" };
    return null;
}
...
function precargarDatosExistentes(bid, reg) {
    ...
    const turnoReg = String(reg.turnoAsignado ?? reg.turno ?? reg.turnosel ?? "").trim();
    const razonReg = (reg.razon || "").trim();

    let especial  = "normal";
    let turnoBase = turnoReg;

    if (turnoReg === "turno3_12hrs")      { turnoBase = "turno3"; especial = "12hrs"; }
    else if (turnoReg === "turno2_13hrs") { turnoBase = "turno2"; especial = "12hrs"; }

    if (razonReg === "Anticipo")  especial = "anticipo";
    if (razonReg === "Reingreso") { especial = "reingreso"; turnoBase = "turno1"; }

    if (!turnoBase && reg.horai && reg.horaf && especial === "normal") {
        const inferido = inferirTurnoDesdeHoras(reg.horai, reg.horaf);
        if (inferido) { turnoBase = inferido.turnoBase; if (inferido.especial) especial = inferido.especial; }
    }
    ...
    if (motSel && reg.motivo) {
        for (let i = 0; i < motSel.options.length; i++) {
            if (motSel.options[i].text.trim() === String(reg.motivo).trim()) { motSel.value = motSel.options[i].value; break; }
        }
        onCambioMotivoDia(bid);
    }
    ...
    if (turSel && turnoBase) {
        const existeOpcion = Array.from(turSel.options).some(o => o.value === turnoBase);
        if (existeOpcion) {
    ...
    if (durEl) {
        ...
        } else if (especial === "normal" && reg.horai && reg.horaf && ![8,9,10,12].includes(motivoNum)) {
            const dur = calcularDuracionEntre(reg.horai, reg.horaf);
            if (dur) { durEl.value = dur; _diasConfigurados[bid].duracion = dur; }
        }
    }
    ...
    const espEl = document.querySelector(`input[name="especial-${bid}"][value="${especial}"]`);
    if (espEl) espEl.checked = true;

    if (especial === "reingreso" && reg.horai) {
        const manWrap = document.querySelector(`.campo-hora-manual-wrap[data-bid="${bid}"]`);
        if (manWrap) manWrap.style.display = "block";
        const manEl = document.querySelector(`.campo-hora-manual[data-bid="${bid}"]`);
...
function calcularDuracionEntre(horaI, horaF) {
    try {
        const hm = (h) => { const p = h.split(":").map(Number); return p[0]*60 + p[1]; };
        let ini = hm(horaI), fin = hm(horaF);
        if (fin < ini) fin += 1440;
        const diff = fin - ini;
        if (diff <= 0) return null;
        return Math.floor(diff/60).toString().padStart(2,"0") + ":" + String(diff%60).padStart(2,"0");
    } catch (e) { return null; }
}
...
function onCambioMotivoDia(bid) {
    ...
    const show = (sel) => { const el = document.querySelector(sel); if (el) el.style.display = "block"; };
    ...
    // ───────────────────────────────────────────────
    const fecha = document.querySelector(`.campo-motivo[data-bid="${bid}"]`)?.closest(".card-dia")?.dataset?.fecha;
    if (fecha) {
        const bids = bidsDeFecha(fecha);
...
function evaluarVisibilidad12hrs(bid) {
    const turno   = document.querySelector(`.campo-turno[data-bid="${bid}"]`)?.value || "";
    const durStr  = document.querySelector(`.campo-duracion[data-bid="${bid}"]`)?.value || "";
    const wrap    = document.querySelector(`.chk12-wrap[data-bid="${bid}"]`);
    const radio12 = document.querySelector(`input[name="especial-${bid}"][value="12hrs"]`);
    if (!wrap) return;

    const hm = (h) => { const [a,b] = h.split(":").map(Number); return a*60+b; };
    const minutos = durStr && /^\d{1,2}:\d{2}$/.test(durStr) ? hm(durStr) : 0;

    if ((turno === "turno3" || turno === "turno2") && minutos >= 210 && minutos <= 270) {
        wrap.style.display = "inline-block";
    } else {
        wrap.style.display = "none";
        if (radio12 && radio12.checked) {
            const normal = document.querySelector(`input[name="especial-${bid}"][value="normal"]`);
            if (normal) { normal.checked = true; onEspecialDia(bid, "normal"); }
        }
    }
}
...
function onEspecialDia(bid, valor) {
    ...
    const wrap = document.querySelector(`.campo-hora-manual-wrap[data-bid="${bid}"]`);
    if (wrap) wrap.style.display = valor === "reingreso" ? "block" : "none";

    if (valor === "12hrs") {
        const turnoSel = document.querySelector(`.campo-turno[data-bid="${bid}"]`)?.value || "";
        const durEl = document.querySelector(`.campo-duracion[data-bid="${bid}"]`);
        if (durEl) {
            if (turnoSel === "turno3")      { durEl.value = "03:30"; _diasConfigurados[bid].duracion = "03:30"; }
            else if (turnoSel === "turno2") { durEl.value = "04:30"; _diasConfigurados[bid].duracion = "04:30"; }
        }
    }
    ...
    if (razonEl) {
        if (valor === "anticipo")  razonEl.value = "Anticipo";
            ...
            if (razonEl.value === "Anticipo" || razonEl.value === "Reingreso") razonEl.value = "";
        }
    }
...
function recalcularHorasDia(bid) {
    const turnoSel  = document.querySelector(`.campo-turno[data-bid="${bid}"]`)?.value || "";
    const durStr    = document.querySelector(`.campo-duracion[data-bid="${bid}"]`)?.value || "";
    const especial  = document.querySelector(`input[name="especial-${bid}"]:checked`)?.value || "normal";
    ...
    const motivoNum = parseInt(document.querySelector(`.campo-motivo[data-bid="${bid}"]`)?.value || "0");
    if (!horaInpI || !horaInpF) return;

    const hm = (hora) => { const [h,m] = hora.split(":").map(Number); return h*60+m; };
    const mh = (min) => { min = ((min % 1440) + 1440) % 1440; return Math.floor(min/60).toString().padStart(2,"0")+":"+String(min%60).padStart(2,"0")+":00"; };

    if (motivoNum === 8) {
        const cambioEl = document.querySelector(`.campo-cambio[data-bid="${bid}"]`);
        const hra = parseFloat(cambioEl?.value || "3");
        if (hra === 3)        { horaInpI.value = "01:00:00"; horaInpF.value = "04:00:00"; }
        else if (hra === 5)   { horaInpI.value = "01:00:00"; horaInpF.value = "06:00:00"; }
        else if (hra === 2.5) { horaInpI.value = "01:00:00"; horaInpF.value = "03:30:00"; }
        return;
    }
    ...
    if ((motivoNum === 10 || motivoNum === 12) && turnoSel && TURNOS_MULTIDIA[turnoSel]) {
        horaInpI.value = TURNOS_MULTIDIA[turnoSel].inicioTurno;
        horaInpF.value = TURNOS_MULTIDIA[turnoSel].inicioSalida;
        return;
    }

    if (!turnoSel || !TURNOS_MULTIDIA[turnoSel]) { horaInpI.value = ""; horaInpF.value = ""; return; }

    if (especial === "reingreso") {
        const horaManual = document.querySelector(`.campo-hora-manual[data-bid="${bid}"]`)?.value || "";
        horaInpI.value = horaManual || "";
        horaInpF.value = "00:00:00";
        return;
    }

    if (!durStr || !/^\d{1,2}:\d{2}$/.test(durStr)) { horaInpI.value = ""; horaInpF.value = ""; return; }
    ...
    if (especial === "anticipo") {
        ...
        if (turnoSel === "turno2")      { horaInpI.value = "10:30:00"; horaInpF.value = "22:30:00"; }
        else if (turnoSel === "turno3") { horaInpI.value = "19:00:00"; horaInpF.value = "07:00:00"; }
        else { horaInpI.value = ""; horaInpF.value = ""; }
    } else {
...
async function copiarConfiguracion(fechaOrigen) {
    ...
    for (let i = 0; i < 7; i++) {
        ...
        const yaConfig = bidsDeFecha(fs).some(bid => (document.querySelector(`.campo-motivo[data-bid="${bid}"]`)?.value || "") !== "");
        otrosDias.push({ fecha: fs, label: DIAS_SEMANA[i] + " " + fs, yaConfigurado: yaConfig });
    }

    const disponibles = otrosDias.filter(d => !d.yaConfigurado);
    if (disponibles.length === 0) {
        Swal.fire("Sin días disponibles", "Todos los demás días ya tienen configuración. Limpia alguno para sobrescribirlo.", "info");
        return;
    }

    const checkboxesHtml = disponibles.map(d => `<div><label><input type="checkbox" value="${d.fecha}" checked> ${d.label}</label></div>`).join("");
    const yaConfigHtml = otrosDias.filter(d => d.yaConfigurado).length > 0
        ? `<hr><small class="text-muted">Días ya configurados (no se sobrescriben): ${otrosDias.filter(d => d.yaConfigurado).map(d => d.label).join(", ")}</small>` : "";
    ...
    const { isConfirmed, value } = await Swal.fire({
        ...
        showCancelButton: true, confirmButtonText: "Copiar", cancelButtonText: "Cancelar",
        preConfirm: () => Array.from(document.querySelectorAll("#swal-dias-copia input[type=checkbox]:checked")).map(c => c.value)
    });
    ...
    const config = bidsOrigen.map(bid => ({
        motivo:  document.querySelector(`.campo-motivo[data-bid="${bid}"]`)?.value || "",
        turno:   document.querySelector(`.campo-turno[data-bid="${bid}"]`)?.value || "",
        dur:     document.querySelector(`.campo-duracion[data-bid="${bid}"]`)?.value || "",
        razon:   document.querySelector(`.campo-razon[data-bid="${bid}"]`)?.value || "",
        espec:   document.querySelector(`input[name="especial-${bid}"]:checked`)?.value || "normal",
        cambio:  document.querySelector(`.campo-cambio[data-bid="${bid}"]`)?.value || "3",
        comida:  document.querySelector(`.campo-comida[data-bid="${bid}"]`)?.value || "01:00",
        horaMan: document.querySelector(`.campo-hora-manual[data-bid="${bid}"]`)?.value || "",
    }));

    value.forEach(fdest => {
        const chk = document.getElementById(`chk-dia-${fdest}`);
        if (chk && !chk.checked) { chk.checked = true; toggleCardDia(fdest); }

        // Asegurar misma cantidad de sub-bloques
        let bidsDest = bidsDeFecha(fdest);
        while (bidsDest.length < config.length) { agregarSubBloque(fdest); bidsDest = bidsDeFecha(fdest); }

        config.forEach((cfg, i) => {
            const bid = bidsDest[i];
            if (!bid) return;
            const motEl = document.querySelector(`.campo-motivo[data-bid="${bid}"]`);
            if (motEl) { motEl.value = cfg.motivo; onCambioMotivoDia(bid); }
            ...
            evaluarVisibilidad12hrs(bid);
            const espEl = document.querySelector(`input[name="especial-${bid}"][value="${cfg.espec}"]`);
            if (espEl) { espEl.checked = true; onEspecialDia(bid, cfg.espec); }
            const razEl = document.querySelector(`.campo-razon[data-bid="${bid}"]`);
    ...
    Swal.fire("Listo", `Configuración copiada a ${value.length} día(s).`, "success");
}
...
function seleccionarTodosDias(activar) {
    for (let i = 0; i < 7; i++) {
        ...
        if (activar) {
            chk.checked = true;
            bidsDeFecha(fs).forEach(bid => { if (!_diasConfigurados[bid]) _diasConfigurados[bid] = {}; _diasConfigurados[bid]._precargado = true; });
            toggleCardDia(fs);
...
function leerConfigBloque(bid) {
    ...
    const horaIEl   = document.querySelector(`.campo-horai[data-bid="${bid}"]`);
    const horaFEl   = document.querySelector(`.campo-horaf[data-bid="${bid}"]`);
    const especEl   = document.querySelector(`input[name="especial-${bid}"]:checked`);
    const cambioEl  = document.querySelector(`.campo-cambio[data-bid="${bid}"]`);
    const comidaEl  = document.querySelector(`.campo-comida[data-bid="${bid}"]`);
    const horaManEl = document.querySelector(`.campo-hora-manual[data-bid="${bid}"]`);
    ...
    if (motivoNum === 8) {
        const hra = parseFloat(cambioEl?.value || "3");
        if (hra === 3)        { horai = "01:00:00"; horaf = "04:00:00"; }
        else if (hra === 5)   { horai = "01:00:00"; horaf = "06:00:00"; }
        else if (hra === 2.5) { horai = "01:00:00"; horaf = "03:30:00"; }
        turno = "turno1";
    } else if (motivoNum === 9) {
        const dur = comidaEl?.value || "01:00";
        horai = "12:00:00";
        ...
        horaf = Math.floor(fin/60).toString().padStart(2,"0") + ":" + String(fin%60).padStart(2,"0") + ":00";
        turno = "turno1";
    }

    if (especial === "anticipo")  razon = "Anticipo";
    if (especial === "reingreso") { razon = "Reingreso"; horai = horaManEl?.value || "00:00:00"; horaf = "00:00:00"; turno = "turno1"; }
    if (especial === "12hrs") {
        if (turno === "turno3") turno = "turno3_12hrs";
        else if (turno === "turno2") turno = "turno2_13hrs";
    }

    return { bid, fecha, motivo: motivoEl?.value || "", turno, turnoBase, horai, horaf, razon, duracion, especial,
...
function validarConfigBloque(cfg) {
    ...
    if (![8,9].includes(parseInt(cfg.motivo)) && cfg.especial !== "reingreso" && !cfg.turno) errores.push("Falta el turno.");
    return errores;
}
...
function parseRespuestaSegura(texto) {
    ...
    if (["Listo","Existe","LimiteSemana"].includes(sinComillas)) return sinComillas;
...
function descripcionError(resp, textoRaw) {
    if (resp && typeof resp === "object") { if (resp.message) return resp.message; return JSON.stringify(resp); }
    if (typeof resp === "string" && resp) return resp;
    const frag = String(textoRaw || "").replace(/\s+/g, " ").trim().slice(0, 120);
    return frag ? ("Respuesta inesperada del servidor: " + frag) : "Respuesta vacía del servidor.";
}
...
async function guardarTodosDias() {
    ...
    if (!folio)  { Swal.fire("Sin folio", "Debes crear un folio primero.", "warning"); return; }
    if (!noemp || !nombre) { Swal.fire("Sin empleado", "Ingresa el número de empleado.", "warning"); return; }
    if (!maquina) { Swal.fire("Sin maquina", "Selecciona una maquina.", "warning"); return; }
    ...
    for (let i = 0; i < 7; i++) {
        ...
        const fs = formatFecha(f);
        const chk = document.getElementById(`chk-dia-${fs}`);
        if (chk && chk.checked) { diasActivos.push(fs); bidsActivos = bidsActivos.concat(bidsDeFecha(fs)); }
    }
    if (bidsActivos.length === 0) { Swal.fire("Sin días", "Activa al menos un día para guardar.", "warning"); return; }
    ...
    bidsActivos.forEach(bid => {
        ...
        if (errs.length > 0) erroresPrevios.push(`<b>${cfg.fecha}</b> (registro): ${errs.join(" ")}`);
    });
    if (erroresPrevios.length > 0) {
        Swal.fire("Campos incompletos", `Corrige antes de guardar:<br><br>${erroresPrevios.join("<br>")}`, "warning");
        return;
    }
    ...
    if (IBM_ESPECIALES.includes(parseInt(noemp))) {
        ...
        bidsActivos.forEach(bid => {
            ...
            if (!MOTIVOS_IBM_ESPECIAL.map(Number).includes(m)) diasInvalidos.push(bidToFecha(bid));
        });
        if (diasInvalidos.length > 0) {
            Swal.fire('Restricción',
    ...
    const confirm = await Swal.fire({
        ...
        icon: "question", showCancelButton: true, confirmButtonText: "Sí, guardar todos", cancelButtonText: "Cancelar"
    });
    if (!confirm.isConfirmed) return;

    // ── Validación de doblete + 12HRS por bloque (igual que el individual) ──
    const razonOverride = {};
    if (typeof evaluarDoblete === "function" && typeof horaAMinutos === "function") {
        for (const bid of bidsActivos) {
            ...
            const esSabado = [6,0].includes(new Date(cfg.fecha + "T00:00:00").getDay());
            ...
            if (resultado === "12HRS" && cfg.especial !== "12hrs") {
                await Swal.fire("Falta marcar 12 hrs",
            ...
            if (resultado === "ERROR_LIMITE") {
                await Swal.fire("Cantidad no permitida",
                    `<b>${cfg.fecha}</b> — En turno <b>${cfg.turnoBase}</b> la cantidad (${durStr}) no es válida. Corrige ese registro.`, "warning");
                return;
            }
            ...
            if (resultado === "DOBLETE" && cfg.especial !== "anticipo") {
                const cd = await Swal.fire({
                    ...
                    icon: "warning", showCancelButton: true, confirmButtonText: "Sí, es doblete", cancelButtonText: "Cancelar todo"
                });
    ...
    for (const fs of diasActivos) {
        ...
        const detalle = existentes.map((r, i) =>
    ...
    const bidsOmitidos  = bidsActivos.filter(bid => diasOmitidos.has(bidToFecha(bid)));
    const totalEfectivo = totalReg - bidsOmitidos.length;
    if (totalEfectivo === 0) {
        Swal.fire("Sin cambios", "No se guardó ningún día (todos fueron omitidos).", "info");
        return;
    }
    ...
    for (const fs of Object.keys(idsABorrarPorDia)) {
        for (const id of idsABorrarPorDia[fs]) {
            try {
                ...
                await fetch("./php/index.php?deletesub", { method: "POST", body: dataDel });
            } catch (e) { console.warn("No se pudo borrar registro previo", e); }
        }
    }
        ...
        try {
            ...
            if (cfg.idExistente) {
                try {
                    ...
                    await fetch("./php/index.php?deletesub", { method: "POST", body: dataDel });
                } catch (eDel) { console.warn("No se pudo borrar registro previo", eDel); }
            }
            ...
            if (resp === "Listo") {
                ...
                const cw = await Swal.fire({
                    title: `Advertencia — ${cfg.fecha}`, text: resp.message, icon: "warning",
                    showCancelButton: true, confirmButtonText: "Sí, guardar igual", cancelButtonText: "Omitir"
                });
                if (cw.isConfirmed) {
                    const dataExt = new FormData(); data.forEach((v, k) => dataExt.append(k, v));
                    const r2 = await fetch("php/index.php?guardartiempoextraExt", { method: "POST", body: dataExt });
                    const resp2 = parseRespuestaSegura(await r2.text());
                    if (resp2 === "Listo") { exitosos++; if (_diasConfigurados[bid]) delete _diasConfigurados[bid]._idExistente; }
                    else fallidos.push({ fecha: cfg.fecha, error: descripcionError(resp2, "") });
                } else fallidos.push({ fecha: cfg.fecha, error: "Omitido por el usuario." });
            } else if (resp && resp.error_especial) {
                fallidos.push({ fecha: cfg.fecha, error: resp.message });
            } else if (resp === "Existe") {
                fallidos.push({ fecha: cfg.fecha, error: "Ya existe un tiempo extra igual." });
            } else if (resp === "LimiteSemana") {
                fallidos.push({ fecha: cfg.fecha, error: "Se alcanzó el límite de 60.5 hrs." });
            } else {
                fallidos.push({ fecha: cfg.fecha, error: descripcionError(resp, textoRaw) });
            }
        } catch (err) {
    ...
    if (btn) { btn.disabled = false; btn.innerHTML = '<i class="fa-solid fa-floppy-disk"></i> Guardar todos los días'; }
    ...
    if (TE) {
        ...
        } catch (e) { console.warn("No se pudo recargar la tabla:", e); }
    }
...
function marcarCardGuardada(fecha) {
    ...
    const badge = card?.querySelector(".card-dia__header .badge");
    if (badge) { badge.className = "badge bg-success ms-auto"; badge.textContent = "Guardado ✓"; }
}
...
function lanzarTutorialMulti() {
    if (!window.driver || !window.driver.js) { console.warn("Librerias no cargadas para mostrar tutorial, contacta al area de sistemas."); return; }
    ...
    if (chk0 && !chk0.checked) { chk0.checked = true; toggleCardDia(fechaPrimera); }
    ...
    const pasos = [
        {
            ...
            popover: {
                ...
                description: "Escribe el número de empleado, sus datos (nombre, departamento, puesto) y sus máquinas se cargarán automáticamente y aplicarán a TODOS los días que registres. Si el departamento no coincide con el del folio, te avisará.",
                side: "bottom"
            }
        },
        {
            ...
            popover: {
                ...
                description: "El nombre, departamento y puesto se llenan solos al buscar el empleado. Estos datos son compartidos: el mismo empleado aplica para todos los días que actives.",
                side: "bottom"
            }
        },
        {
            ...
            popover: {
                title: "Máquina",
                description: "Selecciona la máquina. Se cargan las máquinas del departamento del folio, igual que en el registro individual. La máquina es la misma para todos los días seleccionados.",
                side: "bottom"
            }
        },
        {
            ...
            popover: {
                ...
                description: "Cada día de la semana (Lun a Dom) tiene su propia tarjeta independiente. Puedes configurar cada día con su propio motivo, turno, horas y caso especial.",
                side: "right"
            }
        },
        {
            ...
            popover: {
                ...
                description: "Marca esta casilla para activar el día. Solo los días activos se guardarán al final. Si lo desmarcas, ese día se ignora.",
                side: "right"
            }
        },
        {
        ...
        },
        {
            ...
            popover: {
                ...
                description: "Presiona las flechas de navegacion para desplazarte entre los registros existentes durante el mismo día, al centro encontraras la cantidad de solicitudes existentes para un empleado en un mismo día",
                side: "left"
            }
        },
        {
            ...
            popover: {
                ...
                description: "Presiona este boton para agregar mas de un registro en el mismo dia",
                side: "left",
                popoverClass: "popover-importante"
            }
        },
        {
            ...
            popover: {
                ...
                description: "Cada día puede tener su propio motivo. Según el motivo, se muestran u ocultan los campos correspondientes (igual que en el registro individual). La opción de Hora de Comida solo aparece para empleados sindicalizados.",
                side: "top"
            }
        },
        {
            ...
            popover: {
                title: "Turno (por día)",
                description: "Selecciona el turno de ese día: Turno 1, 2, 3 o Mixto 1-4. La opción de 12 hrs aparece automáticamente solo cuando el turno es 2 o 3 y la cantidad de horas está entre 3:30 y 4:30.",
                side: "top"
            }
        },
        {
            ...
            popover: {
                ...
                description: "Escribe las horas de tiempo extra en formato <b>hh:mm</b>. Los campos De y A se calculan automáticamente según el turno y el caso especial. Para 12 hrs la duración se fija sola (3:30 en turno 3, 4:30 en turno 2).",
                side: "top"
            }
        },
        {
            ...
            popover: {
                title: "Horas De / A",
                description: "Estos campos se llenan solos a partir del turno, la cantidad de horas y el caso especial. No es necesario escribirlos manualmente: para normal suman al final del turno, para anticipo restan al inicio, para 12 hrs usan el horario completo (turno 2: 10:30-22:30, turno 3: 19:00-07:00).",
                side: "top"
            }
        },
        {
            ...
            popover: {
                ...
                description: "Indica si ese día es <b>Normal</b>, <b>12 hrs</b>, <b>Anticipo</b> o <b>Reingreso</b>. Cada caso ajusta las horas De/A de forma distinta. En Reingreso podrás escribir la hora de inicio manualmente.",
                side: "top"
            }
        },
        {
            ...
            popover: {
                title: "Razón (por día)",
                description: "La razón se llena automáticamente según el motivo o el caso especial (Anticipo, Reingreso). Si el motivo es 'Otros', podrás escribirla manualmente. La razón es independiente para cada día.",
                side: "top"
            }
        },
        {
            ...
            popover: {
                ...
                description: "Si varios días comparten la misma configuración, configura uno y cópialo a los demás. Solo aparecerán como destino los días que aún no tengan configuración, para no sobrescribir lo que ya armaste.",
                side: "top"
            }
        },
        {
            ...
            popover: {
                title: "Limpiar día",
                description: "Borra la configuración de ese día específico (en pantalla) sin afectar a los demás. Si el día ya estaba guardado en la base, esto no lo elimina de ahí; solo limpia la tarjeta.",
                side: "top"
            }
        },
        {
        ...
        },
        {
            ...
            popover: {
                ...
                description: "Si necesitas repasar cómo funciona la captura múltiple, presiona este botón para repetir el tutorial cuando quieras.",
                side: "left"
            }
        }
    ];
...
(function () {
    function instalarHookDelete() {
        if (typeof window.deleteSub !== "function") return setTimeout(instalarHookDelete, 300);
        ...
        const envuelta = function (id) {
            ...
            const refrescar = () => {
                ...
                const noemp = document.getElementById("multiNoemp")?.value;
                if (panelVisible && panelVisible.style.display !== "none" && noemp) setTimeout(() => marcarDiasExistentes(noemp, true), 600);
            };
            if (r && typeof r.then === "function") r.then(refrescar); else refrescar();
            return r;
        };
