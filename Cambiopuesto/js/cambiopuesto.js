...
class Folios {
    async consulta() {
        ...
        respuesta.forEach(folio => {
            ...
            if (folio.totalRegistros == 0) {
                accionesHtml = `
                    <span class="badge bg-warning text-dark">
                        No hay registros creados en este folio
                    </span>
                    <button class="btn btn-danger btn-sm" onclick="verPDF(${folio.id})">
                        <i class="fa-solid fa-file-pdf"></i> Consultar PDF
                    </button>
                `;
            } else if (folio.noempSupIntendente && folio.autorizaSupInt === null) {
                accionesHtml = `<span class="badge bg-info text-dark">Solicitud aún no validada por SuperIntendente</span>`;
            } else {
                if (folio.estadoTer == null || folio.estadoTer === "") {
                    accionesHtml = `
                        <button class="btn btn-success btn-sm" onclick="autorizarFolio(${folio.id},1)">
            ...
            if (!folio.estadoTer || folio.estadoTer === 0) {
                pendientes += row;
                countPendientes++;
            } else {
                procesadas += row;
