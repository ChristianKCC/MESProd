...
// ─────────────────────────────────────────────────────────────────────────────
// crearRegistroExcedente — versión actualizada que acepta horas exactas
// ─────────────────────────────────────────────────────────────────────────────
...
// ─────────────────────────────────────────────────────────────────────────────
// procesarDescansoOFestivo — motivos 10 (descanso trabajado) y 12 (día festivo)
//
// Reglas:
//   < 55 min trabajados          → no apto
//   55 min a 8 hrs               → apto, ajustar horas reales (entrada → salida real)
//   > 8 hrs                      → apto, ajustar primer registro a 8 hrs exactas,
//                                   crear registro excedente con el resto (mismo motivo)
//
// IMPORTANTE: Para estos motivos NO se aplica la regla de guardia (sáb/dom).
// El empleado trabajó su día de descanso — se reconocen sus horas reales completas.
// ─────────────────────────────────────────────────────────────────────────────
async function procesarDescansoOFestivo(noemp, date, folioRegistro, turnoAsignado, motivo, tipoEmpleado, datosRegistro, mostrarError) {
    const def = CATALOGO_TURNOS[turnoAsignado];
    if (!def) { mostrarError(`Turno <b>${turnoAsignado}</b> no reconocido.`); return; }
