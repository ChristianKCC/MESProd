...
function u($t){ return utf8_decode($t); }
...
function buscarFirma($ibm, $extensiones) {
    ...
    foreach ($extensiones as $ext) {
        $ruta = "../firmas/{$ibm}.{$ext}";
        if (file_exists($ruta)) return ['ruta' => $ruta, 'w' => 13];
    }
    foreach ($extensiones as $ext) {
        $ruta = "../../FirmaDigital/firmas/{$ibm}.{$ext}";
        if (file_exists($ruta)) return ['ruta' => $ruta, 'w' => 26];
    }
    return null;
}
...
$sigTop = $y;
$lineY  = $sigTop + 8;
$lx1 = $cx + 14;  $lx2 = $mid - 6;   // línea izquierda (Supervisor)
$rx1 = $mid + 6;  $rx2 = $R - 14;    // línea derecha  (Empleado)
$lcen = ($lx1 + $lx2) / 2;
$rcen = ($rx1 + $rx2) / 2;

// Imágenes de firma (si existen)
$firmaSup = buscarFirma($row['Ctt_ibmAutoriza'], $extensiones);
if ($firmaSup) { $w = $firmaSup['w']; $pdf->Image($firmaSup['ruta'], $lcen - $w / 2, $lineY - 9, $w); }
$firmaEmp = buscarFirma($row['Ctt_ibmEmpleado'], $extensiones);
if ($firmaEmp) { $w = $firmaEmp['w']; $pdf->Image($firmaEmp['ruta'], $rcen - $w / 2, $lineY - 9, $w); }
...
// $pdf->SetTextColor(40, 40, 120);
$pdf->SetFont('Arial', '', 8);
$pdf->SetXY($lx1, $lineY + 0.5); $pdf->Cell($lx2 - $lx1, 4, ucwords(strtolower(u($row['NombreSupervisor'] ?? ''))), 0, 0, 'C');
$pdf->SetXY($rx1, $lineY + 0.5); $pdf->Cell($rx2 - $rx1, 4, ucwords(strtolower(u($row['NombreEmpleado'] ?? ''))), 0, 0, 'C');

// Etiquetas
$pdf->SetTextColor(90, 90, 90);
$pdf->SetFont('Arial', '', 7);
$pdf->SetXY($lx1, $lineY + 4.5); $pdf->Cell($lx2 - $lx1, 4, 'Firma del Supervisor', 0, 0, 'C');
$pdf->SetXY($rx1, $lineY + 4.5); $pdf->Cell($rx2 - $rx1, 4, 'Firma del Empleado',   0, 0, 'C');
$y = $lineY + 9.5;
...
// $pdf->SetTextColor(40, 40, 120);
$midY = ($T + $BOT) / 2;
$pdf->RotatedText($L + 5, $midY + 58,
    u('PARA MAYOR INFORMACION FAVOR DE CONTACTAR A SU SUPERVISOR'), 90);
